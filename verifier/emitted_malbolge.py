...
@dataclass(frozen=True, slots=True)
class BoundedWorklistDataMutationObservationSourceContext:
    """Source coordinates for one exact effective committed mutation."""
...
class StaticImageReport:
    ...
    bounded_worklist_evolved_fetch_observation_source_contexts: tuple[
        BoundedWorklistEvolvedReadObservationSourceContext, ...
    ]
...
def _worklist_data_mutation_observation_source_contexts(
    worklist: worklist_transfer.WorklistAnalysis | None,
    cells: tuple[InitialCell, ...],
) -> tuple[BoundedWorklistDataMutationObservationSourceContext, ...]:
    if worklist is None:
        return ()
    contexts: list[BoundedWorklistDataMutationObservationSourceContext] = []
    for index, observation in enumerate(
        worklist.explored_effective_data_mutation_observations
    ):
        ...
        mutation_source = _worklist_mutation_address_source_context(
        ...
        contexts.append(
            BoundedWorklistDataMutationObservationSourceContext(
                ...
                mutation_source_position=mutation_source.source_position,
                mutation_source_byte_offset=mutation_source.source_byte_offset,
            )
        )
    return tuple(contexts)
...
def _worklist_evolved_read_observation_source_contexts(
    ...
    cells: tuple[InitialCell, ...],
) -> tuple[BoundedWorklistEvolvedReadObservationSourceContext, ...]:
    contexts: list[BoundedWorklistEvolvedReadObservationSourceContext] = []
    for index, observation in enumerate(observations):
        ...
        read_source = _worklist_mutation_address_source_context(
            observation.address, cells
        )
        contexts.append(
            BoundedWorklistEvolvedReadObservationSourceContext(
                ...
                data_source_byte_offset=data_source.source_byte_offset,
                initial_data_source_byte=data_source.initial_source_byte,
                read_source_position=read_source.source_position,
                read_source_byte_offset=read_source.source_byte_offset,
                initial_read_source_byte=read_source.initial_source_byte,
            )
        )
    return tuple(contexts)
...
def analyze_source(
    ...
    *,
    ...
    return StaticImageReport(
        ...
        bounded_worklist_non_graphical_fetch_entry_path_source_map=(
            _worklist_control_path_source_map(
                (
                    None
                    if worklist is None
                    or worklist.explored_non_graphical_fetch_witness is None
                    else (
                        worklist.explored_non_graphical_fetch_witness.entry_path
                    )
                ),
                prefix.cells,
            )
        ...
        bounded_worklist_evolved_fetch_observation_source_contexts=(
            _worklist_evolved_read_observation_source_contexts(
                ()
                if worklist is None
                else worklist.explored_evolved_fetch_observations,
                prefix.cells,
            )
        ),
        ...
        bounded_worklist_evolved_data_read_observation_source_contexts=(
            _worklist_evolved_read_observation_source_contexts(
                ()
                if worklist is None
                else worklist.explored_evolved_data_read_observations,
                prefix.cells,
            )
        ),
