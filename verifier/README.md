...
load decode. Schema v20 retains the legacy exact entry-through-fifth
...
For each resolved bounded fetch, schema v20 maps loaded-source addresses back
...
whether the fetched value still equals that initial source byte. Schema v20
...
v20 publishes the first such proof as `bounded_exact_cycle`, including the
...
Schema v20 also records `reachable_cycle_detected` from exact directed edges in
...
a fixed-fetch self-loop. The `b"u="` input-then-crazy fixture closes in 258
states: every byte/EOF branch resolves concretely instead of retaining an
unknown accumulator.
A four-word `b"('&%"` fixture uses it to prove a recurrence-backed fifth
fixed-fetch cycle at `C=4`, `D=29490`, `M[4]=29489`; schema v20 publishes that
exact transition and its bounded memory footprint. Schema v20 keeps sixteen
