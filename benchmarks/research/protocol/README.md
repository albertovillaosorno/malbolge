...
`deterministic.benchmark.toml` demonstrates equivalent-workload measurement with explicit
warmup, retained samples, dispersion, uncertainty, resources, and a retain-all
outlier policy. `stochastic.benchmark.toml` additionally fixes one seed per trial and keeps
a failed trial in the raw data instead of silently dropping it.

The validator is `scripts/validate/benchmark_protocol.py`. New research
