...
`malbolge-static-image/v20` retains exact `entry_transition` through
...
known. Schema v20 publishes the first such repeated-state proof as
`bounded_exact_cycle`, including first-seen/repeated transition indices, period,
registers, and sparse memory overrides. Schema v20 also publishes
...
Schema v20 adds opt-in `bounded_worklist` evidence under an explicit
...
nonzero. Schema v20 derives `reachable_cycle_detected` from exact directed
...
nonzero. Schema v20 models a provable non-graphical fixed fetch as an exact
...
The two-word `b"u="` fixture (input then crazy) closes in 258 states,
...
Schema v20 publishes `bounded_fetch_source_map`: each resolved instruction
...
fetch value. Schema v20 separately publishes `bounded_fetch_value_lineage` for
...
Schema v20 keeps 16 exact transitions as the default and admits an explicit
