# Repository implementation baseline — 2026-08-05
...
Preserve the exact implementation-status narrative removed from the root README
when planning was consolidated. This record is historical evidence, not current
runtime authority or unfinished-work state.

## Scope

The snapshot covers repository behavior and planning state as observed on
2026-08-05. Current behavior remains governed by narrower technical contracts,
ADRs, code, manifests, tests, and the live typed TODO registry.
...
At snapshot time, `TODO.md` contained 80 unfinished headings with one typed
record each under `docs/todo/open/`. A TODO disappeared only after its contract,
implementation or research result, tests, and evidence became durable.

## Invariants

- This record is a dated historical snapshot rather than current policy.
- Narrower contracts, ADRs, code, manifests, tests, and typed TODO records
  remain authoritative for current behavior and unfinished work.
- The snapshot is preserved verbatim except for link removal and line wrapping
  needed to make it a durable standalone record.

## Failure Behavior

A contradiction with current repository state is resolved in favor of the
current owning authority. This snapshot is never used as runtime, build,
verification, or planning input.

## Verification

- Jig validates the document profile, local catalog entry, links, and text
  policy.
- Planning-index tests reject migrated root-detail and README-snapshot markers
  in active typed TODO records.

## References

- [Repository responsibility scaffold](repository-responsibility-model.md)
- [Documentation authority taxonomy](../adr/documentation-authority-taxonomy.md)
- [Planning corpus promotion
  contract](../contracts/planning-corpus-promotion.md)
