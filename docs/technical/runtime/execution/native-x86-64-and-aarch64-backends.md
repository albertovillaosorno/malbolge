...
The fused emitter now covers three reviewed two-step regions on x86-64 and
AArch64: rotate/output, no-operation/output, and no-operation/no-operation.
Every fused guard completes before the first store, including region-entry
live-ins and output capacity where output is present. Independent semantic
promotion reconstructs admission, structurally admits COFF, rebuilds canonical
bytes, and rejects text-byte drift. The no-operation templates commit their
code-cell encryptions plus only the final region state, without publishing an
intermediate one-step observation.

VM-derived traces exercise both no-operation templates on both ISAs through
emission, semantic promotion, relocation-free load-image extraction, and
whole-region state comparison with the profile VM.
...
no-operation/output, and no-operation/no-operation objects are also emitted,
independently verified, and extracted as relocation-free load images on both
ISAs, with dedicated safe lifecycle
