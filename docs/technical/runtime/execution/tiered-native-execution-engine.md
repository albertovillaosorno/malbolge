...
retry keeps only mappings whose release still fails.
...
`RegisterMaskedNativeLeaseCache` now owns weighted multi-entry v6 resident
policy with caller-selected published limits. Active residents retain exact
program, artifact, key, mapping, and mapped-byte weight under FIFO lookup
authority. Hits clone the same
`Arc`
...
Explicit limit reconfiguration expands without adapter work and shrinks active
FIFO authority through release-or-retire processing. Existing retired entries
are never reconciled implicitly; blocked or failed publication preserves the
previous limits plus exact retry evidence. Durable storage and cross-process
leasing remain separate policy boundaries.
...
A reusable Output owner now keeps the
verified program, artifact, and ready mapping across rebased calls. Platform
mapped length defines its residency weight and release failure retains exact
retry ownership.
...
An uncovered result retains both its complete `NativeArtifactKey` and exact
`RegionEffectProgram`, so later JIT compilation and semantic admission consume
the same IR authority that produced the miss. Ordered sequences require complete
