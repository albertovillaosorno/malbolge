...
`SearchRequest` binds an algorithm ID, deterministic seed, evaluation budget,
and
...
recorded independently. `SearchSelection` plus `SearchAdapterBinding` resolve
one
...
257-candidate corpus, `SearchRunIdentity` records CUDA as actual execution, and
a
...
seed/budget, and hex-encoded proposals marked `untrusted`. A supported CUDA
route
...
0.972x CUDA/CPU ratio. The preregistered speedup hypothesis is therefore
rejected
...
`PreparedEvaluatedSearch` now implements the target: `prepare()` validates
request
...
the
next neutral boundary: fixed-width opaque payloads share one byte buffer and
inherit
logical identity from validated request order. Existing item-based results
remain
...
with an optional candidate-state preparer and backend-specific prepared
evaluator.
...
complete phase total stays flat/slightly worse because proposal selection rises
to
46.331 ms. `PreparedEvaluatedSearch` now builds an immutable exact membership
index from the validated candidate `(logical_id, payload)` pairs. Prepared
CPU/CUDA
...
the
full scan. Missing/excluded positions, forged state, and nonmatching evidence
remain
...
and
backend phases only 1.034x/1.035x, strongly bounding attribution. Primitive
backend
...
1.091x/1.330x while ordinary controls remain nearly flat. Prepared CPU rotate
now
...
(4.243x), and CPU backend evaluation from 13.190 to 2.906 ms (4.540x). CPU
ordinary
...
still validates capability identity, exact result count, and every word's
classic
...
exactly
when a word exceeds the classic maximum. Failure falls back to scalar decoding
only
for diagnostics. First/last-lane threshold and high-bit adversaries fail closed,
and
...
cover resident CUDA launch/sync, transfer, immutable-byte materialization, and
total
...
`u32le-broadword-domain-v1`. Capability, immutable representation, and exact
count
...
membership,
and selector proofs. Retained CUDA prepared search is 0.488 ms, 2.407x better
than
broadword validation and 6.786x faster than same-run CPU. CUDA backend/total
improve
...
0.1935 ms. The 236,196-byte reference and its construction are preparation
costs,
...
`identity-sorted-candidate-reference-binary-search-v1`, a proof-bound sorted
tuple of
references to the original immutable batch items. Binary search locates logical
IDs
...
preparation. Complete prepared retention/peak fall 32.083%/26.051%. Binary
hit/miss
...
records 16.425x CPU prepared, 11.601x CUDA prepared, and 1.270x CUDA
...
- `tests/optimizer/test_search_config.py` verifies schema versioning,
  independent
...
- `tests/optimizer/test_search_cli.py` verifies CPU execution evidence, explicit
  ...
  configured identity preserved, file-backed JSON output, and live CUDA routes
  for
...
- `benchmarks/accelerator/evidence/2026-07-28-prepared-search-rtx4060/`
  ...
  source/workload identity, proposal/admission checks, 1.976x CPU and 1.886x
  CUDA
...
- `benchmarks/accelerator/evidence/2026-07-28-packed-search-rtx4060/` and
  ...
  plus 150 phase samples. All four route medians improve; backend evaluation
  falls
  2.326x on CPU and 2.058x on CUDA, while exact proposals/admission remain
  stable.
<!-- jig-ignore-next-line: canonical path or identifier is indivisible -->
- `benchmarks/accelerator/evidence/2026-07-28-prepared-primitive-search-rtx4060/`
  ...
  Prepared medians improve 1.792x CPU and 1.592x CUDA; ordinary-route
  regressions
...
- `benchmarks/accelerator/evidence/2026-07-28-indexed-membership-search-rtx4060/`
  and its phase sibling retain full index/session identity. Prepared medians
  improve
...
- `benchmarks/accelerator/evidence/2026-07-28-extrema-validation-search-rtx4060/`
  and its phase sibling retain negative/overflow failure plus all prepared
  proofs.
...
<!-- jig-ignore-next-line: canonical path or identifier is indivisible -->
- `benchmarks/accelerator/evidence/2026-07-29-crazy-target-performance-matrix-rtx4060/`
  retains Benchmark Protocol v1 and Experiment Manifest v1 identity,
  75 chronological samples, 59,049/1,024 membership/projection proof, exact
  proposal/admission checks, prepared CPU/CUDA session counters, and one-shot
  ticket
  identity. CPU prepared, CUDA prepared, and CUDA ticket improve 16.425x,
  11.601x,
