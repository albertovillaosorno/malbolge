...
microbenchmark plus crossover/throughput/phase evidence is pending before promotion.
  ...
  increasing immutable tuple of request-order positions to the exact validated
  full-batch object and constructs the evaluator-preserving sub-batch from those
  positions. Mutable, duplicate, reordered, out-of-range, forged, and cross-batch
  subsets fail closed. Rotate adopts the neutral proof under
  `classic-rotate-preimage-position-subset-v2` and wraps it with the exact prepared
  primitive state. Proposal membership and independent trusted admission remain
  full-batch checks. The retained version-8 performance measurements above remain
  attributable only to v1; clean v2 performance evidence is still required before making
  a new speed claim.
