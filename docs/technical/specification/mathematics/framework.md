...
`math/malbolge-notation.tex` is the common notation include for standalone
...
`math/specification/profile-model.tex` defines the profile-width model for a
...
`math/specification/malbolge-1998.tex` remains the normative classic arithmetic
specialization and imports the same notation surface.

### Build and validation boundary

`scripts/validate/math_specifications.py` discovers every standalone `.tex`
...
the document set, shared-notation requirement, cache-only output mapping, and the
...
that executable implementations satisfy them. Exhaustive/property/machine-checked
...
- Build evidence: `scripts/validate/math_specifications.py` compiles all nine
