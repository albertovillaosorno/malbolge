...
OVERLAP_WORKSPACE_ID: Final = "caller-owned-double-window-overlap-u32-arrays-v1"
...
def _open_route_states(
    ...
    try:
        for route_id in ROUTE_IDS:
            ...
            _validate_workspace(workspace, window_items, overlap=overlap)
            states.append(
                _RouteState(
                    allocate=allocation,
                    overlap=overlap,
...
def _validate_summary(
    ...
    *,
    ...
    if overlap and (
        ...
        or summary.prefetched_windows != expected_prefetch_count(window_items)
    ):
