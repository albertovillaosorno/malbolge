...
def validate_repository_lifecycle(path: Path) -> AlgorithmLifecycle:
    ...
    algorithm_id = directory_ids.get(path.parent, path.parent.name)
    if record.identifier != algorithm_id:
        message = (
            "lifecycle algorithm ID does not match canonical directory: "
            f"{record.identifier} != {algorithm_id}"
        )
        _fail(message)
    expected_experiment = (
        (path.parent / "experiment.toml").relative_to(ROOT).as_posix()
    )
...
def validate_repository() -> tuple[AlgorithmLifecycle, ...]:
    ...
    """
    records = [
        validate_repository_lifecycle(directory / "lifecycle.toml")
        for _identifier, directory in research_algorithm_directories(ROOT)
    ]
    if not records:
