...
> dedicated guest validator and six-target matrix remain the hard acceptance
> gates.
...
`generate.py` still defaults to the local oracle for authoring experiments, so
final
acceptance refreshes pass `--after algorithms/doom/quality/out/doom_fixed`
explicitly.
...
```text
python \
  src/research/algorithms/domain/algorithms/doom/quality/comparison/generate.py
```
