// Copyright:
//   - Copyright © 2026 Alberto Villa Osorno.
// SPDX-License-Identifier:
//   - MIT
// Confidential:
//   - false
// License-File:
//   - LICENSE-MIT
//
// Boundary-Contract:
// - Owns:
//   - Single-resident lease reuse for one exact v5 jump/rotate/crazy/halt
//     theorem path.
// - Must-Not:
//   - Mix sequence identities, invoke legacy cache keys, or release live
//     leases.
// - Allows:
//   - Inputs: admitted v5 sequence, executable-memory adapter, and runner.
//   - Outputs: immutable resident leases and explicit release disposition.
//   - Side effects: theorem loading on miss and release after every lease is
//     gone.
// - Split-When:
//   - Multi-entry eviction, weighted capacity, or concurrent mutation is
//     needed.
// - Merge-When:
//   - A general v5 native lease cache subsumes exact theorem path identity and
//     cleanup.
// - Summary:
//   - Reuses one exact seven-mapping theorem owner behind cloneable leases.
// - Description:
//   - Uses full admitted-sequence equality and never canonical legacy identity.
// - Usage:
//   - Ensure a sequence, clone/use leases, drop them, then release the
//     resident.
// - Defaults:
//   - Different identity rejects; live leases block release without adapter
//     work.
//

//! Single-resident lease cache for the complete explicit-geometry crazy
//! theorem path.

use std::fmt::{Display, Formatter, Result as FormatResult};
use std::sync::Arc;

use crate::execution_native::{
    ExecutionGeometryNativeRunner, NativeExecutableMemoryAdapter,
    NativeRegionBuffers,
};
use crate::geometry_native_jump_rotate_crazy_halt_owner::{
    ExecutionGeometryNativeJumpRotateCrazyHaltLoadFailure,
    ExecutionGeometryNativeJumpRotateCrazyHaltReleaseFailure,
    ExecutionGeometryNativeJumpRotateCrazyHaltResidentWeight,
    ExecutionGeometryNativeJumpRotateCrazyHaltResidentWeightError,
    GeometryNativeJumpRotateCrazyHaltOwnedResult, LoadedCrazyTheoremSequence,
};
use crate::geometry_native_jump_rotate_crazy_halt_sequence as theorem_sequence;

/// Short cache-local alias for the exact admitted theorem sequence.
pub type CrazyTheoremSequence =
    theorem_sequence::ExecutionGeometryNativeJumpRotateCrazyHaltSequence;

type TheoremCacheFailure<MemoryError> =
    GeometryNativeJumpRotateCrazyHaltCacheAcquireFailure<MemoryError>;

/// Whether one lease acquisition loaded or reused resident mappings.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum GeometryNativeJumpRotateCrazyHaltCacheDisposition {
    /// The exact sequence already owned resident mappings.
    Hit,
    /// This acquisition loaded and published an empty resident slot.
    Inserted,
    /// An unleased different resident was released and replaced.
    Replaced,
}

/// Failure while acquiring one exact resident v5 theorem path.
#[derive(Debug, Eq, PartialEq)]
pub enum GeometryNativeJumpRotateCrazyHaltCacheAcquireFailure<MemoryError> {
    /// A different exact sequence already owns the single resident slot.
    IdentityOccupied,
    /// Live leases prevent replacing a different resident identity.
    Leased {
        /// External resident owners that must be dropped first.
        leases: usize,
    },
    /// Loading the requested exact theorem path failed.
    Load(
        Box<ExecutionGeometryNativeJumpRotateCrazyHaltLoadFailure<MemoryError>>,
    ),
    /// Releasing the previous unleased resident failed during replacement.
    Release(
        Box<
            ExecutionGeometryNativeJumpRotateCrazyHaltReleaseFailure<
                MemoryError,
            >,
        >,
    ),
}

/// Lease plus whether its mappings were inserted or reused.
#[derive(Debug)]
pub struct GeometryNativeJumpRotateCrazyHaltCacheAcquisition {
    disposition: GeometryNativeJumpRotateCrazyHaltCacheDisposition,
    lease: GeometryNativeJumpRotateCrazyHaltLease,
}

/// Explicit result of attempting to release the single resident theorem path.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum GeometryNativeJumpRotateCrazyHaltCacheRelease {
    /// External leases still retain the resident theorem path.
    Leased {
        /// Number of external lease owners blocking release.
        leases: usize,
    },
    /// No resident theorem path exists.
    Missing,
    /// The unleased resident theorem path released all seven mappings.
    Released,
}

/// One immutable external owner of the resident v5 theorem path.
#[derive(Clone, Debug)]
pub struct GeometryNativeJumpRotateCrazyHaltLease {
    resident: Arc<LoadedCrazyTheoremSequence>,
}

/// Single exact resident slot for cloneable v5 theorem leases.
#[derive(Debug, Default)]
pub struct GeometryNativeJumpRotateCrazyHaltLeaseCache {
    resident: Option<Arc<LoadedCrazyTheoremSequence>>,
}

/// Result of acquiring one exact resident theorem path lease.
pub type GeometryNativeJumpRotateCrazyHaltCacheAcquireResult<MemoryError> =
    Result<
        GeometryNativeJumpRotateCrazyHaltCacheAcquisition,
        Box<GeometryNativeJumpRotateCrazyHaltCacheAcquireFailure<MemoryError>>,
    >;

/// Result of releasing the resident theorem path after all leases are gone.
pub type GeometryNativeJumpRotateCrazyHaltCacheReleaseResult<MemoryError> =
    Result<
        GeometryNativeJumpRotateCrazyHaltCacheRelease,
        Box<
            ExecutionGeometryNativeJumpRotateCrazyHaltReleaseFailure<
                MemoryError,
            >,
        >,
    >;

impl<MemoryError: Display> Display
    for GeometryNativeJumpRotateCrazyHaltCacheAcquireFailure<MemoryError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        match self {
            Self::IdentityOccupied => {
                f.write_str("different v5 theorem identity already resident")
            },
            Self::Leased { leases } => {
                write!(f, "v5 theorem replacement blocked by {leases} lease(s)")
            },
            Self::Load(error) => {
                write!(f, "v5 theorem resident load failed: {error}")
            },
            Self::Release(error) => {
                write!(f, "v5 theorem replacement release failed: {error}")
            },
        }
    }
}

impl GeometryNativeJumpRotateCrazyHaltCacheAcquisition {
    /// Returns whether this acquisition loaded or reused mappings.
    #[must_use]
    pub const fn disposition(
        &self,
    ) -> GeometryNativeJumpRotateCrazyHaltCacheDisposition {
        self.disposition
    }

    /// Consumes the acquisition and returns its immutable external lease.
    #[must_use]
    pub fn into_lease(self) -> GeometryNativeJumpRotateCrazyHaltLease {
        self.lease
    }

    /// Returns the immutable lease retained by this acquisition.
    #[must_use]
    pub const fn lease(&self) -> &GeometryNativeJumpRotateCrazyHaltLease {
        &self.lease
    }
}

impl GeometryNativeJumpRotateCrazyHaltLease {
    /// Executes through the resident exact theorem path without mapping
    /// operations.
    ///
    /// # Errors
    ///
    /// Returns indexed v5 runner/completion failure from the resident theorem
    /// path.
    pub fn execute<Runner>(
        &self,
        runner: &mut Runner,
        buffers: NativeRegionBuffers<'_>,
    ) -> GeometryNativeJumpRotateCrazyHaltOwnedResult<Runner::Error>
    where
        Runner: ExecutionGeometryNativeRunner,
    {
        self.resident.execute(runner, buffers)
    }

    /// Returns exact resident weight from the synchronized mapping reports.
    ///
    /// # Errors
    ///
    /// Returns overflow when mapped byte capacities cannot be summed.
    pub fn resident_weight(
        &self,
    ) -> Result<
        ExecutionGeometryNativeJumpRotateCrazyHaltResidentWeight,
        ExecutionGeometryNativeJumpRotateCrazyHaltResidentWeightError,
    > {
        self.resident.resident_weight()
    }

    /// Returns the exact admitted v5 sequence retained beside the mappings.
    #[must_use]
    pub fn sequence(&self) -> &CrazyTheoremSequence {
        self.resident.sequence()
    }

    /// Reports whether two leases share the same resident theorem path
    /// allocation.
    #[must_use]
    pub fn shares_resident_with(&self, other: &Self) -> bool {
        Arc::ptr_eq(&self.resident, &other.resident)
    }

    /// Returns all strong owners, including the cache resident owner.
    #[must_use]
    pub fn strong_owner_count(&self) -> usize {
        Arc::strong_count(&self.resident)
    }
}

impl GeometryNativeJumpRotateCrazyHaltLeaseCache {
    /// Loads or reuses the exact requested sequence as one immutable lease.
    ///
    /// A different sequence cannot evict or replace the current resident. This
    /// deliberately keeps this v5 cache boundary smaller than the legacy
    /// multi-entry cache and makes exact geometry identity visible.
    ///
    /// # Errors
    ///
    /// Returns exact occupied-identity or theorem-loading failure.
    pub fn ensure<Adapter>(
        &mut self,
        adapter: &mut Adapter,
        sequence: &CrazyTheoremSequence,
    ) -> GeometryNativeJumpRotateCrazyHaltCacheAcquireResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        if let Some(resident) = &self.resident {
            if resident.sequence() != sequence {
                return Err(Box::new(TheoremCacheFailure::IdentityOccupied));
            }
            return Ok(GeometryNativeJumpRotateCrazyHaltCacheAcquisition {
                disposition:
                    GeometryNativeJumpRotateCrazyHaltCacheDisposition::Hit,
                lease: GeometryNativeJumpRotateCrazyHaltLease {
                    resident: Arc::clone(resident),
                },
            });
        }
        let loaded = LoadedCrazyTheoremSequence::load(sequence, adapter)
            .map_err(|error| Box::new(TheoremCacheFailure::Load(error)))?;
        let replacement_resident = Arc::new(loaded);
        let lease = GeometryNativeJumpRotateCrazyHaltLease {
            resident: Arc::clone(&replacement_resident),
        };
        self.resident = Some(replacement_resident);
        Ok(GeometryNativeJumpRotateCrazyHaltCacheAcquisition {
            disposition:
                GeometryNativeJumpRotateCrazyHaltCacheDisposition::Inserted,
            lease,
        })
    }

    /// Reports whether any exact theorem path currently owns the resident slot.
    #[must_use]
    pub const fn has_resident(&self) -> bool {
        self.resident.is_some()
    }

    /// Constructs one empty single-resident v5 theorem path lease cache.
    #[must_use]
    pub const fn new() -> Self {
        Self { resident: None }
    }

    /// Releases resident mappings only when no external leases remain.
    ///
    /// Live leases return
    /// [`GeometryNativeJumpRotateCrazyHaltCacheRelease::Leased`]
    /// without calling the adapter. A cleanup failure empties the cache because
    /// retry ownership moves into the returned theorem release failure.
    ///
    /// # Errors
    ///
    /// Returns exact theorem cleanup retry ownership when any mapping fails to
    /// release.
    pub fn release_if_unleased<Adapter>(
        &mut self,
        adapter: &mut Adapter,
    ) -> GeometryNativeJumpRotateCrazyHaltCacheReleaseResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        let Some(resident) = self.resident.take() else {
            return Ok(GeometryNativeJumpRotateCrazyHaltCacheRelease::Missing);
        };
        let leases = Arc::strong_count(&resident).saturating_sub(1);
        if leases > 0 {
            self.resident = Some(resident);
            return Ok(GeometryNativeJumpRotateCrazyHaltCacheRelease::Leased {
                leases,
            });
        }
        match Arc::try_unwrap(resident) {
            Ok(loaded) => loaded.release(adapter).map(|()| {
                GeometryNativeJumpRotateCrazyHaltCacheRelease::Released
            }),
            Err(retained) => {
                let remaining_leases =
                    Arc::strong_count(&retained).saturating_sub(1);
                self.resident = Some(retained);
                Ok(GeometryNativeJumpRotateCrazyHaltCacheRelease::Leased {
                    leases: remaining_leases,
                })
            },
        }
    }

    /// Replaces a different resident only after every external lease is gone.
    ///
    /// Equal identity delegates to ordinary hit acquisition. Different identity
    /// with live leases rejects before adapter work. For an unleased different
    /// resident, old mappings release completely before the new theorem owner
    /// is loaded; any release or load failure leaves the cache empty and
    /// transfers exact cleanup ownership through the returned failure.
    ///
    /// # Errors
    ///
    /// Returns lease blockage, previous-resident cleanup ownership, or new
    /// theorem load failure without publishing partial replacement state.
    pub fn replace_if_unleased<Adapter>(
        &mut self,
        adapter: &mut Adapter,
        sequence: &CrazyTheoremSequence,
    ) -> GeometryNativeJumpRotateCrazyHaltCacheAcquireResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        let Some(current_resident) = self.resident.as_ref() else {
            return self.ensure(adapter, sequence);
        };
        if current_resident.sequence() == sequence {
            return self.ensure(adapter, sequence);
        }
        let current_leases =
            Arc::strong_count(current_resident).saturating_sub(1);
        if current_leases > 0 {
            return Err(Box::new(TheoremCacheFailure::Leased {
                leases: current_leases,
            }));
        }
        match self.release_if_unleased(adapter) {
            Ok(
                GeometryNativeJumpRotateCrazyHaltCacheRelease::Released
                | GeometryNativeJumpRotateCrazyHaltCacheRelease::Missing,
            ) => {},
            Ok(GeometryNativeJumpRotateCrazyHaltCacheRelease::Leased {
                leases: blocked_leases,
            }) => {
                return Err(Box::new(TheoremCacheFailure::Leased {
                    leases: blocked_leases,
                }));
            },
            Err(error) => {
                return Err(Box::new(TheoremCacheFailure::Release(error)));
            },
        }
        let loaded = LoadedCrazyTheoremSequence::load(sequence, adapter)
            .map_err(|error| Box::new(TheoremCacheFailure::Load(error)))?;
        let replacement_resident = Arc::new(loaded);
        let lease = GeometryNativeJumpRotateCrazyHaltLease {
            resident: Arc::clone(&replacement_resident),
        };
        self.resident = Some(replacement_resident);
        Ok(GeometryNativeJumpRotateCrazyHaltCacheAcquisition {
            disposition:
                GeometryNativeJumpRotateCrazyHaltCacheDisposition::Replaced,
            lease,
        })
    }

    /// Returns the number of external resident leases.
    #[must_use]
    pub fn resident_lease_count(&self) -> usize {
        self.resident
            .as_ref()
            .map_or(0, |resident| Arc::strong_count(resident).saturating_sub(1))
    }
}
