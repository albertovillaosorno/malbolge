...
// Boundary-Contract:
// - Owns:
//   - Single-resident lease reuse for one exact v5 rotate/halt pair.
// - Must-Not:
...
//! Single-resident lease cache for the explicit-geometry rotate/halt pair.
...
use crate::geometry_native_rotate_sequence::{
    ExecutionGeometryNativeRotateHaltLoadedResult,
...
pub enum GeometryNativeRotateHaltPairCacheDisposition {
...
pub enum GeometryNativeRotateHaltPairCacheAcquireFailure<MemoryError> {
    ...
    Load(Box<ExecutionGeometryNativeRotateHaltPairLoadFailure<MemoryError>>),
...
#[derive(Debug)]
pub struct GeometryNativeRotateHaltPairCacheAcquisition {
    disposition: GeometryNativeRotateHaltPairCacheDisposition,
    lease: GeometryNativeRotateHaltPairLease,
}

/// Explicit result of attempting to release the single resident pair.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum GeometryNativeRotateHaltPairCacheRelease {
...
#[derive(Clone, Debug)]
pub struct GeometryNativeRotateHaltPairLease {
    resident: Arc<LoadedExecutionGeometryNativeRotateHaltSequence>,
}

/// Single exact resident slot for cloneable v5 pair leases.
#[derive(Debug, Default)]
pub struct GeometryNativeRotateHaltPairLeaseCache {
    resident: Option<Arc<LoadedExecutionGeometryNativeRotateHaltSequence>>,
}

/// Result of acquiring one exact resident pair lease.
pub type GeometryNativeRotateHaltPairCacheAcquireResult<MemoryError> = Result<
    GeometryNativeRotateHaltPairCacheAcquisition,
    Box<GeometryNativeRotateHaltPairCacheAcquireFailure<MemoryError>>,
>;

/// Result of releasing the resident pair after all leases are gone.
pub type GeometryNativeRotateHaltPairCacheReleaseResult<MemoryError> = Result<
    GeometryNativeRotateHaltPairCacheRelease,
    Box<ExecutionGeometryNativeRotateHaltPairReleaseFailure<MemoryError>>,
>;

impl<MemoryError: Display> Display
    for GeometryNativeRotateHaltPairCacheAcquireFailure<MemoryError>
{
...
impl GeometryNativeRotateHaltPairCacheAcquisition {
    ...
    #[must_use]
    pub fn into_lease(self) -> GeometryNativeRotateHaltPairLease {
        self.lease
    }

    /// Returns the immutable lease retained by this acquisition.
    #[must_use]
    pub const fn lease(&self) -> &GeometryNativeRotateHaltPairLease {
        &self.lease
    }
}

impl GeometryNativeRotateHaltPairLease {
    ...
    #[must_use]
    pub fn sequence(&self) -> &ExecutionGeometryNativeRotateHaltSequence {
...
impl GeometryNativeRotateHaltPairLeaseCache {
    ...
    /// # Errors
    ///
    /// Returns [`GeometryNativeRotateHaltPairCacheAcquireFailure`] for occupied
    /// different identity or pair loading failure.
    pub fn ensure<Adapter>(
        &mut self,
        adapter: &mut Adapter,
        sequence: &ExecutionGeometryNativeRotateHaltSequence,
    ) -> GeometryNativeRotateHaltPairCacheAcquireResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        if let Some(resident) = &self.resident {
            if resident.sequence() != sequence {
                use GeometryNativeRotateHaltPairCacheAcquireFailure as Failure;
                return Err(Box::new(Failure::IdentityOccupied));
            }
            return Ok(GeometryNativeRotateHaltPairCacheAcquisition {
                disposition: GeometryNativeRotateHaltPairCacheDisposition::Hit,
                lease: GeometryNativeRotateHaltPairLease {
        ...
        let loaded = sequence.load_pair(adapter).map_err(|error| {
            Box::new(GeometryNativeRotateHaltPairCacheAcquireFailure::Load(
                error,
            ))
        })?;
        let replacement_resident = Arc::new(loaded);
        let lease = GeometryNativeRotateHaltPairLease {
            resident: Arc::clone(&replacement_resident),
        };
        self.resident = Some(replacement_resident);
        Ok(GeometryNativeRotateHaltPairCacheAcquisition {
            disposition: GeometryNativeRotateHaltPairCacheDisposition::Inserted,
            lease,
        })
    }
    ...
    ///
    /// Live leases return [`GeometryNativeRotateHaltPairCacheRelease::Leased`]
    ...
    pub fn release_if_unleased<Adapter>(
        &mut self,
        adapter: &mut Adapter,
    ) -> GeometryNativeRotateHaltPairCacheReleaseResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        let Some(resident) = self.resident.take() else {
            return Ok(GeometryNativeRotateHaltPairCacheRelease::Missing);
        };
        let leases = Arc::strong_count(&resident).saturating_sub(1);
        if leases > 0 {
            self.resident = Some(resident);
            return Ok(GeometryNativeRotateHaltPairCacheRelease::Leased {
                leases,
            });
        }
        match Arc::try_unwrap(resident) {
            Ok(loaded) => loaded
                .release(adapter)
                .map(|()| GeometryNativeRotateHaltPairCacheRelease::Released),
            Err(retained) => {
                let remaining_leases =
                    Arc::strong_count(&retained).saturating_sub(1);
                self.resident = Some(retained);
                Ok(GeometryNativeRotateHaltPairCacheRelease::Leased {
        ...
        Adapter: NativeExecutableMemoryAdapter,
    {
        ...
        if current_leases > 0 {
            return Err(Box::new(
                GeometryNativeRotateHaltPairCacheAcquireFailure::Leased {
        ...
        match self.release_if_unleased(adapter) {
            Ok(
                GeometryNativeRotateHaltPairCacheRelease::Released
                | GeometryNativeRotateHaltPairCacheRelease::Missing,
            ) => {},
            Ok(GeometryNativeRotateHaltPairCacheRelease::Leased {
            ...
            Err(error) => {
                return Err(Box::new(
                    GeometryNativeRotateHaltPairCacheAcquireFailure::Release(
                        error,
                    ),
                ));
            },
        ...
        self.resident = Some(replacement_resident);
        Ok(GeometryNativeRotateHaltPairCacheAcquisition {
            disposition: GeometryNativeRotateHaltPairCacheDisposition::Replaced,
            lease,
        })
    }
