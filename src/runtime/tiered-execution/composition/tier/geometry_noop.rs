...
// Boundary-Contract:
// - Owns:
//   - Checkpoint-bound admission and execution of verified v5 no-operation.
// - Must-Not:
...
// - Summary:
//   - Executes only normatively replayed explicit-geometry no-operation.
// - Description:
//   - Binds v5 identity to opaque state and exact self-encryption transition.
// - Usage:
...
//! Checkpoint-bound native execution for explicit-geometry no-operation.
...
pub enum ExecutionGeometryNativeNoOperationAdmissionError {
...
/// Failure while binding one prepared v5 no-operation to synchronized code.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ExecutionGeometryNativeNoOperationBindingError {
    /// Synchronized executable image differs from checkpoint-bound admission.
    ExecutableIdentity,
}

/// Failure while admitting one completed v5 no-operation ABI transition.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ExecutionGeometryNativeNoOperationCompletionError {
    /// Native result disagreed with the exact prepared transition.
    Invocation(NativeRegionInvocationError),
}

/// Failure after a checkpoint-bound v5 no-operation enters the runner.
#[derive(Debug, Eq, PartialEq)]
pub enum ExecutionGeometryNativeNoOperationExecutionError<RunnerError> {
    /// Returned status or caller-visible state failed exact completion.
    Completion(ExecutionGeometryNativeNoOperationCompletionError),
    /// External runner failed before returning a raw ABI status.
    Runner(Box<RunnerError>),
}

/// Failure while executing one reusable owned no-operation mapping.
#[derive(Debug, Eq, PartialEq)]
pub enum ExecutionGeometryNativeNoOperationOwnedFailure<RunnerError> {
    /// Prepared caller buffers could not bind to the retained ready mapping.
    Binding(ExecutionGeometryNativeNoOperationBindingError),
    ...
    Preparation(ExecutionGeometryNativeNoOperationPreparationError),
}

/// Result of one dedicated checkpoint-bound v5 no-operation runner call.
pub type ExecutionGeometryNativeNoOperationExecutionResult<RunnerError> =
    Result<
        ExecutionGeometryNativeNoOperationCompletion,
        Box<ExecutionGeometryNativeNoOperationExecutionError<RunnerError>>,
    >;

/// Result of executing one reusable owned no-operation mapping.
pub type ExecutionGeometryNativeNoOperationOwnedResult<RunnerError> = Result<
    ExecutionGeometryNativeNoOperationCompletion,
    Box<ExecutionGeometryNativeNoOperationOwnedFailure<RunnerError>>,
>;

/// Failure while preparing checkpoint-exact buffers for v5 no-operation.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ExecutionGeometryNativeNoOperationPreparationError {
    /// Borrowed input bytes differ from the admitted checkpoint.
    Input,
    /// Native ABI preparation rejected the exact v5 no-operation contract.
    Invocation(NativeRegionInvocationError),
...
/// Failure from complete v5 no-operation load/call/admit/release composition.
#[derive(Debug, Eq, PartialEq)]
pub enum ExecutionGeometryNativeNoOperationTransactionFailure<
    MemoryError,
    RunnerError,
> {
    ...
    Binding {
        ...
        error: ExecutionGeometryNativeNoOperationBindingError,
    ...
    Release {
        ...
        /// failure.
        completion: Box<ExecutionGeometryNativeNoOperationCompletion>,
...
/// Result of one complete guarded no-operation transaction for adapter ports.
pub type ExecutionGeometryNativeNoOperationAdapterTransactionResult<
    MemoryAdapter,
    Runner,
> = ExecutionGeometryNativeNoOperationTransactionResult<
    <MemoryAdapter as NativeExecutableMemoryAdapter>::Error,
    <Runner as ExecutionGeometryNativeRunner>::Error,
>;

/// Result of one complete guarded v5 no-operation transaction.
pub type ExecutionGeometryNativeNoOperationTransactionResult<
    MemoryError,
    RunnerError,
> = Result<
    ExecutionGeometryNativeNoOperationCompletion,
...
/// Result of loading one reusable exact no-operation mapping.
pub type GeometryNativeNoOperationOwnedLoadResult<MemoryError> = Result<
    LoadedExecutionGeometryNativeNoOperation,
    Box<NativeExecutableLoadFailure<MemoryError>>,
>;

/// Result of releasing one reusable exact no-operation mapping.
pub type GeometryNativeNoOperationOwnedReleaseResult<MemoryError> = Result<
    (),
    Box<ExecutionGeometryNativeExecutableReleaseFailure<MemoryError>>,
>;

/// Verified v5 no-operation bound to one opaque checkpoint and normative exit.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ExecutionGeometryNativeNoOperationAdmission {
...
/// Exact synchronized mapping weight retained by one owned no-operation.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ExecutionGeometryNativeNoOperationResidentWeight {
    mapped_bytes: usize,
    mappings: usize,
}

/// One reusable ready no-operation mapping beside exact admission.
#[derive(Debug)]
pub struct LoadedExecutionGeometryNativeNoOperation {
    admission: Box<ExecutionGeometryNativeNoOperationAdmission>,
    executable: ReadyExecutionGeometryNativeExecutable,
}

/// Prepared checkpoint-owned no-operation bound to exact synchronized v5 code.
#[derive(Debug)]
pub struct ExecutionGeometryNativeNoOperationBoundCall<
    'admission,
    'buffers,
    'executable,
> {
    executable: &'executable ReadyExecutionGeometryNativeExecutable,
    prepared: PreparedExecutionGeometryNativeNoOperation<'admission, 'buffers>,
}

/// One admitted no-operation result retaining opaque geometry authority.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ExecutionGeometryNativeNoOperationCompletion {
    outcome: NativeRegionInvocationOutcome,
    state: ProfileMachineState,
}

/// Borrow-scoped no-operation ABI contract retaining checkpoint admission.
#[derive(Debug)]
pub struct PreparedExecutionGeometryNativeNoOperation<'admission, 'buffers> {
    admission: &'admission ExecutionGeometryNativeNoOperationAdmission,
    invocation: PreparedNativeRegionInvocation<'buffers>,
}

impl<RunnerError: Display> Display
    for ExecutionGeometryNativeNoOperationOwnedFailure<RunnerError>
{
...
impl Display for ExecutionGeometryNativeNoOperationAdmissionError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        match self {
            Self::ArtifactIdentity => f.write_str(
                "v5 no-operation artifact identity differs from program",
            ),
            Self::Checkpoint(error) => Display::fmt(error, f),
            Self::Identity(_error) => {
                f.write_str("v5 no-operation identity reconstruction failed")
            },
...
impl Display for ExecutionGeometryNativeNoOperationBindingError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str("v5 no-operation executable identity differs from call")
    }
}

impl Display for ExecutionGeometryNativeNoOperationCompletionError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        match self {
...
impl<RunnerError: Display> Display
    for ExecutionGeometryNativeNoOperationExecutionError<RunnerError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        match self {
            Self::Completion(error) => Display::fmt(error, f),
            Self::Runner(error) => {
                write!(f, "v5 no-operation runner failed: {error}")
            },
        }
    }
}

impl Display for ExecutionGeometryNativeNoOperationPreparationError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        match self {
            Self::Input => {
                f.write_str("v5 no-operation input differs from checkpoint")
            },
            Self::Invocation(error) => Display::fmt(error, f),
            Self::Memory => {
                f.write_str("v5 no-operation memory differs from checkpoint")
            },
            Self::Output => {
                f.write_str("v5 no-operation output differs from checkpoint")
            },
        }
    }
}
...
impl<MemoryError: Display, RunnerError: Display> Display
    for ExecutionGeometryNativeNoOperationTransactionFailure<
        MemoryError,
        RunnerError,
    >
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        match self {
            Self::Binding { error, .. } => {
                write!(f, "v5 no-operation transaction binding failed: {error}")
            },
            Self::Execution { error, .. } => {
                write!(
                    f,
                    "v5 no-operation transaction execution failed: {error}"
                )
            },
            Self::Load(error) => {
                write!(f, "v5 no-operation transaction load failed: {error}")
            },
            Self::Preparation(error) => {
                write!(
                    f,
                    "v5 no-operation transaction preparation failed: {error}"
                )
            },
            Self::Release { release_failure, .. } => {
                write!(f, "v5 no-operation transaction {release_failure}")
            },
        }
    }
}

impl ExecutionGeometryNativeNoOperationAdmission {
    /// Returns the exact verified v5 no-operation artifact retained by
    /// admission.
    #[must_use]
    ...
    /// Loads, binds, runs, admits, and releases one guarded v5 no-operation.
    ///
    ...
    /// # Errors
    ///
    /// Returns [`ExecutionGeometryNativeNoOperationTransactionFailure`] with
    ...
    ) -> ExecutionGeometryNativeNoOperationAdapterTransactionResult<
        MemoryAdapter,
        Runner,
    >
    where
    ...
    {
        use ExecutionGeometryNativeNoOperationTransactionFailure as Failure;
    ...
    /// Loads and retains one reusable synchronized no-operation mapping.
    ///
    /// # Errors
    ///
    ...
    pub fn load_owned<Adapter>(
        &self,
        adapter: &mut Adapter,
    ) -> GeometryNativeNoOperationOwnedLoadResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        ...
        .map_err(Box::new)?;
        Ok(LoadedExecutionGeometryNativeNoOperation {
            admission: Box::new(self.clone()),
            executable,
        })
    }

    /// Binds verified no-operation evidence to a normatively replayed
    /// checkpoint.
    ///
    ...
    /// # Errors
    ///
    /// Returns [`ExecutionGeometryNativeNoOperationAdmissionError`] for
    /// checkpoint, replay, identity, or load-image disagreement.
    pub fn new(
        program: ExecutionGeometryRegionEffectProgram,
        checkpoint: ProfileMachineState,
        artifact: VerifiedExecutionGeometryNoOperationNativeObjectArtifact,
    ) -> Result<Self, ExecutionGeometryNativeNoOperationAdmissionError> {
        let replay = ExecutionGeometryInterpreterHandoff::new(
            program.clone(),
            checkpoint.clone(),
        )
        .map_err(
            ExecutionGeometryNativeNoOperationAdmissionError::Checkpoint,
        )?;
        let completion = replay.execute().map_err(|failure| {
            ExecutionGeometryNativeNoOperationAdmissionError::NormativeReplay(
                failure.cause(),
            )
        })?;
        ...
        .map_err(ExecutionGeometryNativeNoOperationAdmissionError::Identity)?;
        if artifact.key() != &expected_key {
            use ExecutionGeometryNativeNoOperationAdmissionError as Error;
            return Err(Error::ArtifactIdentity);
        }
        let load_image =
            VerifiedExecutionGeometryLoadImage::from_no_operation(&artifact)
                .map_err(
                    ExecutionGeometryNativeNoOperationAdmissionError::Load,
                )?;
        Ok(Self {
            artifact,
            checkpoint,
    ...
    /// Prepares exact checkpoint-owned buffers for guarded v5 no-operation.
    ///
    /// # Errors
    ///
    /// Returns [`ExecutionGeometryNativeNoOperationPreparationError`] when any
    ...
    pub fn prepare<'admission, 'buffers>(
        ...
        PreparedExecutionGeometryNativeNoOperation<'admission, 'buffers>,
    ...
    > {
        ...
        if input != self.checkpoint.io().input() {
            return Err(
                ExecutionGeometryNativeNoOperationPreparationError::Input,
            );
        }
        if memory != self.checkpoint.memory() {
            return Err(
                ExecutionGeometryNativeNoOperationPreparationError::Memory,
            );
        }
        if output != self.checkpoint.io().output() {
            return Err(
                ExecutionGeometryNativeNoOperationPreparationError::Output,
            );
        }
        let invocation =
    ...
    /// Returns the exact v5 program whose no-operation was normatively
    /// replayed.
    #[must_use]
...
impl ExecutionGeometryNativeNoOperationResidentWeight {
...
impl LoadedExecutionGeometryNativeNoOperation {
    ...
    pub fn execute<Runner>(
        ...
        runner: &mut Runner,
        buffers: NativeRegionBuffers<'_>,
    ) -> ExecutionGeometryNativeNoOperationOwnedResult<Runner::Error>
    where
        Runner: ExecutionGeometryNativeRunner,
    {
        let prepared = self.admission.prepare(buffers).map_err(|error| {
            Box::new(
                ExecutionGeometryNativeNoOperationOwnedFailure::Preparation(
                    error,
                ),
            )
        })?;
        let bound =
            prepared
                ...
                .map_err(|error| {
                    Box::new(
                        ExecutionGeometryNativeNoOperationOwnedFailure::Binding(
                            error,
                        ),
                    )
                })?;
        bound.execute(runner).map_err(|error| {
            Box::new(ExecutionGeometryNativeNoOperationOwnedFailure::Execution(
                error,
            ))
        })
    }
    ...
    pub fn release<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> GeometryNativeNoOperationOwnedReleaseResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
    ...
    pub const fn resident_weight(
        &self,
    ) -> ExecutionGeometryNativeNoOperationResidentWeight {
        ExecutionGeometryNativeNoOperationResidentWeight {
...
impl ExecutionGeometryNativeNoOperationBoundCall<'_, '_, '_> {
    ...
    /// # Errors
    ///
    /// Returns [`ExecutionGeometryNativeNoOperationCompletionError`] when the
    ...
    /// # Errors
    ///
    /// Returns [`ExecutionGeometryNativeNoOperationExecutionError`] for runner
    /// or completion failure.
    pub fn execute<Runner>(
        self,
        runner: &mut Runner,
    ) -> ExecutionGeometryNativeNoOperationExecutionResult<Runner::Error>
    where
        Runner: ExecutionGeometryNativeRunner,
    {
        let Self { executable, prepared } = self;
        let PreparedExecutionGeometryNativeNoOperation {
            admission,
            invocation,
        } = prepared;
        ...
        let raw_status = match runner.run(&mut runner_invocation) {
            ...
            Err(error) => {
                ...
                return Err(Box::new(
                    ExecutionGeometryNativeNoOperationExecutionError::Runner(
                        Box::new(error),
                    ),
                ));
            },
        ...
        let outcome = runner_invocation.complete(raw_status).map_err(|error| {
            use ExecutionGeometryNativeNoOperationCompletionError as Completion;
            use ExecutionGeometryNativeNoOperationExecutionError as Execution;
            Box::new(Execution::Completion(Completion::Invocation(error)))
        })?;
        Ok(ExecutionGeometryNativeNoOperationCompletion {
            outcome,
...
impl ExecutionGeometryNativeNoOperationCompletion {
    ...
    PreparedExecutionGeometryNativeNoOperation<'admission, 'buffers>
{
    ...
    /// # Errors
    ///
    /// Returns [`ExecutionGeometryNativeNoOperationBindingError`] when the
    /// executable carries any different verified load image.
    pub fn bind_executable<'executable>(
        self,
        executable: &'executable ReadyExecutionGeometryNativeExecutable,
    ) -> Result<
        ExecutionGeometryNativeNoOperationBoundCall<
            'admission,
            'buffers,
            'executable,
        >,
    ...
    > {
        ...
        if admission.load_image() != executable.image() {
            let error = NoOperationBindingError::ExecutableIdentity;
            invocation.abort();
            return Err(error);
        }
        Ok(ExecutionGeometryNativeNoOperationBoundCall {
            executable,
    ...
    /// # Errors
    ///
    /// Returns [`ExecutionGeometryNativeNoOperationCompletionError`] when exact
