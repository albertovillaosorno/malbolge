...
// Boundary-Contract:
// - Owns:
//   - Indexed runner execution for already-loaded Output v6 sequences.
// - Must-Not:
//   - Allocate, release, cache, or alter admitted sequence topology.
// - Allows:
//   - Inputs: loaded Output owner, runner, rebased entry, caller buffers.
...
// - Merge-When:
//   - One reviewed Output coordinator safely owns load and execution.
// - Summary:
//   - Executes retained Output mappings without adapter work.
// - Description:
...
//! Indexed execution for loaded Output register-masked v6 sequences.
...
use super::{
    register_masked_output_loaded_sequence as loaded_sequence,
    register_masked_resident as resident,
};

/// Admitted result of executing one loaded Output v6 sequence.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedOutputNativeSequenceOutcome {
...
pub struct RegisterMaskedOutputNativeSequenceExecutionFailure<RunnerError> {
    cause: Box<
        resident::RegisterMaskedOutputNativeOwnerExecutionFailure<RunnerError>,
    >,
...
/// Result of executing one already-loaded Output v6 sequence.
pub type RegisterMaskedOutputNativeSequenceExecutionResult<RunnerError> =
    Result<
        RegisterMaskedOutputNativeSequenceOutcome,
        Box<RegisterMaskedOutputNativeSequenceExecutionFailure<RunnerError>>,
    >;

impl<RunnerError>
    RegisterMaskedOutputNativeSequenceExecutionFailure<RunnerError>
{
...
impl<RunnerError: Display> Display
    for RegisterMaskedOutputNativeSequenceExecutionFailure<RunnerError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "Output v6 sequence step {} failed after {} steps: {}",
            self.step_index, self.completed_steps, self.cause,
        )
    }
}

impl RegisterMaskedOutputNativeSequenceOutcome {
...
pub fn execute_loaded_register_masked_output_native_sequence<Runner>(
    loaded: &loaded_sequence::LoadedRegisterMaskedOutputNativeSequence,
    runner: &mut Runner,
    entry: ProfileMachineObservation,
    buffers: NativeRegionBuffers<'_>,
) -> RegisterMaskedOutputNativeSequenceExecutionResult<Runner::Error>
where
    Runner: RegisterMaskedOutputNativeRunner,
{
    use RegisterMaskedOutputNativeSequenceOutcome as Outcome;
    ...
    for (index, owner) in loaded.owners().iter().enumerate() {
        let outcome = owner
            ...
            .map_err(|cause| {
                Box::new(RegisterMaskedOutputNativeSequenceExecutionFailure {
                    cause,
