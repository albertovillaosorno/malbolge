...
/// Relocation-free image for one verified register-masked v6 halt artifact.
///
/// This type deliberately has no executable lifecycle or runner integration.
/// It proves only that exact v6 COFF can be copied under the strict W^X policy.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct VerifiedRegisterMaskedLoadImage {
    code: Box<[u8]>,
    entry_offset: usize,
    key: NativeArtifactKey,
    policy: NativeExecutableLoadPolicy,
...
/// Relocation-free image for one verified register-masked v6 no-operation.
///
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct VerifiedRegisterMaskedNoOperationLoadImage {
    code: Box<[u8]>,
    entry_offset: usize,
    key: NativeArtifactKey,
    policy: NativeExecutableLoadPolicy,
...
/// Relocation-free image for the collapsed v6 no-operation/halt object.
///
/// This value proves exact COFF closure, ISA alignment, and strict W^X policy.
/// It grants no executable mapping, binding, runner, or invocation authority.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct VerifiedRegisterMaskedNoOperationHaltLoadImage {
    code: Box<[u8]>,
    entry_offset: usize,
    key: NativeArtifactKey,
    policy: NativeExecutableLoadPolicy,
...
/// Relocation-free image for the collapsed v6 no-operation-pair object.
///
/// This value proves exact COFF closure, ISA alignment, and strict W^X policy.
/// It grants no executable mapping, binding, runner, or invocation authority.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct VerifiedRegisterMaskedNoOperationPairLoadImage {
    code: Box<[u8]>,
    entry_offset: usize,
    key: NativeArtifactKey,
    policy: NativeExecutableLoadPolicy,
...
/// Relocation-free image for collapsed v6 no-operation then rotate.
///
/// This value proves exact COFF closure, ISA alignment, and strict W^X policy.
...
/// Relocation-free image for one verified register-masked v6 Crazy object.
///
/// This type grants no lifecycle, platform, binding, or runner authority. It
/// proves only relocation closure, ISA alignment, and strict W^X policy.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Relocation-free image for one verified register-masked v6 rotate.
///
/// This type grants no lifecycle, platform, binding, or runner authority. It
/// proves only relocation closure, ISA alignment, and strict W^X policy.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct VerifiedRegisterMaskedRotateLoadImage {
    code: Box<[u8]>,
    entry_offset: usize,
    key: NativeArtifactKey,
    policy: NativeExecutableLoadPolicy,
...
/// Relocation-free image for one verified register-masked v6 non-graphical
/// artifact.
///
/// This value is intentionally distinct from
/// [`VerifiedRegisterMaskedLoadImage`]. No executable lifecycle or platform API
...
impl VerifiedExecutionGeometryLoadImage {
    ...
    /// Derives a relocation-free image from verified v5 jump-code bytes.
    ///
    /// # Errors
    ///
    /// Returns [`VerifiedDirectLoadError`] when COFF extraction, relocation, or
    /// target instruction alignment is invalid.
    pub fn from_jump_code(
...
impl VerifiedRegisterMaskedLoadImage {
    /// Returns the exact number of admitted v6 code bytes.
    #[must_use]
    pub const fn allocation_len(&self) -> usize {
        self.code.len()
    }
    ...
    /// Extracts one immutable relocation-free image from verified v6 bytes.
    ///
    /// # Errors
    ///
    /// Returns [`VerifiedDirectLoadError`] when COFF extraction, relocation, or
...
impl VerifiedRegisterMaskedNoOperationHaltLoadImage {
    /// Returns the exact number of admitted collapsed v6 code bytes.
    #[must_use]
    pub const fn allocation_len(&self) -> usize {
        self.code.len()
    }
    ...
    /// Returns the exact ISA retained by the complete collapsed v6 key.
    #[must_use]
    pub const fn host_isa(&self) -> HostIsa {
        self.key.target().host_isa()
    }
    ...
    /// Extracts a relocation-free image from one verified collapsed object.
    ///
    /// # Errors
    ///
    /// Returns [`VerifiedDirectLoadError`] when COFF extraction, relocation, or
    ...
    /// Returns exact target assumptions retained by this collapsed v6 image.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.key.target()
    }
...
impl VerifiedRegisterMaskedNoOperationPairLoadImage {
    /// Returns the exact number of admitted collapsed no-operation-pair v6 code
    /// bytes.
    #[must_use]
    pub const fn allocation_len(&self) -> usize {
        self.code.len()
    ...
    /// no-operation-pair v6 key.
    #[must_use]
    pub const fn host_isa(&self) -> HostIsa {
        self.key.target().host_isa()
    }

    /// Returns the complete retained collapsed no-operation-pair v6 artifact
    /// identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        &self.key
    ...
    /// Extracts a relocation-free image from one verified collapsed pair
    /// object.
    ///
    /// # Errors
    ///
...
impl VerifiedRegisterMaskedNoOperationRotateLoadImage {
    /// Returns the exact number of admitted collapsed no-operation/rotate v6
    /// code bytes.
    #[must_use]
    pub const fn allocation_len(&self) -> usize {
        self.code.len()
    ...
    /// Returns the complete retained collapsed no-operation/rotate v6 artifact
    /// identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        &self.key
...
impl VerifiedRegisterMaskedNoOperationLoadImage {
    /// Returns the exact number of admitted v6 no-operation code bytes.
    #[must_use]
    pub const fn allocation_len(&self) -> usize {
        self.code.len()
    }
    ...
    /// Extracts a supplied object under verified v6 no-operation identity.
    #[cfg(test)]
    #[doc(hidden)]
    ...
    /// Extracts relocation-free bytes from verified v6 no-operation code.
    ///
    /// # Errors
    ///
    /// Returns [`VerifiedDirectLoadError`] when COFF extraction, relocation, or
...
impl VerifiedRegisterMaskedCrazyLoadImage {
    /// Returns the exact number of admitted v6 Crazy code bytes.
    #[must_use]
    pub const fn allocation_len(&self) -> usize {
        self.code.len()
    }
    ...
    /// Extracts a supplied object under verified v6 Crazy identity.
    #[cfg(test)]
    #[doc(hidden)]
    pub fn from_object_for_test(
        artifact: &VerifiedRegisterMaskedCrazyNativeObjectArtifact,
    ...
    /// Extracts relocation-free bytes from verified v6 Crazy code.
    ///
    /// # Errors
    ///
    /// Returns [`VerifiedDirectLoadError`] when COFF extraction, relocation, or
...
impl VerifiedRegisterMaskedRotateLoadImage {
    /// Returns the exact number of admitted v6 rotate code bytes.
    #[must_use]
    pub const fn allocation_len(&self) -> usize {
        self.code.len()
    }
    ...
    /// Extracts a supplied object under verified v6 rotate identity.
    #[cfg(test)]
    #[doc(hidden)]
    pub fn from_object_for_test(
        artifact: &VerifiedRegisterMaskedRotateNativeObjectArtifact,
    ...
    /// Extracts relocation-free bytes from verified v6 rotate code.
    ///
    /// # Errors
    ///
    /// Returns [`VerifiedDirectLoadError`] when COFF extraction, relocation, or
...
impl VerifiedRegisterMaskedNonGraphicalLoadImage {
    /// Returns the exact number of admitted v6 code bytes.
    #[must_use]
    pub const fn allocation_len(&self) -> usize {
        self.code.len()
    ...
    /// Extracts relocation-free bytes from verified non-graphical v6 code.
    ///
    /// # Errors
    ///
    /// Returns [`VerifiedDirectLoadError`] when COFF extraction, relocation, or
