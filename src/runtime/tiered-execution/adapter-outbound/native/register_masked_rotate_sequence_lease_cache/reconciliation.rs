...
// Boundary-Contract:
// - Owns:
//   - Explicit reclamation and retry ownership for retired rotate leases.
// - Must-Not:
...
// - Summary:
//   - Reclaims retired rotate mappings and retains failed release owners.
// - Description:
...
//! Explicit reclamation and keyed retry for rotate sequence leases.

use super::{
    Arc, CacheValue, Display, FormatResult, Formatter,
    NativeExecutableMemoryAdapter, NativeExecutableSequenceCacheUsage,
    NativeExecutableSequenceWeight, RegisterMaskedRotateNativeSequenceKey,
    RegisterMaskedRotateNativeSequenceReleaseFailure, VecDeque,
};
...
#[derive(Debug)]
pub struct RegisterMaskedRotateNativeSequenceLeaseCacheLoadReleaseFailures<E> {
    candidate: Option<
...
/// Successful explicit reclamation pass over retired rotate sequences.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RegisterMaskedRotateNativeSequenceLeaseCacheReconciliation {
    released_keys: Vec<RegisterMaskedRotateNativeSequenceKey>,
    retained_keys: Vec<RegisterMaskedRotateNativeSequenceKey>,
}

/// One keyed sequence release failure removed from cache ownership for retry.
#[derive(Debug)]
pub struct RegisterMaskedRotateNativeSequenceLeaseCacheEntryReleaseFailure<E> {
    failure: RegisterMaskedRotateNativeSequenceReleaseFailure<E>,
    key: RegisterMaskedRotateNativeSequenceKey,
}

/// Aggregate reclamation failure after attempting every releasable entry.
#[derive(Debug)]
pub struct RegisterMaskedRotateNativeSequenceLeaseCacheReleaseFailure<E> {
    failures:
        Vec<RegisterMaskedRotateNativeSequenceLeaseCacheEntryReleaseFailure<E>>,
    released_keys: Vec<RegisterMaskedRotateNativeSequenceKey>,
    retained_keys: Vec<RegisterMaskedRotateNativeSequenceKey>,
}

/// Result of reconciling retired rotate resident sequences.
pub type RegisterMaskedRotateNativeSequenceLeaseReconciliationResult<E> =
    Result<
        RegisterMaskedRotateNativeSequenceLeaseCacheReconciliation,
        Box<RegisterMaskedRotateNativeSequenceLeaseCacheReleaseFailure<E>>,
    >;

type EntryReleaseFailure<E> =
    RegisterMaskedRotateNativeSequenceLeaseCacheEntryReleaseFailure<E>;

impl RegisterMaskedRotateNativeSequenceLeaseCacheReconciliation {
    /// Returns retired keys released during this reclamation pass.
    #[must_use]
    pub fn released_keys(&self) -> &[RegisterMaskedRotateNativeSequenceKey] {
        &self.released_keys
    }

    /// Returns retired keys still resident behind external leases.
    #[must_use]
    pub fn retained_keys(&self) -> &[RegisterMaskedRotateNativeSequenceKey] {
        &self.retained_keys
    }
}

impl<E> RegisterMaskedRotateNativeSequenceLeaseCacheEntryReleaseFailure<E> {
...
impl<E: Display> Display
    for RegisterMaskedRotateNativeSequenceLeaseCacheEntryReleaseFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "rotate v6 sequence lease keyed release failed: {}",
            self.failure,
        )
    }
}

impl<E> RegisterMaskedRotateNativeSequenceLeaseCacheLoadReleaseFailures<E> {
    ...
    pub fn retry<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedRotateNativeSequenceLeaseReconciliationResult<E>
    where
...
impl<E> RegisterMaskedRotateNativeSequenceLeaseCacheReleaseFailure<E> {
...
impl<E: Display> Display
    for RegisterMaskedRotateNativeSequenceLeaseCacheReleaseFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "rotate v6 sequence lease cache retained {} failed releases",
            self.failures.len(),
        )
    }
}
...
) -> RegisterMaskedRotateNativeSequenceLeaseReconciliationResult<Adapter::Error>
where
...
fn reconciliation_result<E>(
    ...
    if failures.is_empty() {
        Ok(RegisterMaskedRotateNativeSequenceLeaseCacheReconciliation {
            released_keys,
            retained_keys,
        })
    } else {
...
fn retry_keyed_release_failures<Adapter>(
    adapter: &mut Adapter,
    mut released_keys: Vec<RegisterMaskedRotateNativeSequenceKey>,
    retained_keys: Vec<RegisterMaskedRotateNativeSequenceKey>,
    pending: Vec<
        RegisterMaskedRotateNativeSequenceLeaseCacheEntryReleaseFailure<
            Adapter::Error,
