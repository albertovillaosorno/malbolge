...
// Boundary-Contract:
// - Owns:
//   - Weighted FIFO residency for loaded rotate v6 sequences.
// - Must-Not:
...
// - Merge-When:
//   - One reviewed rotate sequence store subsumes cache and lease policy.
// - Summary:
//   - Reuses exact loaded rotate chains under weighted FIFO limits.
// - Description:
...
//! Weighted FIFO cache for loaded rotate register-masked v6 sequences.

#[path = "register_masked_rotate_sequence_cache/reconfiguration.rs"]
mod reconfiguration;
...
enum LoadFailureCause<E> {
    ...
    Eviction(Box<RegisterMaskedRotateNativeSequenceReleaseFailure<E>>),
...
#[derive(Debug)]
pub struct RegisterMaskedRotateNativeSequenceCache {
...
pub enum RegisterMaskedRotateNativeSequenceCacheDisposition {
...
#[derive(Debug)]
pub struct RegisterMaskedRotateNativeSequenceCacheEntry<'cache> {
    disposition: RegisterMaskedRotateNativeSequenceCacheDisposition,
    key: &'cache RegisterMaskedRotateNativeSequenceKey,
    sequence: &'cache LoadedRegisterMaskedRotateNativeSequence,
}

/// Internal cache inconsistency rejected before exposing a borrow.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedRotateNativeSequenceCacheInvariantError {
...
#[derive(Debug)]
pub struct RegisterMaskedRotateNativeSequenceCacheLoadReleaseFailures<E> {
    candidate: Option<RegisterMaskedRotateNativeSequenceReleaseFailure<E>>,
    eviction: Option<RegisterMaskedRotateNativeSequenceReleaseFailure<E>>,
}

/// Failure while loading, evicting, or publishing one exact cache miss.
#[derive(Debug)]
pub struct RegisterMaskedRotateNativeSequenceCacheLoadFailure<E> {
...
#[derive(Debug)]
pub struct RegisterMaskedRotateNativeSequenceCacheReleaseFailure<E> {
    attempted_entries: usize,
    failures: Vec<RegisterMaskedRotateNativeSequenceReleaseFailure<E>>,
    released_entries: usize,
}

type CapacityError = NativeExecutableSequenceCacheCapacityError;
type CacheInvalidationResult<E> =
    RegisterMaskedRotateNativeSequenceCacheInvalidationResult<E>;
type CacheLoadResult<'cache, E> =
    RegisterMaskedRotateNativeSequenceCacheLoadResult<'cache, E>;
type CacheReleaseResult<E> =
    RegisterMaskedRotateNativeSequenceCacheReleaseResult<E>;
type CacheReconfigurationResult<E> =
    RegisterMaskedRotateNativeSequenceCacheReconfigurationResult<E>;

type CachePublicationResult<E> = Result<
    RegisterMaskedRotateNativeSequenceCacheDisposition,
    Box<RegisterMaskedRotateNativeSequenceCacheLoadFailure<E>>,
>;
type CacheFitResult<E> = Result<
    (CacheCandidate, Vec<RegisterMaskedRotateNativeSequenceKey>),
    Box<RegisterMaskedRotateNativeSequenceCacheLoadFailure<E>>,
>;
...
type SequenceReleaseFailure<E> =
    RegisterMaskedRotateNativeSequenceReleaseFailure<E>;

/// Published weighted capacity limits for this rotate sequence cache.
pub type RegisterMaskedRotateNativeSequenceCacheLimits =
    NativeExecutableSequenceCacheLimits;

/// Exact resources retained by this rotate sequence cache.
pub type RegisterMaskedRotateNativeSequenceCacheUsage =
    NativeExecutableSequenceCacheUsage;

/// Capacity rejection produced by this rotate sequence cache.
pub type RegisterMaskedRotateNativeSequenceCacheCapacityError =
    NativeExecutableSequenceCacheCapacityError;

/// Result of loading or reusing one exact rotate sequence plan.
pub type RegisterMaskedRotateNativeSequenceCacheLoadResult<'cache, E> = Result<
    RegisterMaskedRotateNativeSequenceCacheEntry<'cache>,
    Box<RegisterMaskedRotateNativeSequenceCacheLoadFailure<E>>,
>;

/// Result of invalidating one exact loaded rotate sequence.
pub type RegisterMaskedRotateNativeSequenceCacheInvalidationResult<E> =
    Result<bool, Box<RegisterMaskedRotateNativeSequenceReleaseFailure<E>>>;

/// Result of releasing every sequence removed from cache authority.
pub type RegisterMaskedRotateNativeSequenceCacheReleaseResult<E> =
    Result<(), Box<RegisterMaskedRotateNativeSequenceCacheReleaseFailure<E>>>;

impl RegisterMaskedRotateNativeSequenceCacheDisposition {
    /// Returns the first exact FIFO key evicted for an inserted miss.
    #[must_use]
    pub fn evicted_key(
        &self,
    ) -> Option<&RegisterMaskedRotateNativeSequenceKey> {
...
impl RegisterMaskedRotateNativeSequenceCacheEntry<'_> {
    ...
    #[must_use]
    pub const fn sequence(&self) -> &LoadedRegisterMaskedRotateNativeSequence {
        self.sequence
    }
}

impl Display for RegisterMaskedRotateNativeSequenceCacheInvariantError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str(match self {
            Self::EntryMissing => "located exact rotate sequence is missing",
        })
    }
}

impl<E> RegisterMaskedRotateNativeSequenceCacheLoadFailure<E> {
    ...
    pub const fn capacity_error(
        &self,
    ) -> Option<RegisterMaskedRotateNativeSequenceCacheCapacityError> {
        match self.cause {
    ...
    pub fn into_release_failures(
        self,
    ) -> RegisterMaskedRotateNativeSequenceCacheLoadReleaseFailures<E> {
...
impl<E: Display> Display
    for RegisterMaskedRotateNativeSequenceCacheLoadFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str("rotate v6 sequence cache miss failed: ")?;
        match &self.cause {
...
impl<E> RegisterMaskedRotateNativeSequenceCacheLoadReleaseFailures<E> {
    /// Returns failed candidate cleanup after unsuccessful publication.
    #[must_use]
...
impl<E> RegisterMaskedRotateNativeSequenceCacheReleaseFailure<E> {
...
impl<E: Display> Display
    for RegisterMaskedRotateNativeSequenceCacheReleaseFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "rotate v6 sequence cache retained {} of {} entries",
            self.failed_entries(),
            self.attempted_entries,
        )
    }
...
impl RegisterMaskedRotateNativeSequenceCache {
    ...
    /// Loads or reuses one exact admitted rotate sequence.
    ///
        ...
        Adapter: NativeExecutableMemoryAdapter,
    {
        ...
        if self.position(&key).is_some() {
            return self.entry_for_key(
                &key,
                RegisterMaskedRotateNativeSequenceCacheDisposition::Hit,
            );
        }
        let sequence =
    ...
    fn entry_for_key<'cache, E>(
        ...
        self.entries
            ...
            .map_or_else(
                ...
                |entry| {
                    Ok(RegisterMaskedRotateNativeSequenceCacheEntry {
                        disposition,
                        key: &entry.key,
        ...
        Adapter: NativeExecutableMemoryAdapter,
    {
        ...
        Ok(
            RegisterMaskedRotateNativeSequenceCacheDisposition::Inserted {
                evicted,
            },
        )
    }
    ...
    #[must_use]
    pub const fn usage(&self) -> RegisterMaskedRotateNativeSequenceCacheUsage {
        self.usage
    }
    ...
    pub const fn with_limits(
        limits: RegisterMaskedRotateNativeSequenceCacheLimits,
    ) -> Self {
        Self {
...
) -> RegisterMaskedRotateNativeSequenceCacheLoadFailure<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
{
...
const fn cache_invariant_failure<E>(
    requested_key: RegisterMaskedRotateNativeSequenceKey,
) -> RegisterMaskedRotateNativeSequenceCacheLoadFailure<E> {
    RegisterMaskedRotateNativeSequenceCacheLoadFailure {
        candidate_cleanup_failure: None,
