...
// Boundary-Contract:
// - Owns:
//   - Loaded mapping ownership and reverse cleanup for admitted Output
//     register-masked v6 sequence plans.
// - Must-Not:
//   - Execute native code, call runners, own lease caches, or alter plan
//     admission semantics.
// - Allows:
//   - Inputs: admitted Output plans and a memory adapter.
...
// - Merge-When:
//   - One reviewed Output sequence coordinator owns load and execution.
// - Summary:
//   - Publishes only fully loaded Output plans and releases in reverse.
// - Description:
...
//! Loaded ownership for admitted Output register-masked v6 plans.
...
use super::register_masked_output_sequence as sequence;
use super::register_masked_resident::{
    RegisterMaskedOutputNativeExecutableOwner,
    RegisterMaskedOutputNativeOwnerLoadFailure,
};

/// Indexed failure while loading one admitted Output v6 sequence.
#[derive(Debug, Eq, PartialEq)]
pub struct RegisterMaskedOutputNativeSequenceLoadFailure<MemoryError> {
    cause: Box<RegisterMaskedOutputNativeOwnerLoadFailure<MemoryError>>,
    cleanup_failure: Option<
        Box<RegisterMaskedOutputNativeSequenceReleaseFailure<MemoryError>>,
    >,
    index: usize,
    loaded_count: usize,
}
...
#[derive(Debug, Eq, PartialEq)]
pub struct RegisterMaskedOutputNativeSequenceReleaseFailure<MemoryError> {
    attempted_count: usize,
    failures:
        Vec<RegisterMaskedOutputNativeExecutableReleaseFailure<MemoryError>>,
    released_count: usize,
}

/// Result of loading every mapping for one admitted Output v6 plan.
pub type RegisterMaskedOutputNativeSequenceLoadResult<MemoryError> = Result<
    LoadedRegisterMaskedOutputNativeSequence,
    Box<RegisterMaskedOutputNativeSequenceLoadFailure<MemoryError>>,
>;

/// Result of releasing every mapping retained by a Output sequence.
pub type RegisterMaskedOutputNativeSequenceReleaseResult<MemoryError> = Result<
    (),
    Box<RegisterMaskedOutputNativeSequenceReleaseFailure<MemoryError>>,
>;

/// Fully loaded admitted Output v6 sequence without runner authority.
#[derive(Debug)]
pub struct LoadedRegisterMaskedOutputNativeSequence {
    owners: Vec<RegisterMaskedOutputNativeExecutableOwner>,
    plan: sequence::RegisterMaskedOutputNativeSequencePlan,
}

impl<MemoryError> RegisterMaskedOutputNativeSequenceLoadFailure<MemoryError> {
    /// Returns retryable prefix-cleanup failure, when cleanup also failed.
    #[must_use]
    pub const fn cleanup_failure(
        &self,
    ) -> Option<&RegisterMaskedOutputNativeSequenceReleaseFailure<MemoryError>>
    {
    ...
    pub const fn owner_failure(
        &self,
    ) -> &RegisterMaskedOutputNativeOwnerLoadFailure<MemoryError> {
        &self.cause
    }
}

impl<MemoryError: Display> Display
    for RegisterMaskedOutputNativeSequenceLoadFailure<MemoryError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "Output v6 sequence load failed at step {}: {}",
...
impl<MemoryError>
    RegisterMaskedOutputNativeSequenceReleaseFailure<MemoryError>
{
    ...
    pub fn failures(
        &self,
    ) -> &[RegisterMaskedOutputNativeExecutableReleaseFailure<MemoryError>]
    {
        &self.failures
    }
    ...
    pub fn retry<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedOutputNativeSequenceReleaseResult<MemoryError>
    where
        Adapter: NativeExecutableMemoryAdapter<Error = MemoryError>,
    {
        retry_output_release_failures(adapter, self.failures)
    }
}

impl<MemoryError: Display> Display
    for RegisterMaskedOutputNativeSequenceReleaseFailure<MemoryError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "Output v6 sequence release retained {} of {} mappings",
            self.failed_count(),
            self.attempted_count,
        )
    }
...
impl LoadedRegisterMaskedOutputNativeSequence {
    ...
    pub const fn plan(
        &self,
    ) -> &sequence::RegisterMaskedOutputNativeSequencePlan {
        &self.plan
    }
    ...
    pub fn release<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedOutputNativeSequenceReleaseResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        release_output_owners(adapter, self.owners)
    }
}
...
pub fn load_register_masked_output_native_sequence<Adapter>(
    plan: &sequence::RegisterMaskedOutputNativeSequencePlan,
    adapter: &mut Adapter,
) -> RegisterMaskedOutputNativeSequenceLoadResult<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
{
    ...
    {
        match RegisterMaskedOutputNativeExecutableOwner::load(
            ...
            Err(cause) => {
                ...
                let cleanup_failure =
                    release_output_owners(adapter, owners).err();
                return Err(Box::new(
    ...
    Ok(LoadedRegisterMaskedOutputNativeSequence {
        owners,
        plan: plan.clone(),
    })
}

fn release_output_owners<Adapter>(
    adapter: &mut Adapter,
    owners: Vec<RegisterMaskedOutputNativeExecutableOwner>,
) -> RegisterMaskedOutputNativeSequenceReleaseResult<Adapter::Error>
where
...
fn retry_output_release_failures<Adapter>(
    adapter: &mut Adapter,
    pending: Vec<
        RegisterMaskedOutputNativeExecutableReleaseFailure<Adapter::Error>,
    >,
) -> RegisterMaskedOutputNativeSequenceReleaseResult<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
...
fn release_pass_result<MemoryError>(
    ...
    if failures.is_empty() {
        ...
        Err(Box::new(RegisterMaskedOutputNativeSequenceReleaseFailure {
            attempted_count,
            failures,
            released_count,
        }))
