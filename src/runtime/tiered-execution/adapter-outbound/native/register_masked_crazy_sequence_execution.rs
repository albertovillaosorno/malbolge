...
// Boundary-Contract:
// - Owns:
//   - Indexed runner execution for already-loaded Crazy v6 sequences.
// - Must-Not:
//   - Allocate, release, cache, or alter admitted sequence topology.
// - Allows:
//   - Inputs: loaded Crazy owner, runner, rebased entry, caller buffers.
...
// - Merge-When:
//   - One reviewed Crazy coordinator safely owns load and execution.
// - Summary:
//   - Executes retained Crazy mappings without adapter work.
// - Description:
...
//! Indexed execution for loaded Crazy register-masked v6 sequences.
...
use super::{
    register_masked_crazy_loaded_sequence as loaded_sequence,
    register_masked_resident as resident,
};

/// Admitted result of executing one loaded Crazy v6 sequence.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedCrazyNativeSequenceOutcome {
...
pub struct RegisterMaskedCrazyNativeSequenceExecutionFailure<RunnerError> {
    cause: Box<
        resident::RegisterMaskedCrazyNativeOwnerExecutionFailure<RunnerError>,
    >,
...
/// Result of executing one already-loaded Crazy v6 sequence.
pub type RegisterMaskedCrazyNativeSequenceExecutionResult<RunnerError> = Result<
    RegisterMaskedCrazyNativeSequenceOutcome,
    Box<RegisterMaskedCrazyNativeSequenceExecutionFailure<RunnerError>>,
>;

impl<RunnerError>
    RegisterMaskedCrazyNativeSequenceExecutionFailure<RunnerError>
{
...
impl<RunnerError: Display> Display
    for RegisterMaskedCrazyNativeSequenceExecutionFailure<RunnerError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "Crazy v6 sequence step {} failed after {} steps: {}",
            self.step_index, self.completed_steps, self.cause,
        )
    }
}

impl RegisterMaskedCrazyNativeSequenceOutcome {
...
pub fn execute_loaded_register_masked_crazy_native_sequence<Runner>(
    loaded: &loaded_sequence::LoadedRegisterMaskedCrazyNativeSequence,
    runner: &mut Runner,
    entry: ProfileMachineObservation,
    buffers: NativeRegionBuffers<'_>,
) -> RegisterMaskedCrazyNativeSequenceExecutionResult<Runner::Error>
where
    Runner: RegisterMaskedCrazyNativeRunner,
{
    use RegisterMaskedCrazyNativeSequenceOutcome as Outcome;
    ...
    for (index, owner) in loaded.owners().iter().enumerate() {
        let outcome = owner
            ...
            .map_err(|cause| {
                Box::new(RegisterMaskedCrazyNativeSequenceExecutionFailure {
                    cause,
