...
// Boundary-Contract:
// - Owns:
//   - Weighted FIFO residency and limit publication for Output v6.
// - Must-Not:
//   - Project authority into halt caches/sequences, reconcile retirement
//     implicitly on hit/miss, or refresh FIFO age on hits.
// - Allows:
//   - Inputs: exact Output v6 programs/artifacts, published limits, and a
//     memory adapter.
...
// - Summary:
//   - Reuses exact Output v6 mappings under explicit weighted limits.
// - Description:
...
//! Weighted multi-entry lease cache for Output register-masked v6.

#[path = "register_masked_output_lease_cache/reconfiguration.rs"]
mod reconfiguration;
...
use super::runner::RegisterMaskedOutputNativeRunner;
use crate::execution_cache::NativeArtifactKey;

type OwnerLoadFailure<E> = RegisterMaskedOutputNativeOwnerLoadFailure<E>;
type EntryReleaseFailure<E> =
    RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E>;

type CacheInvalidation = RegisterMaskedOutputLeaseCacheInvalidation;

type CacheReconfigurationResult<E> =
    RegisterMaskedOutputLeaseCacheReconfigurationResult<E>;
...
enum CacheVictimOutcome<E> {
    ReleaseFailed {
        failure: RegisterMaskedOutputNativeExecutableReleaseFailure<E>,
        weight: NativeExecutableSequenceWeight,
    },
...
/// Caller-owned fixed-limit cache with exact Output v6 leases.
#[derive(Debug)]
pub struct RegisterMaskedOutputLeaseCache {
...
/// Immutable external ownership of one exact Output v6 resident.
#[derive(Clone, Debug)]
pub struct RegisterMaskedOutputLease {
    key: NativeArtifactKey,
    resident: Arc<RegisterMaskedOutputNativeExecutableOwner>,
}
...
pub enum RegisterMaskedOutputLeaseCacheDisposition {
...
#[derive(Debug)]
pub struct RegisterMaskedOutputLeaseCacheAcquisition {
    disposition: RegisterMaskedOutputLeaseCacheDisposition,
    lease: RegisterMaskedOutputLease,
}

/// Exact resident state preventing one candidate from fitting fixed limits.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RegisterMaskedOutputLeaseCacheBlock {
    limits: NativeExecutableSequenceCacheLimits,
    retired_keys: Vec<NativeArtifactKey>,
    usage: NativeExecutableSequenceCacheUsage,
}
...
enum LoadFailureCause<E> {
    ...
    Leases(RegisterMaskedOutputLeaseCacheBlock),
    Load(Box<RegisterMaskedOutputNativeOwnerLoadFailure<E>>),
    Release(RegisterMaskedOutputNativeExecutableReleaseFailure<E>),
}

/// Failure while loading, weighing, evicting, or publishing one miss.
#[derive(Debug)]
pub struct RegisterMaskedOutputLeaseCacheLoadFailure<E> {
    candidate_cleanup_failure:
        Option<RegisterMaskedOutputNativeExecutableReleaseFailure<E>>,
...
/// Result of invalidating one exact active Output resident.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedOutputLeaseCacheInvalidation {
...
#[derive(Debug)]
pub struct RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E> {
    failure: RegisterMaskedOutputNativeExecutableReleaseFailure<E>,
    key: NativeArtifactKey,
}
...
#[derive(Debug)]
pub struct RegisterMaskedOutputLeaseCacheLoadReleaseFailures<E> {
    candidate: Option<RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E>>,
    eviction: Option<RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E>>,
}

/// Successful reclamation pass over cache-owned Output residents.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RegisterMaskedOutputLeaseCacheReleaseSummary {
...
#[derive(Debug)]
pub struct RegisterMaskedOutputLeaseCacheReleaseFailure<E> {
    failures: Vec<RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E>>,
    released_keys: Vec<NativeArtifactKey>,
    retained_keys: Vec<NativeArtifactKey>,
}

/// Published weighted capacity limits for this Output resident cache.
pub type RegisterMaskedOutputLeaseCacheLimits =
    NativeExecutableSequenceCacheLimits;

/// Exact active-plus-retired resource usage retained by this cache.
pub type RegisterMaskedOutputLeaseCacheUsage =
    NativeExecutableSequenceCacheUsage;

/// Capacity rejection produced by this Output resident cache.
pub type RegisterMaskedOutputLeaseCacheCapacityError =
    NativeExecutableSequenceCacheCapacityError;

/// Result of acquiring one active immutable Output resident lease.
pub type RegisterMaskedOutputLeaseCacheLoadResult<E> = Result<
    RegisterMaskedOutputLeaseCacheAcquisition,
    Box<RegisterMaskedOutputLeaseCacheLoadFailure<E>>,
>;
...
/// Result of invalidating one exact active Output resident.
pub type RegisterMaskedOutputLeaseCacheInvalidationResult<E> = Result<
    RegisterMaskedOutputLeaseCacheInvalidation,
    Box<RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E>>,
>;

/// Result of reconciling retired Output residents.
pub type RegisterMaskedOutputLeaseCacheReconciliationResult<E> =
    RegisterMaskedOutputLeaseCacheReleaseResult<E>;

/// Result of reclaiming cache-owned Output residents.
pub type RegisterMaskedOutputLeaseCacheReleaseResult<E> = Result<
    RegisterMaskedOutputLeaseCacheReleaseSummary,
    Box<RegisterMaskedOutputLeaseCacheReleaseFailure<E>>,
>;

impl RegisterMaskedOutputLease {
...
impl RegisterMaskedOutputLeaseCacheDisposition {
...
impl RegisterMaskedOutputLeaseCacheAcquisition {
    ...
    #[must_use]
    pub fn into_lease(self) -> RegisterMaskedOutputLease {
        self.lease
    }

    /// Returns the immutable lease retained by this acquisition.
    #[must_use]
    pub const fn lease(&self) -> &RegisterMaskedOutputLease {
        &self.lease
    }
}

impl RegisterMaskedOutputLeaseCacheBlock {
    /// Returns fixed resident limits that could not admit the candidate.
    #[must_use]
    pub const fn limits(&self) -> RegisterMaskedOutputLeaseCacheLimits {
        self.limits
    }
    ...
    #[must_use]
    pub const fn usage(&self) -> RegisterMaskedOutputLeaseCacheUsage {
        self.usage
    }
}

impl<E> RegisterMaskedOutputLeaseCacheLoadFailure<E> {
    /// Returns lease blockage when retired weight prevents candidate admission.
    #[must_use]
    pub const fn block(&self) -> Option<&RegisterMaskedOutputLeaseCacheBlock> {
        match &self.cause {
    ...
    pub const fn capacity_error(
        &self,
    ) -> Option<RegisterMaskedOutputLeaseCacheCapacityError> {
        match self.cause {
    ...
    pub fn into_release_failures(
        self,
    ) -> RegisterMaskedOutputLeaseCacheLoadReleaseFailures<E> {
        ...
        let eviction = match self.cause {
            LoadFailureCause::Release(failure) => {
                Some(RegisterMaskedOutputLeaseCacheEntryReleaseFailure {
                    failure,
                    key: eviction_key,
                })
            },
    ...
    pub const fn load_failure(
        &self,
    ) -> Option<&RegisterMaskedOutputNativeOwnerLoadFailure<E>> {
        match &self.cause {
...
impl<E: Display> Display for RegisterMaskedOutputLeaseCacheLoadFailure<E> {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str("Output v6 lease cache miss failed: ")?;
        match &self.cause {
...
impl<E> RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E> {
    /// Returns exact executable release ownership retained by this key.
    #[must_use]
    pub const fn failure(
        &self,
    ) -> &RegisterMaskedOutputNativeExecutableReleaseFailure<E> {
        &self.failure
    }
...
impl<E> RegisterMaskedOutputLeaseCacheLoadReleaseFailures<E> {
    /// Returns candidate cleanup failure, when candidate release also failed.
    #[must_use]
    pub const fn candidate_failure(
        &self,
    ) -> Option<&RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E>> {
    ...
    pub fn retry<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedOutputLeaseCacheReconciliationResult<E>
    where
        Adapter: NativeExecutableMemoryAdapter<Error = E>,
    {
...
impl RegisterMaskedOutputLeaseCacheReleaseSummary {
...
impl<E> RegisterMaskedOutputLeaseCacheReleaseFailure<E> {
    ...
    pub fn into_failures(
        self,
    ) -> Vec<RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E>> {
        self.failures
    }
...
impl<E: Display> Display for RegisterMaskedOutputLeaseCacheReleaseFailure<E> {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "Output v6 lease cache release had {} failure(s)",
            self.failures.len()
        )
    }
}

impl RegisterMaskedOutputLeaseCache {
    ...
    /// Loads or reuses one exact Output resident and returns a lease.
    ///
    ...
    ) -> RegisterMaskedOutputLeaseCacheLoadResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
            ...
            && entry.resident.artifact() == artifact
        {
            return Ok(Self::active_acquisition(
                entry,
                RegisterMaskedOutputLeaseCacheDisposition::Hit,
            ));
        }
        if self.position(&key).is_some() {
            let failure = Box::new(OwnerLoadFailure::ArtifactIdentity);
            return Err(Box::new(RegisterMaskedOutputLeaseCacheLoadFailure {
        ...
        .map_err(|failure| {
            Box::new(RegisterMaskedOutputLeaseCacheLoadFailure {
                candidate_cleanup_failure: None,
                cause: LoadFailureCause::Load(failure),
                evicted_keys: Vec::new(),
    ...
    pub fn invalidate_key<Adapter>(
        ...
        adapter: &mut Adapter,
        key: &NativeArtifactKey,
    ) -> RegisterMaskedOutputLeaseCacheInvalidationResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        ...
        Adapter: NativeExecutableMemoryAdapter,
    {
        ...
        Ok(RegisterMaskedOutputLeaseCacheAcquisition {
            disposition: RegisterMaskedOutputLeaseCacheDisposition::Inserted {
    ...
    pub fn reconcile_retired<Adapter>(
        &mut self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedOutputLeaseCacheReleaseResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
...
fn blocked_failure<Adapter>(
    ...
    cache: &RegisterMaskedOutputLeaseCache,
) -> RegisterMaskedOutputLeaseCacheLoadFailure<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
{
    let block = RegisterMaskedOutputLeaseCacheBlock {
        limits: cache.limits,
        retired_keys: cache
            .retired
            .iter()
...
fn reconciliation_result<E>(
    ...
    if failures.is_empty() {
        Ok(RegisterMaskedOutputLeaseCacheReleaseSummary {
            released_keys,
            retained_keys,
        })
    } else {
        Err(Box::new(RegisterMaskedOutputLeaseCacheReleaseFailure {
            failures,
            released_keys,
            retained_keys,
        }))
...
fn retry_keyed_release_failures<Adapter>(
    adapter: &mut Adapter,
    pending: RegisterMaskedOutputLeaseCacheReleaseFailure<Adapter::Error>,
) -> RegisterMaskedOutputLeaseCacheReconciliationResult<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
{
    let mut failures = Vec::new();
