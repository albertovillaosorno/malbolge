...
// Boundary-Contract:
// - Owns:
//   - Transactional weighted-limit publication for the no-operation cache.
// - Must-Not:
...
// - Summary:
//   - Shrinks no-operation cache limits through explicit FIFO cleanup.
// - Description:
//   - Prior limits remain published until every required release succeeds.
// - Usage:
//   - Called only by the no-operation sequence cache limit coordinator.
// - Defaults:
//   - Expansion and already-satisfied requests perform no adapter work.
//

//! Weighted-limit reconfiguration for cached no-operation sequences.
...
pub struct RegisterMaskedNoOperationNativeSequenceCacheReconfiguration {
    evicted_keys: Vec<RegisterMaskedNoOperationNativeSequenceKey>,
    new_limits: RegisterMaskedNoOperationNativeSequenceCacheLimits,
    previous_limits: RegisterMaskedNoOperationNativeSequenceCacheLimits,
}

/// Failed weighted-limit publication retaining exact cleanup ownership.
#[derive(Debug)]
pub struct RegisterMaskedNoOperationNativeSequenceCacheReconfigurationFailure<E>
{
    cause: ReconfigurationFailureCause<E>,
    evicted_keys: Vec<RegisterMaskedNoOperationNativeSequenceKey>,
    requested_limits: RegisterMaskedNoOperationNativeSequenceCacheLimits,
    retained_limits: RegisterMaskedNoOperationNativeSequenceCacheLimits,
}

/// Result of publishing new weighted limits after required FIFO cleanup.
pub type RegisterMaskedNoOperationNativeSequenceCacheReconfigurationResult<E> =
    Result<
...
impl<E> RegisterMaskedNoOperationNativeSequenceCacheReconfigurationFailure<E> {
...
impl<E: Display> Display
    for RegisterMaskedNoOperationNativeSequenceCacheReconfigurationFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str("no-operation v6 cache reconfiguration failed: ")?;
        match &self.cause {
...
pub(super) fn evict_for_reconfiguration<Adapter>(
    cache: &mut RegisterMaskedNoOperationNativeSequenceCache,
    adapter: &mut Adapter,
    requested_limits: RegisterMaskedNoOperationNativeSequenceCacheLimits,
    retained_limits: RegisterMaskedNoOperationNativeSequenceCacheLimits,
