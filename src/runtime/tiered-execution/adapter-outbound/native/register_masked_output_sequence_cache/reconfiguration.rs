...
// Boundary-Contract:
// - Owns:
//   - Transactional weighted-limit publication for the Output cache.
// - Must-Not:
...
// - Summary:
//   - Shrinks Output cache limits through explicit FIFO cleanup.
// - Description:
//   - Prior limits remain published until every required release succeeds.
// - Usage:
//   - Called only by the Output sequence cache limit coordinator.
// - Defaults:
//   - Expansion and already-satisfied requests perform no adapter work.
//

//! Weighted-limit reconfiguration for cached Output sequences.
...
enum ReconfigurationFailureCause<E> {
    ...
    Release(Box<RegisterMaskedOutputNativeSequenceReleaseFailure<E>>),
}

/// Successful weighted-limit publication and its FIFO removals.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RegisterMaskedOutputNativeSequenceCacheReconfiguration {
    evicted_keys: Vec<RegisterMaskedOutputNativeSequenceKey>,
    new_limits: RegisterMaskedOutputNativeSequenceCacheLimits,
    previous_limits: RegisterMaskedOutputNativeSequenceCacheLimits,
}

/// Failed weighted-limit publication retaining exact cleanup ownership.
#[derive(Debug)]
pub struct RegisterMaskedOutputNativeSequenceCacheReconfigurationFailure<E> {
    cause: ReconfigurationFailureCause<E>,
    evicted_keys: Vec<RegisterMaskedOutputNativeSequenceKey>,
    requested_limits: RegisterMaskedOutputNativeSequenceCacheLimits,
    retained_limits: RegisterMaskedOutputNativeSequenceCacheLimits,
}

/// Result of publishing new weighted limits after required FIFO cleanup.
pub type RegisterMaskedOutputNativeSequenceCacheReconfigurationResult<E> =
    Result<
        RegisterMaskedOutputNativeSequenceCacheReconfiguration,
        Box<RegisterMaskedOutputNativeSequenceCacheReconfigurationFailure<E>>,
    >;

type ReconfigurationEvictionResult<E> = Result<
    Vec<RegisterMaskedOutputNativeSequenceKey>,
    Box<RegisterMaskedOutputNativeSequenceCacheReconfigurationFailure<E>>,
>;

impl RegisterMaskedOutputNativeSequenceCacheReconfiguration {
...
impl<E> RegisterMaskedOutputNativeSequenceCacheReconfigurationFailure<E> {
...
impl<E: Display> Display
    for RegisterMaskedOutputNativeSequenceCacheReconfigurationFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str("Output v6 cache reconfiguration failed: ")?;
        match &self.cause {
...
const fn invariant_failure<E>(
    ...
    requested_limits: RegisterMaskedOutputNativeSequenceCacheLimits,
    retained_limits: RegisterMaskedOutputNativeSequenceCacheLimits,
) -> RegisterMaskedOutputNativeSequenceCacheReconfigurationFailure<E> {
...
const fn release_failure<E>(
    evicted_keys: Vec<RegisterMaskedOutputNativeSequenceKey>,
    failure: Box<RegisterMaskedOutputNativeSequenceReleaseFailure<E>>,
    requested_limits: RegisterMaskedOutputNativeSequenceCacheLimits,
    retained_limits: RegisterMaskedOutputNativeSequenceCacheLimits,
) -> RegisterMaskedOutputNativeSequenceCacheReconfigurationFailure<E> {
    RegisterMaskedOutputNativeSequenceCacheReconfigurationFailure {
