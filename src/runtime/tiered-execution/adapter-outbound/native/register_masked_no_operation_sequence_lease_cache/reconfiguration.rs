...
// Boundary-Contract:
// - Owns:
//   - Transactional resident-limit changes for no-operation sequence leases.
// - Must-Not:
...
// - Usage:
//   - Called by the no-operation sequence lease cache reconfiguration API.
// - Defaults:
//   - Existing retired entries are never reconciled implicitly.
//

//! Transactional limit changes for no-operation sequence lease residency.
...
pub struct RegisterMaskedNoOperationNativeSequenceLeaseCacheReconfiguration {
    evicted_keys: Vec<RegisterMaskedNoOperationNativeSequenceKey>,
...
#[derive(Debug)]
pub struct RegisterMaskedNoOperationNativeSequenceLeaseReconfigurationFailure<E>
{
...
pub type RegisterMaskedNoOperationNativeSequenceLeaseReconfigurationResult<E> =
    Result<
...
type ReconfigurationEvictionResult<E> = Result<
    ...
    Box<RegisterMaskedNoOperationNativeSequenceLeaseReconfigurationFailure<E>>,
>;

impl RegisterMaskedNoOperationNativeSequenceLeaseCacheReconfiguration {
...
impl<E> RegisterMaskedNoOperationNativeSequenceLeaseReconfigurationFailure<E> {
...
impl<E: Display> Display
    for RegisterMaskedNoOperationNativeSequenceLeaseReconfigurationFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str("no-operation v6 sequence lease reconfiguration failed: ")?;
        match &self.cause {
...
) -> RegisterMaskedNoOperationNativeSequenceLeaseReconfigurationFailure<E> {
