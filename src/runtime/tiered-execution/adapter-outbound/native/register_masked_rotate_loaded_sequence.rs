...
// Boundary-Contract:
// - Owns:
//   - Loaded mapping ownership and reverse cleanup for admitted rotate
//     register-masked v6 sequence plans.
// - Must-Not:
//   - Execute native code, call runners, own lease caches, or alter plan
//     admission semantics.
// - Allows:
//   - Inputs: admitted rotate plans and a memory adapter.
...
// - Merge-When:
//   - One reviewed rotate sequence coordinator owns load and execution.
// - Summary:
//   - Publishes only fully loaded rotate plans and releases in reverse.
// - Description:
...
//! Loaded ownership for admitted rotate register-masked v6 plans.
...
use super::register_masked_rotate_sequence as sequence;

/// Indexed failure while loading one admitted rotate v6 sequence.
#[derive(Debug, Eq, PartialEq)]
pub struct RegisterMaskedRotateNativeSequenceLoadFailure<MemoryError> {
    cause: Box<RegisterMaskedRotateNativeOwnerLoadFailure<MemoryError>>,
    cleanup_failure: Option<
        Box<RegisterMaskedRotateNativeSequenceReleaseFailure<MemoryError>>,
    >,
    index: usize,
    loaded_count: usize,
}
...
#[derive(Debug, Eq, PartialEq)]
pub struct RegisterMaskedRotateNativeSequenceReleaseFailure<MemoryError> {
    attempted_count: usize,
    failures:
        Vec<RegisterMaskedRotateNativeExecutableReleaseFailure<MemoryError>>,
    released_count: usize,
}

/// Result of loading every mapping for one admitted rotate v6 plan.
pub type RegisterMaskedRotateNativeSequenceLoadResult<MemoryError> = Result<
    LoadedRegisterMaskedRotateNativeSequence,
    Box<RegisterMaskedRotateNativeSequenceLoadFailure<MemoryError>>,
>;

/// Result of releasing every mapping retained by a rotate sequence.
pub type RegisterMaskedRotateNativeSequenceReleaseResult<MemoryError> = Result<
    (),
    Box<RegisterMaskedRotateNativeSequenceReleaseFailure<MemoryError>>,
>;

/// Fully loaded admitted rotate v6 sequence without runner authority.
#[derive(Debug)]
pub struct LoadedRegisterMaskedRotateNativeSequence {
    owners: Vec<RegisterMaskedRotateNativeExecutableOwner>,
    plan: sequence::RegisterMaskedRotateNativeSequencePlan,
}

impl<MemoryError> RegisterMaskedRotateNativeSequenceLoadFailure<MemoryError> {
    /// Returns retryable prefix-cleanup failure, when cleanup also failed.
    #[must_use]
    pub const fn cleanup_failure(
        &self,
    ) -> Option<&RegisterMaskedRotateNativeSequenceReleaseFailure<MemoryError>>
    {
    ...
    pub const fn owner_failure(
        &self,
    ) -> &RegisterMaskedRotateNativeOwnerLoadFailure<MemoryError> {
        &self.cause
    }
}

impl<MemoryError: Display> Display
    for RegisterMaskedRotateNativeSequenceLoadFailure<MemoryError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "rotate v6 sequence load failed at step {}: {}",
...
impl<MemoryError>
    RegisterMaskedRotateNativeSequenceReleaseFailure<MemoryError>
{
    ...
    pub fn failures(
        &self,
    ) -> &[RegisterMaskedRotateNativeExecutableReleaseFailure<MemoryError>]
    {
        &self.failures
    }
    ...
    pub fn retry<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedRotateNativeSequenceReleaseResult<MemoryError>
    where
        Adapter: NativeExecutableMemoryAdapter<Error = MemoryError>,
    {
        retry_rotate_release_failures(adapter, self.failures)
    }
}

impl<MemoryError: Display> Display
    for RegisterMaskedRotateNativeSequenceReleaseFailure<MemoryError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "rotate v6 sequence release retained {} of {} mappings",
            self.failed_count(),
            self.attempted_count,
        )
    }
...
impl LoadedRegisterMaskedRotateNativeSequence {
    ...
    pub const fn plan(
        &self,
    ) -> &sequence::RegisterMaskedRotateNativeSequencePlan {
        &self.plan
    }
    ...
    pub fn release<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedRotateNativeSequenceReleaseResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        release_rotate_owners(adapter, self.owners)
    }
}
...
pub fn load_register_masked_rotate_native_sequence<Adapter>(
    plan: &sequence::RegisterMaskedRotateNativeSequencePlan,
    adapter: &mut Adapter,
) -> RegisterMaskedRotateNativeSequenceLoadResult<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
{
    ...
    {
        match RegisterMaskedRotateNativeExecutableOwner::load(
            ...
            Err(cause) => {
                ...
                let cleanup_failure =
                    release_rotate_owners(adapter, owners).err();
                return Err(Box::new(
    ...
    Ok(LoadedRegisterMaskedRotateNativeSequence {
        owners,
        plan: plan.clone(),
    })
}

fn release_rotate_owners<Adapter>(
    adapter: &mut Adapter,
    owners: Vec<RegisterMaskedRotateNativeExecutableOwner>,
) -> RegisterMaskedRotateNativeSequenceReleaseResult<Adapter::Error>
where
...
fn retry_rotate_release_failures<Adapter>(
    adapter: &mut Adapter,
    pending: Vec<
        RegisterMaskedRotateNativeExecutableReleaseFailure<Adapter::Error>,
    >,
) -> RegisterMaskedRotateNativeSequenceReleaseResult<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
...
fn release_pass_result<MemoryError>(
    ...
    if failures.is_empty() {
        ...
        Err(Box::new(RegisterMaskedRotateNativeSequenceReleaseFailure {
            attempted_count,
            failures,
            released_count,
        }))
