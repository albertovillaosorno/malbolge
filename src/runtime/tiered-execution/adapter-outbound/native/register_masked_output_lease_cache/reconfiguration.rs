...
// Boundary-Contract:
// - Owns:
//   - Transactional resident-limit changes for Output v6 leases.
// - Must-Not:
//   - Reconcile prior retired entries, refresh hits, or reclaim live leases.
// - Allows:
//   - Inputs: Output lease cache owner, adapter, and requested limits.
...
// - Merge-When:
//   - Admission and reconfiguration become one Output transaction.
// - Summary:
//   - Shrinks active Output authority while preserving leased weight.
// - Description:
//   - Prior limits remain published until every required transition succeeds.
// - Usage:
//   - Called by `RegisterMaskedOutputLeaseCache::reconfigure_limits`.
// - Defaults:
//   - Existing retired entries are never reconciled implicitly.
//

//! Transactional weighted-limit changes for Output v6 residents.
...
use super::{
    ...
    RegisterMaskedOutputLeaseCache, RegisterMaskedOutputLeaseCacheBlock,
    RegisterMaskedOutputLeaseCacheEntryReleaseFailure, process_victim,
};

#[derive(Debug, Eq, PartialEq)]
struct OutputReconfigurationContext {
...
#[derive(Debug)]
enum OutputReconfigurationFailureCause<E> {
    Leases(RegisterMaskedOutputLeaseCacheBlock),
    Release(Box<RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E>>),
}

/// Successful Output resident-limit publication and FIFO removals.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RegisterMaskedOutputLeaseCacheReconfiguration {
    evicted_keys: Vec<NativeArtifactKey>,
...
/// Failed publication with exact Output blocker or cleanup ownership.
#[derive(Debug)]
pub struct RegisterMaskedOutputLeaseCacheReconfigurationFailure<E> {
    cause: OutputReconfigurationFailureCause<E>,
    evicted_keys: Vec<NativeArtifactKey>,
    requested_limits: NativeExecutableSequenceCacheLimits,
    retained_limits: NativeExecutableSequenceCacheLimits,
    retired_keys: Vec<NativeArtifactKey>,
...
/// Result of publishing new Output limits after active FIFO processing.
pub type RegisterMaskedOutputLeaseCacheReconfigurationResult<E> = Result<
    RegisterMaskedOutputLeaseCacheReconfiguration,
    Box<RegisterMaskedOutputLeaseCacheReconfigurationFailure<E>>,
>;
...
impl RegisterMaskedOutputLeaseCacheReconfiguration {
...
impl<E> RegisterMaskedOutputLeaseCacheReconfigurationFailure<E> {
    ...
    pub const fn block(&self) -> Option<&RegisterMaskedOutputLeaseCacheBlock> {
        match &self.cause {
            OutputReconfigurationFailureCause::Leases(block) => Some(block),
            OutputReconfigurationFailureCause::Release(_) => None,
        }
    }
    ...
    pub fn into_release_failure(
        ...
        match self.cause {
            OutputReconfigurationFailureCause::Leases(_) => None,
            OutputReconfigurationFailureCause::Release(failure) => {
                Some(*failure)
            },
        }
    }
...
impl<E: Display> Display
    for RegisterMaskedOutputLeaseCacheReconfigurationFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str("Output v6 lease cache reconfiguration failed: ")?;
        match &self.cause {
            OutputReconfigurationFailureCause::Leases(_) => {
...
fn blocked_failure<E>(
    context: OutputReconfigurationContext,
    cache: &RegisterMaskedOutputLeaseCache,
) -> RegisterMaskedOutputLeaseCacheReconfigurationFailure<E> {
    ...
    RegisterMaskedOutputLeaseCacheReconfigurationFailure {
        cause: OutputReconfigurationFailureCause::Leases(block),
...
fn release_failure<E>(
    context: OutputReconfigurationContext,
    failure: RegisterMaskedOutputLeaseCacheEntryReleaseFailure<E>,
) -> RegisterMaskedOutputLeaseCacheReconfigurationFailure<E> {
    RegisterMaskedOutputLeaseCacheReconfigurationFailure {
        cause: OutputReconfigurationFailureCause::Release(Box::new(failure)),
        evicted_keys: context.evicted_keys,
        requested_limits: context.requested_limits,
        retained_limits: context.retained_limits,
        retired_keys: context.retired_keys,
...
pub(super) fn evict_for_reconfiguration<Adapter>(
    cache: &mut RegisterMaskedOutputLeaseCache,
    adapter: &mut Adapter,
    requested_limits: NativeExecutableSequenceCacheLimits,
    retained_limits: NativeExecutableSequenceCacheLimits,
    ...
    while requested_limits.usage_exceeds(cache.usage) {
        ...
        match process_victim(adapter, victim) {
            ...
            CacheVictimOutcome::ReleaseFailed { failure, weight } => {
                cache.usage.remove(weight);
                let keyed = RegisterMaskedOutputLeaseCacheEntryReleaseFailure {
                    failure,
                    key: victim_key,
                };
