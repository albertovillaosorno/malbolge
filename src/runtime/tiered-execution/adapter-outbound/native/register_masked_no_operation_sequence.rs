...
// Boundary-Contract:
// - Owns:
//   - Exact no-operation register-masked v6 sequence plan admission.
// - Must-Not:
...
// - Summary:
//   - Admits exact no-operation v6 sequence topology before any mapping.
// - Description:
...
//! Exact plan admission for no-operation register-masked v6 sequences.
...
use super::direct::VerifiedRegisterMaskedNoOperationNativeObjectArtifact;
use crate::execution_cache::{NativeArtifactKey, NativeIdentityError};

/// Failure while admitting one ordered no-operation v6 sequence plan.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedNoOperationNativeSequencePlanError {
    ...
    /// A no-operation sequence must contain at least one semantic step.
    Empty,
...
pub struct RegisterMaskedNoOperationNativeSequencePlan {
    artifacts: Vec<VerifiedRegisterMaskedNoOperationNativeObjectArtifact>,
    entry: ProfileMachineObservation,
    exit: ProfileMachineObservation,
    programs: Vec<RegisterMaskedRegionEffectProgram>,
}

impl Display for RegisterMaskedNoOperationNativeSequencePlanError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        match self {
            Self::ArtifactCount { artifacts, programs } => write!(
                f,
                "no-operation v6 sequence has {artifacts} artifacts for \
                 {programs} programs",
            ),
            Self::ArtifactIdentity { index } => write!(
                f,
                "no-operation v6 sequence artifact identity differs at step \
                 {index}",
            ),
            Self::Empty => f.write_str(
                "no-operation v6 sequence requires at least one step",
            ),
            Self::Identity { index, .. } => write!(
                f,
                "no-operation v6 sequence identity failed at step {index}",
            ),
            Self::ObservationChain { index } => write!(
                f,
                "no-operation v6 sequence observation chain broke at step \
                 {index}",
            ),
            Self::ProfileMismatch { index } => write!(
                f,
                "no-operation v6 sequence profile changed at step {index}",
            ),
...
impl RegisterMaskedNoOperationNativeSequencePlan {
    ...
    /// authority. Reviewed no-operation artifacts each cover one semantic
    /// step. Ordered continuous steps may compose; a later position after
    /// termination fails.
    ///
    /// # Errors
    ///
    /// Returns [`RegisterMaskedNoOperationNativeSequencePlanError`] for empty,
    ...
    pub fn new(
        ...
        artifacts: &[VerifiedRegisterMaskedNoOperationNativeObjectArtifact],
    ) -> Result<Self, RegisterMaskedNoOperationNativeSequencePlanError> {
        use RegisterMaskedNoOperationNativeSequencePlanError as Error;
...
fn validate_program_chain(
    programs: &[RegisterMaskedRegionEffectProgram],
) -> Result<(), RegisterMaskedNoOperationNativeSequencePlanError> {
    use RegisterMaskedNoOperationNativeSequencePlanError as Error;
