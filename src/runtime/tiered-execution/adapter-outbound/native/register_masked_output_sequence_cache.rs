...
// Boundary-Contract:
// - Owns:
//   - Weighted FIFO residency for loaded Output v6 sequences.
// - Must-Not:
...
// - Merge-When:
//   - One reviewed Output sequence store subsumes cache and lease policy.
// - Summary:
//   - Reuses exact loaded Output chains under weighted FIFO limits.
// - Description:
...
//! Weighted FIFO cache for loaded Output register-masked v6 sequences.

#[path = "register_masked_output_sequence_cache/reconfiguration.rs"]
mod reconfiguration;
...
use super::register_masked_output_loaded_sequence::{
...
use super::register_masked_output_sequence::{
...
#[derive(Debug)]
struct CacheValue {
    key: RegisterMaskedOutputNativeSequenceKey,
    sequence: LoadedRegisterMaskedOutputNativeSequence,
...
struct CacheCapacityFailureContext {
    ...
    evicted_keys: Vec<RegisterMaskedOutputNativeSequenceKey>,
    key: RegisterMaskedOutputNativeSequenceKey,
    sequence: LoadedRegisterMaskedOutputNativeSequence,
}
...
enum LoadFailureCause<E> {
    ...
    Eviction(Box<RegisterMaskedOutputNativeSequenceReleaseFailure<E>>),
    Invariant(RegisterMaskedOutputNativeSequenceCacheInvariantError),
    Load(Box<RegisterMaskedOutputNativeSequenceLoadFailure<E>>),
}

/// Caller-owned exact-plan cache with one positive FIFO entry limit.
#[derive(Debug)]
pub struct RegisterMaskedOutputNativeSequenceCache {
...
pub enum RegisterMaskedOutputNativeSequenceCacheDisposition {
    ...
    Inserted {
        ...
        evicted: Vec<RegisterMaskedOutputNativeSequenceKey>,
    },
}

/// Borrowed cache result retaining exact disposition and loaded sequence.
#[derive(Debug)]
pub struct RegisterMaskedOutputNativeSequenceCacheEntry<'cache> {
    disposition: RegisterMaskedOutputNativeSequenceCacheDisposition,
    key: &'cache RegisterMaskedOutputNativeSequenceKey,
    sequence: &'cache LoadedRegisterMaskedOutputNativeSequence,
}

/// Internal cache inconsistency rejected before exposing a borrow.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedOutputNativeSequenceCacheInvariantError {
...
#[derive(Debug)]
pub struct RegisterMaskedOutputNativeSequenceCacheLoadReleaseFailures<E> {
    candidate: Option<RegisterMaskedOutputNativeSequenceReleaseFailure<E>>,
    eviction: Option<RegisterMaskedOutputNativeSequenceReleaseFailure<E>>,
}

/// Failure while loading, evicting, or publishing one exact cache miss.
#[derive(Debug)]
pub struct RegisterMaskedOutputNativeSequenceCacheLoadFailure<E> {
    candidate_cleanup_failure:
        Option<Box<RegisterMaskedOutputNativeSequenceReleaseFailure<E>>>,
    cause: LoadFailureCause<E>,
    evicted_keys: Vec<RegisterMaskedOutputNativeSequenceKey>,
    requested_key: RegisterMaskedOutputNativeSequenceKey,
}

/// Aggregate cleanup failure after removing cache-owned sequence entries.
#[derive(Debug)]
pub struct RegisterMaskedOutputNativeSequenceCacheReleaseFailure<E> {
    attempted_entries: usize,
    failures: Vec<RegisterMaskedOutputNativeSequenceReleaseFailure<E>>,
    released_entries: usize,
}

type CapacityError = NativeExecutableSequenceCacheCapacityError;
type CacheInvalidationResult<E> =
    RegisterMaskedOutputNativeSequenceCacheInvalidationResult<E>;
type CacheLoadResult<'cache, E> =
    RegisterMaskedOutputNativeSequenceCacheLoadResult<'cache, E>;
type CacheReleaseResult<E> =
    RegisterMaskedOutputNativeSequenceCacheReleaseResult<E>;
type CacheReconfigurationResult<E> =
    RegisterMaskedOutputNativeSequenceCacheReconfigurationResult<E>;

type CachePublicationResult<E> = Result<
    RegisterMaskedOutputNativeSequenceCacheDisposition,
    Box<RegisterMaskedOutputNativeSequenceCacheLoadFailure<E>>,
>;
type CacheFitResult<E> = Result<
    (CacheCandidate, Vec<RegisterMaskedOutputNativeSequenceKey>),
    Box<RegisterMaskedOutputNativeSequenceCacheLoadFailure<E>>,
>;
...
type SequenceReleaseFailure<E> =
    RegisterMaskedOutputNativeSequenceReleaseFailure<E>;

/// Published weighted capacity limits for this Output sequence cache.
pub type RegisterMaskedOutputNativeSequenceCacheLimits =
    NativeExecutableSequenceCacheLimits;

/// Exact resources retained by this Output sequence cache.
pub type RegisterMaskedOutputNativeSequenceCacheUsage =
    NativeExecutableSequenceCacheUsage;

/// Capacity rejection produced by this Output sequence cache.
pub type RegisterMaskedOutputNativeSequenceCacheCapacityError =
    NativeExecutableSequenceCacheCapacityError;

/// Result of loading or reusing one exact Output sequence plan.
pub type RegisterMaskedOutputNativeSequenceCacheLoadResult<'cache, E> = Result<
    RegisterMaskedOutputNativeSequenceCacheEntry<'cache>,
    Box<RegisterMaskedOutputNativeSequenceCacheLoadFailure<E>>,
>;

/// Result of invalidating one exact loaded Output sequence.
pub type RegisterMaskedOutputNativeSequenceCacheInvalidationResult<E> =
    Result<bool, Box<RegisterMaskedOutputNativeSequenceReleaseFailure<E>>>;

/// Result of releasing every sequence removed from cache authority.
pub type RegisterMaskedOutputNativeSequenceCacheReleaseResult<E> =
    Result<(), Box<RegisterMaskedOutputNativeSequenceCacheReleaseFailure<E>>>;

impl RegisterMaskedOutputNativeSequenceCacheDisposition {
    /// Returns the first exact FIFO key evicted for an inserted miss.
    #[must_use]
    pub fn evicted_key(
        &self,
    ) -> Option<&RegisterMaskedOutputNativeSequenceKey> {
        self.evicted_keys().first()
    }

    /// Returns every exact FIFO key evicted to admit this acquisition.
    #[must_use]
    pub fn evicted_keys(&self) -> &[RegisterMaskedOutputNativeSequenceKey] {
        match self {
            Self::Hit => &[],
...
impl RegisterMaskedOutputNativeSequenceCacheEntry<'_> {
    ...
    #[must_use]
    pub const fn key(&self) -> &RegisterMaskedOutputNativeSequenceKey {
        self.key
    }

    /// Returns the loaded sequence borrowed from cache authority.
    #[must_use]
    pub const fn sequence(&self) -> &LoadedRegisterMaskedOutputNativeSequence {
        self.sequence
    }
}

impl Display for RegisterMaskedOutputNativeSequenceCacheInvariantError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str(match self {
            Self::EntryMissing => "located exact Output sequence is missing",
        })
    }
}

impl<E> RegisterMaskedOutputNativeSequenceCacheLoadFailure<E> {
    /// Returns failed candidate cleanup after unsuccessful publication.
    #[must_use]
    pub const fn candidate_cleanup_failure(
        &self,
    ) -> Option<&RegisterMaskedOutputNativeSequenceReleaseFailure<E>> {
    ...
    pub const fn capacity_error(
        &self,
    ) -> Option<RegisterMaskedOutputNativeSequenceCacheCapacityError> {
        match self.cause {
    ...
    pub fn into_release_failures(
        self,
    ) -> RegisterMaskedOutputNativeSequenceCacheLoadReleaseFailures<E> {
    ...
    pub const fn invariant_error(
        &self,
    ) -> Option<RegisterMaskedOutputNativeSequenceCacheInvariantError> {
        match self.cause {
    ...
    pub const fn load_failure(
        &self,
    ) -> Option<&RegisterMaskedOutputNativeSequenceLoadFailure<E>> {
        match &self.cause {
...
impl<E: Display> Display
    for RegisterMaskedOutputNativeSequenceCacheLoadFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str("Output v6 sequence cache miss failed: ")?;
        match &self.cause {
...
impl<E> RegisterMaskedOutputNativeSequenceCacheLoadReleaseFailures<E> {
    /// Returns failed candidate cleanup after unsuccessful publication.
    #[must_use]
...
impl<E> RegisterMaskedOutputNativeSequenceCacheReleaseFailure<E> {
...
impl<E: Display> Display
    for RegisterMaskedOutputNativeSequenceCacheReleaseFailure<E>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "Output v6 sequence cache retained {} of {} entries",
            self.failed_entries(),
            self.attempted_entries,
        )
    }
...
impl RegisterMaskedOutputNativeSequenceCache {
    ...
    pub fn contains_plan(
        ...
        let key = RegisterMaskedOutputNativeSequenceKey::from_plan(plan);
        self.position(&key).is_some()
    }

    /// Loads or reuses one exact admitted Output sequence.
    ///
        ...
        Adapter: NativeExecutableMemoryAdapter,
    {
        ...
        if self.position(&key).is_some() {
            return self.entry_for_key(
                &key,
                RegisterMaskedOutputNativeSequenceCacheDisposition::Hit,
            );
        }
        let sequence =
            load_register_masked_output_native_sequence(plan, adapter)
                .map_err(|failure| {
                    Box::new(
    ...
    fn entry_for_key<'cache, E>(
        ...
        self.entries
            ...
            .map_or_else(
                ...
                |entry| {
                    Ok(RegisterMaskedOutputNativeSequenceCacheEntry {
                        disposition,
                        key: &entry.key,
    ...
    pub fn keys(
        &self,
    ) -> impl Iterator<Item = &RegisterMaskedOutputNativeSequenceKey> {
        ...
        Adapter: NativeExecutableMemoryAdapter,
    {
        ...
        Ok(
            RegisterMaskedOutputNativeSequenceCacheDisposition::Inserted {
                evicted,
            },
        )
    }
    ...
    pub fn reconfigure_limits<Adapter>(
        &mut self,
        adapter: &mut Adapter,
        requested_limits: RegisterMaskedOutputNativeSequenceCacheLimits,
    ...
    #[must_use]
    pub const fn usage(&self) -> RegisterMaskedOutputNativeSequenceCacheUsage {
        self.usage
    }
    ...
    pub const fn with_limits(
        limits: RegisterMaskedOutputNativeSequenceCacheLimits,
    ) -> Self {
        Self {
...
) -> RegisterMaskedOutputNativeSequenceCacheLoadFailure<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
{
...
const fn cache_invariant_failure<E>(
    requested_key: RegisterMaskedOutputNativeSequenceKey,
) -> RegisterMaskedOutputNativeSequenceCacheLoadFailure<E> {
    RegisterMaskedOutputNativeSequenceCacheLoadFailure {
        candidate_cleanup_failure: None,
        cause: LoadFailureCause::Invariant(
            RegisterMaskedOutputNativeSequenceCacheInvariantError::EntryMissing,
        ),
        evicted_keys: Vec::new(),
        requested_key,
    }
...
fn retry_cache_release_failures<Adapter>(
    adapter: &mut Adapter,
    pending: Vec<
        RegisterMaskedOutputNativeSequenceReleaseFailure<Adapter::Error>,
    >,
) -> CacheReleaseResult<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
