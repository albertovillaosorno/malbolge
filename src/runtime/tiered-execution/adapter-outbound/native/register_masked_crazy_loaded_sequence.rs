...
// Boundary-Contract:
// - Owns:
//   - Loaded mapping ownership and reverse cleanup for admitted Crazy
//     register-masked v6 sequence plans.
// - Must-Not:
//   - Execute native code, call runners, own lease caches, or alter plan
//     admission semantics.
// - Allows:
//   - Inputs: admitted Crazy plans and a memory adapter.
...
// - Merge-When:
//   - One reviewed Crazy sequence coordinator owns load and execution.
// - Summary:
//   - Publishes only fully loaded Crazy plans and releases in reverse.
// - Description:
...
//! Loaded ownership for admitted Crazy register-masked v6 plans.
...
use super::register_masked_crazy_sequence as sequence;
use super::register_masked_resident::{
    RegisterMaskedCrazyNativeExecutableOwner,
    RegisterMaskedCrazyNativeOwnerLoadFailure,
};

/// Indexed failure while loading one admitted Crazy v6 sequence.
#[derive(Debug, Eq, PartialEq)]
pub struct RegisterMaskedCrazyNativeSequenceLoadFailure<MemoryError> {
    cause: Box<RegisterMaskedCrazyNativeOwnerLoadFailure<MemoryError>>,
    cleanup_failure: Option<
        Box<RegisterMaskedCrazyNativeSequenceReleaseFailure<MemoryError>>,
    >,
    index: usize,
    loaded_count: usize,
}
...
#[derive(Debug, Eq, PartialEq)]
pub struct RegisterMaskedCrazyNativeSequenceReleaseFailure<MemoryError> {
    attempted_count: usize,
    failures:
        Vec<RegisterMaskedCrazyNativeExecutableReleaseFailure<MemoryError>>,
    released_count: usize,
}

/// Result of loading every mapping for one admitted Crazy v6 plan.
pub type RegisterMaskedCrazyNativeSequenceLoadResult<MemoryError> = Result<
    LoadedRegisterMaskedCrazyNativeSequence,
    Box<RegisterMaskedCrazyNativeSequenceLoadFailure<MemoryError>>,
>;

/// Result of releasing every mapping retained by a Crazy sequence.
pub type RegisterMaskedCrazyNativeSequenceReleaseResult<MemoryError> = Result<
    (),
    Box<RegisterMaskedCrazyNativeSequenceReleaseFailure<MemoryError>>,
>;

/// Fully loaded admitted Crazy v6 sequence without runner authority.
#[derive(Debug)]
pub struct LoadedRegisterMaskedCrazyNativeSequence {
    owners: Vec<RegisterMaskedCrazyNativeExecutableOwner>,
    plan: sequence::RegisterMaskedCrazyNativeSequencePlan,
}

impl<MemoryError> RegisterMaskedCrazyNativeSequenceLoadFailure<MemoryError> {
    /// Returns retryable prefix-cleanup failure, when cleanup also failed.
    #[must_use]
    pub const fn cleanup_failure(
        &self,
    ) -> Option<&RegisterMaskedCrazyNativeSequenceReleaseFailure<MemoryError>>
    {
    ...
    pub const fn owner_failure(
        &self,
    ) -> &RegisterMaskedCrazyNativeOwnerLoadFailure<MemoryError> {
        &self.cause
    }
}

impl<MemoryError: Display> Display
    for RegisterMaskedCrazyNativeSequenceLoadFailure<MemoryError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "Crazy v6 sequence load failed at step {}: {}",
...
impl<MemoryError> RegisterMaskedCrazyNativeSequenceReleaseFailure<MemoryError> {
    ...
    pub fn failures(
        &self,
    ) -> &[RegisterMaskedCrazyNativeExecutableReleaseFailure<MemoryError>] {
        &self.failures
    }
    ...
    pub fn retry<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedCrazyNativeSequenceReleaseResult<MemoryError>
    where
        Adapter: NativeExecutableMemoryAdapter<Error = MemoryError>,
    {
        retry_crazy_release_failures(adapter, self.failures)
    }
}

impl<MemoryError: Display> Display
    for RegisterMaskedCrazyNativeSequenceReleaseFailure<MemoryError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "Crazy v6 sequence release retained {} of {} mappings",
            self.failed_count(),
            self.attempted_count,
        )
    }
...
impl LoadedRegisterMaskedCrazyNativeSequence {
    ...
    #[must_use]
    pub(super) fn owners(&self) -> &[RegisterMaskedCrazyNativeExecutableOwner] {
        &self.owners
    }
    ...
    pub const fn plan(
        &self,
    ) -> &sequence::RegisterMaskedCrazyNativeSequencePlan {
        &self.plan
    }
    ...
    pub fn release<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedCrazyNativeSequenceReleaseResult<Adapter::Error>
    where
        Adapter: NativeExecutableMemoryAdapter,
    {
        release_crazy_owners(adapter, self.owners)
    }
}
...
pub fn load_register_masked_crazy_native_sequence<Adapter>(
    plan: &sequence::RegisterMaskedCrazyNativeSequencePlan,
    adapter: &mut Adapter,
) -> RegisterMaskedCrazyNativeSequenceLoadResult<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
{
    ...
    {
        match RegisterMaskedCrazyNativeExecutableOwner::load(
            ...
            Err(cause) => {
                ...
                let cleanup_failure =
                    release_crazy_owners(adapter, owners).err();
                return Err(Box::new(
    ...
    Ok(LoadedRegisterMaskedCrazyNativeSequence {
        owners,
        plan: plan.clone(),
    })
}

fn release_crazy_owners<Adapter>(
    adapter: &mut Adapter,
    owners: Vec<RegisterMaskedCrazyNativeExecutableOwner>,
) -> RegisterMaskedCrazyNativeSequenceReleaseResult<Adapter::Error>
where
...
fn retry_crazy_release_failures<Adapter>(
    adapter: &mut Adapter,
    pending: Vec<
        RegisterMaskedCrazyNativeExecutableReleaseFailure<Adapter::Error>,
    >,
) -> RegisterMaskedCrazyNativeSequenceReleaseResult<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
...
fn release_pass_result<MemoryError>(
    ...
    if failures.is_empty() {
        ...
        Err(Box::new(RegisterMaskedCrazyNativeSequenceReleaseFailure {
            attempted_count,
            failures,
            released_count,
        }))
