// Copyright:
//   - Copyright © 2026 Alberto Villa Osorno.
// SPDX-License-Identifier:
//   - MIT
// Confidential:
//   - false
// License-File:
//   - LICENSE-MIT
//
// Boundary-Contract:
// - Owns:
//   - Safe ordered admission of executable mapping lifecycle evidence.
// - Must-Not:
//   - Allocate pages, change permissions, synchronize instructions, or call
//   - machine code.
// - Allows:
//   - Inputs: verified load images and exact platform-operation reports.
//   - Outputs: staged, sealed, synchronized, and release-bound typed states.
//   - Side effects: process-local allocation only.
// - Split-When:
//   - A concrete operating-system executable-memory adapter is introduced.
// - Merge-When:
//   - One platform owner subsumes mapping operations and lifecycle admission.
// - Summary:
//   - Makes RW-to-RX-to-synchronized ordering explicit and fail closed.
// - Description:
//   - Binds every report to one mapping, image, address range, and exact key.
// - Usage:
//   - Platform adapters report completed operations; this module admits order.
// - Defaults:
//   - Any permission, identity, byte, capacity, or range drift is rejected.
//

//! Safe typestate admission for a future executable-memory platform adapter.

use std::fmt::{Display, Formatter, Result as FormatResult};
use std::num::{NonZeroU64, NonZeroUsize};

use super::loader::{
    NativeExecutablePermission, VerifiedDirectFusedLoadImage,
    VerifiedDirectLoadImage, VerifiedExecutionGeometryLoadImage,
    VerifiedRegisterMaskedCrazyLoadImage, VerifiedRegisterMaskedLoadImage,
    VerifiedRegisterMaskedNoOperationHaltLoadImage,
    VerifiedRegisterMaskedNoOperationLoadImage,
    VerifiedRegisterMaskedNoOperationPairLoadImage,
    VerifiedRegisterMaskedNoOperationRotateLoadImage,
    VerifiedRegisterMaskedNonGraphicalLoadImage,
    VerifiedRegisterMaskedOutputLoadImage,
    VerifiedRegisterMaskedRotateLoadImage,
    VerifiedRegisterMaskedRotateNoOperationLoadImage,
};
use crate::execution_cache::{NativeArtifactKey, NativeTargetIdentity};

/// Stable non-zero identity assigned by one platform mapping owner.
#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct NativeExecutableMappingId(NonZeroU64);

/// Exact observed state of one platform executable-memory mapping.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct NativeExecutableMappingReport {
    base_address: NonZeroUsize,
    mapped_len: usize,
    mapping_id: NativeExecutableMappingId,
    permissions: NativeExecutablePermission,
}

/// Exact instruction-cache synchronization report for one mapping range.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct NativeInstructionSyncReport {
    byte_len: usize,
    mapping_id: NativeExecutableMappingId,
    start_address: NonZeroUsize,
}

/// Exact mapping release request retained by a synchronized executable.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct NativeExecutableReleaseRequest {
    base_address: NonZeroUsize,
    mapped_len: usize,
    mapping_id: NativeExecutableMappingId,
}

/// Failure while admitting one ordered executable mapping transition.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum NativeExecutableLifecycleError {
    /// Address arithmetic overflowed the host address representation.
    AddressOverflow,
    /// Writable mapping bytes differ from the exact verified code image.
    CodeImage,
    /// The exact entrypoint lies outside the admitted mapping range.
    EntryRange,
    /// Mapping base address violates the target instruction alignment.
    MappingAlignment,
    /// Mapping cannot contain the complete verified code image.
    MappingCapacity,
    /// A later report refers to a different mapping identity or range.
    MappingIdentity,
    /// A lifecycle report names permissions invalid for its transition.
    Permissions,
    /// Instruction synchronization covered a different mapping byte range.
    SynchronizationRange,
}

/// Verified bytes admitted in one exact writable staging mapping.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedNativeExecutable {
    image: VerifiedDirectLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact mapping admitted after its RW-to-RX permission transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedNativeExecutable {
    image: VerifiedDirectLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping admitted after mandatory instruction synchronization.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedDirectLoadImage,
    mapping: NativeExecutableMappingReport,
}

type DirectFusedReadyResult =
    Result<ReadyDirectFusedNativeExecutable, NativeExecutableLifecycleError>;
type DirectFusedSealedResult =
    Result<SealedDirectFusedNativeExecutable, NativeExecutableLifecycleError>;

/// Exact RX mapping for one verified fused direct load image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyDirectFusedNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedDirectFusedLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Fused direct mapping admitted after its RW-to-RX permission transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedDirectFusedNativeExecutable {
    image: VerifiedDirectFusedLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified fused direct bytes admitted in one writable staging mapping.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedDirectFusedNativeExecutable {
    image: VerifiedDirectFusedLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one verified explicit-geometry load image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyExecutionGeometryNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedExecutionGeometryLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Explicit-geometry mapping admitted after its RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedExecutionGeometryNativeExecutable {
    image: VerifiedExecutionGeometryLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified explicit-geometry bytes admitted in one writable staging mapping.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedExecutionGeometryNativeExecutable {
    image: VerifiedExecutionGeometryLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one verified register-masked v6 load image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Register-masked v6 mapping admitted after its RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedNativeExecutable {
    image: VerifiedRegisterMaskedLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified register-masked v6 bytes admitted in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedNativeExecutable {
    image: VerifiedRegisterMaskedLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one register-masked v6 Crazy image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedCrazyNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedCrazyLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// V6 Crazy mapping admitted after its RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedCrazyNativeExecutable {
    image: VerifiedRegisterMaskedCrazyLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified v6 Crazy bytes admitted in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedCrazyNativeExecutable {
    image: VerifiedRegisterMaskedCrazyLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one register-masked v6 output image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedOutputNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedOutputLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// V6 output mapping admitted after its RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedOutputNativeExecutable {
    image: VerifiedRegisterMaskedOutputLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified v6 output bytes admitted in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedOutputNativeExecutable {
    image: VerifiedRegisterMaskedOutputLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one register-masked v6 no-operation image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedNoOperationNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// V6 no-operation mapping admitted after its RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedNoOperationNativeExecutable {
    image: VerifiedRegisterMaskedNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified v6 no-operation bytes admitted in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedNoOperationNativeExecutable {
    image: VerifiedRegisterMaskedNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one collapsed register-masked v6 no-op/halt image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedNoOperationHaltNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedNoOperationHaltLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Collapsed v6 no-op/halt mapping admitted after RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedNoOperationHaltNativeExecutable {
    image: VerifiedRegisterMaskedNoOperationHaltLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified collapsed v6 no-op/halt bytes in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedNoOperationHaltNativeExecutable {
    image: VerifiedRegisterMaskedNoOperationHaltLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one collapsed register-masked v6 no-operation pair
/// image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedNoOperationPairNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedNoOperationPairLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Collapsed v6 no-operation pair mapping admitted after RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedNoOperationPairNativeExecutable {
    image: VerifiedRegisterMaskedNoOperationPairLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified collapsed v6 no-operation pair bytes in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedNoOperationPairNativeExecutable {
    image: VerifiedRegisterMaskedNoOperationPairLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one collapsed register-masked v6 no-op/rotate image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedNoOperationRotateNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedNoOperationRotateLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Collapsed v6 no-op/rotate mapping admitted after RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedNoOperationRotateNativeExecutable {
    image: VerifiedRegisterMaskedNoOperationRotateLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified collapsed v6 no-op/rotate bytes in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedNoOperationRotateNativeExecutable {
    image: VerifiedRegisterMaskedNoOperationRotateLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one collapsed register-masked v6 rotate/no-op image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedRotateNoOperationNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedRotateNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Collapsed v6 rotate/no-op mapping admitted after RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedRotateNoOperationNativeExecutable {
    image: VerifiedRegisterMaskedRotateNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified collapsed v6 rotate/no-op bytes in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedRotateNoOperationNativeExecutable {
    image: VerifiedRegisterMaskedRotateNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one register-masked v6 rotate image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedRotateNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedRotateLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// V6 rotate mapping admitted after its RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedRotateNativeExecutable {
    image: VerifiedRegisterMaskedRotateLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified v6 rotate bytes admitted in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedRotateNativeExecutable {
    image: VerifiedRegisterMaskedRotateLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Exact RX mapping for one non-graphical register-masked v6 image.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReadyRegisterMaskedNonGraphicalNativeExecutable {
    entry_address: NonZeroUsize,
    image: VerifiedRegisterMaskedNonGraphicalLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Non-graphical v6 mapping admitted after its RW-to-RX transition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SealedRegisterMaskedNonGraphicalNativeExecutable {
    image: VerifiedRegisterMaskedNonGraphicalLoadImage,
    mapping: NativeExecutableMappingReport,
}

/// Verified non-graphical v6 bytes admitted in writable staging memory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct StagedRegisterMaskedNonGraphicalNativeExecutable {
    image: VerifiedRegisterMaskedNonGraphicalLoadImage,
    mapping: NativeExecutableMappingReport,
}

impl NativeExecutableMappingId {
    /// Returns the underlying non-zero platform mapping identity.
    #[must_use]
    pub const fn get(self) -> u64 {
        self.0.get()
    }

    /// Constructs one mapping identity, rejecting zero.
    #[must_use]
    pub const fn new(value: u64) -> Option<Self> {
        match NonZeroU64::new(value) {
            Some(non_zero) => Some(Self(non_zero)),
            None => None,
        }
    }
}

impl NativeExecutableMappingReport {
    /// Returns the non-zero mapping base address.
    #[must_use]
    pub const fn base_address(self) -> NonZeroUsize {
        self.base_address
    }

    /// Returns the complete mapped byte capacity.
    #[must_use]
    pub const fn mapped_len(self) -> usize {
        self.mapped_len
    }

    /// Returns the exact platform mapping identity.
    #[must_use]
    pub const fn mapping_id(self) -> NativeExecutableMappingId {
        self.mapping_id
    }

    /// Constructs one exact platform mapping report.
    #[must_use]
    pub const fn new(
        mapping_id: NativeExecutableMappingId,
        base_address: NonZeroUsize,
        mapped_len: usize,
        permissions: NativeExecutablePermission,
    ) -> Self {
        Self {
            base_address,
            mapped_len,
            mapping_id,
            permissions,
        }
    }

    /// Returns the observed page permissions.
    #[must_use]
    pub const fn permissions(self) -> NativeExecutablePermission {
        self.permissions
    }
}

impl NativeInstructionSyncReport {
    /// Returns the synchronized byte length.
    #[must_use]
    pub const fn byte_len(self) -> usize {
        self.byte_len
    }

    /// Returns the exact platform mapping identity.
    #[must_use]
    pub const fn mapping_id(self) -> NativeExecutableMappingId {
        self.mapping_id
    }

    /// Constructs one exact instruction synchronization report.
    #[must_use]
    pub const fn new(
        mapping_id: NativeExecutableMappingId,
        start_address: NonZeroUsize,
        byte_len: usize,
    ) -> Self {
        Self {
            byte_len,
            mapping_id,
            start_address,
        }
    }

    /// Returns the synchronized range start address.
    #[must_use]
    pub const fn start_address(self) -> NonZeroUsize {
        self.start_address
    }
}

impl NativeExecutableReleaseRequest {
    /// Returns the exact mapping base address to release.
    #[must_use]
    pub const fn base_address(self) -> NonZeroUsize {
        self.base_address
    }

    /// Constructs the exact release request for any admitted mapping report.
    #[must_use]
    pub const fn from_mapping(mapping: NativeExecutableMappingReport) -> Self {
        Self {
            base_address: mapping.base_address(),
            mapped_len: mapping.mapped_len(),
            mapping_id: mapping.mapping_id(),
        }
    }

    /// Returns the complete mapped byte length to release.
    #[must_use]
    pub const fn mapped_len(self) -> usize {
        self.mapped_len
    }

    /// Returns the exact mapping identity to release.
    #[must_use]
    pub const fn mapping_id(self) -> NativeExecutableMappingId {
        self.mapping_id
    }
}

impl Display for NativeExecutableLifecycleError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        f.write_str(match self {
            Self::AddressOverflow => "native executable address overflowed",
            Self::CodeImage => "native executable staging bytes differ",
            Self::EntryRange => "native executable entrypoint is out of range",
            Self::MappingAlignment => {
                "native executable mapping base is misaligned"
            },
            Self::MappingCapacity => {
                "native executable mapping capacity is insufficient"
            },
            Self::MappingIdentity => {
                "native executable mapping identity drifted"
            },
            Self::Permissions => "native executable permissions are invalid",
            Self::SynchronizationRange => {
                "native executable synchronization range drifted"
            },
        })
    }
}

impl ReadyDirectFusedNativeExecutable {
    /// Returns the non-zero fused native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified fused load image retained by this executable.
    #[must_use]
    pub const fn image(&self) -> &VerifiedDirectFusedLoadImage {
        &self.image
    }

    /// Returns the complete retained fused artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this fused mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this fused executable.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedDirectFusedNativeExecutable {
    /// Admits synchronization of the complete fused code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact fused image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> DirectFusedReadyResult {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address =
            direct_fused_entry_address(&self.image, self.mapping)?;
        Ok(ReadyDirectFusedNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified fused load image retained by this state.
    #[must_use]
    pub const fn image(&self) -> &VerifiedDirectFusedLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedDirectFusedNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same fused mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> DirectFusedSealedResult {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedDirectFusedNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified fused image retained by this state.
    #[must_use]
    pub const fn image(&self) -> &VerifiedDirectFusedLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits exact copied fused code in one writable platform mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedDirectFusedLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_direct_fused_writable_mapping(image, mapping)?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyExecutionGeometryNativeExecutable {
    /// Returns the non-zero explicit-geometry native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified v5 load image retained by this executable.
    #[must_use]
    pub const fn image(&self) -> &VerifiedExecutionGeometryLoadImage {
        &self.image
    }

    /// Returns the complete retained v5 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns an exact cleanup request for this complete mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns the exact target assumptions retained by this executable.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedExecutionGeometryNativeExecutable {
    /// Admits synchronization of the complete explicit-geometry code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact v5 image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyExecutionGeometryNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address =
            execution_geometry_entry_address(&self.image, self.mapping)?;
        Ok(ReadyExecutionGeometryNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified v5 load image retained by this state.
    #[must_use]
    pub const fn image(&self) -> &VerifiedExecutionGeometryLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedExecutionGeometryNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same v5 mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity, range,
    /// or final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedExecutionGeometryNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedExecutionGeometryNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified v5 load image retained by this state.
    #[must_use]
    pub const fn image(&self) -> &VerifiedExecutionGeometryLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits exact copied v5 code in one writable platform mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedExecutionGeometryLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_execution_geometry_writable_mapping(image, mapping)?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedNativeExecutable {
    /// Returns the non-zero register-masked v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified v6 load image retained by this executable.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedLoadImage {
        &self.image
    }

    /// Returns the complete retained v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns an exact cleanup request for this complete v6 mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this executable.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedNativeExecutable {
    /// Admits synchronization of the complete register-masked v6 code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact v6 image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address =
            register_masked_entry_address(&self.image, self.mapping)?;
        Ok(ReadyRegisterMaskedNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified v6 load image retained by this state.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same v6 mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity, range,
    /// or final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified v6 load image retained by this state.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits exact copied v6 code in one writable platform mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_writable_mapping(image, mapping)?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedCrazyNativeExecutable {
    /// Returns the non-zero Crazy v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified v6 Crazy load image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedCrazyLoadImage {
        &self.image
    }

    /// Returns the complete retained v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this lifecycle state.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedCrazyNativeExecutable {
    /// Admits synchronization of the complete v6 Crazy code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedCrazyNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address =
            register_masked_crazy_entry_address(&self.image, self.mapping)?;
        Ok(ReadyRegisterMaskedCrazyNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified v6 Crazy image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedCrazyLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedCrazyNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedCrazyNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedCrazyNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified v6 Crazy image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedCrazyLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits copied v6 Crazy code in one writable mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedCrazyLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_crazy_writable_mapping(image, mapping)?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedOutputNativeExecutable {
    /// Returns the non-zero output v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified v6 output load image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedOutputLoadImage {
        &self.image
    }

    /// Returns the complete retained v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this lifecycle state.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedOutputNativeExecutable {
    /// Admits synchronization of the complete v6 output code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedOutputNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address =
            register_masked_output_entry_address(&self.image, self.mapping)?;
        Ok(ReadyRegisterMaskedOutputNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified v6 output image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedOutputLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedOutputNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedOutputNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedOutputNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified v6 output image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedOutputLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits copied v6 output code in one writable mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedOutputLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_output_writable_mapping(image, mapping)?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedNoOperationNativeExecutable {
    /// Returns the non-zero no-operation v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified v6 no-operation load image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedNoOperationLoadImage {
        &self.image
    }

    /// Returns the complete retained v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this lifecycle state.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedNoOperationNativeExecutable {
    /// Admits synchronization of the complete v6 no-operation code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedNoOperationNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address = register_masked_no_operation_entry_address(
            &self.image,
            self.mapping,
        )?;
        Ok(ReadyRegisterMaskedNoOperationNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified v6 no-operation image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedNoOperationLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedNoOperationNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedNoOperationNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedNoOperationNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified v6 no-operation image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedNoOperationLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits copied v6 no-operation code in one writable mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedNoOperationLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_no_operation_writable_mapping(image, mapping)?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedNoOperationHaltNativeExecutable {
    /// Returns the non-zero collapsed v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified collapsed v6 load image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedNoOperationHaltLoadImage {
        &self.image
    }

    /// Returns the complete retained collapsed v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this lifecycle state.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedNoOperationHaltNativeExecutable {
    /// Admits synchronization of the complete collapsed v6 code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedNoOperationHaltNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address = register_masked_no_operation_halt_entry_address(
            &self.image,
            self.mapping,
        )?;
        Ok(ReadyRegisterMaskedNoOperationHaltNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified collapsed v6 image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedNoOperationHaltLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedNoOperationHaltNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedNoOperationHaltNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedNoOperationHaltNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified collapsed v6 image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedNoOperationHaltLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits copied collapsed v6 code in one writable mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedNoOperationHaltLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_no_operation_halt_writable_mapping(
            image, mapping,
        )?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedNoOperationPairNativeExecutable {
    /// Returns the non-zero collapsed v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified collapsed v6 load image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedNoOperationPairLoadImage {
        &self.image
    }

    /// Returns the complete retained collapsed v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this lifecycle state.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedNoOperationPairNativeExecutable {
    /// Admits synchronization of the complete collapsed v6 code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedNoOperationPairNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address = register_masked_no_operation_pair_entry_address(
            &self.image,
            self.mapping,
        )?;
        Ok(ReadyRegisterMaskedNoOperationPairNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified collapsed v6 image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedNoOperationPairLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedNoOperationPairNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedNoOperationPairNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedNoOperationPairNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified collapsed v6 image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedNoOperationPairLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits copied collapsed v6 code in one writable mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedNoOperationPairLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_no_operation_pair_writable_mapping(
            image, mapping,
        )?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedNoOperationRotateNativeExecutable {
    /// Returns the non-zero collapsed v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified collapsed v6 load image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedNoOperationRotateLoadImage {
        &self.image
    }

    /// Returns the complete retained collapsed v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this lifecycle state.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedNoOperationRotateNativeExecutable {
    /// Admits synchronization of the complete collapsed v6 code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedNoOperationRotateNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address = register_masked_no_operation_rotate_entry_address(
            &self.image,
            self.mapping,
        )?;
        Ok(ReadyRegisterMaskedNoOperationRotateNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified collapsed v6 image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedNoOperationRotateLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedNoOperationRotateNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedNoOperationRotateNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedNoOperationRotateNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified collapsed v6 image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedNoOperationRotateLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits copied collapsed v6 code in one writable mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedNoOperationRotateLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_no_operation_rotate_writable_mapping(
            image, mapping,
        )?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedRotateNoOperationNativeExecutable {
    /// Returns the non-zero collapsed v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified collapsed v6 load image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedRotateNoOperationLoadImage {
        &self.image
    }

    /// Returns the complete retained collapsed v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this lifecycle state.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedRotateNoOperationNativeExecutable {
    /// Admits synchronization of the complete collapsed v6 code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedRotateNoOperationNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address = register_masked_rotate_no_operation_entry_address(
            &self.image,
            self.mapping,
        )?;
        Ok(ReadyRegisterMaskedRotateNoOperationNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified collapsed v6 image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedRotateNoOperationLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedRotateNoOperationNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedRotateNoOperationNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedRotateNoOperationNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified collapsed v6 image.
    #[must_use]
    pub const fn image(
        &self,
    ) -> &VerifiedRegisterMaskedRotateNoOperationLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits copied collapsed v6 code in one writable mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedRotateNoOperationLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_rotate_no_operation_writable_mapping(
            image, mapping,
        )?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedRotateNativeExecutable {
    /// Returns the non-zero rotate v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified v6 rotate load image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedRotateLoadImage {
        &self.image
    }

    /// Returns the complete retained v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this lifecycle state.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedRotateNativeExecutable {
    /// Admits synchronization of the complete v6 rotate code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedRotateNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address =
            register_masked_rotate_entry_address(&self.image, self.mapping)?;
        Ok(ReadyRegisterMaskedRotateNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified v6 rotate image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedRotateLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedRotateNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedRotateNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedRotateNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified v6 rotate image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedRotateLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits copied v6 rotate code in one writable mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedRotateLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_rotate_writable_mapping(image, mapping)?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl ReadyRegisterMaskedNonGraphicalNativeExecutable {
    /// Returns the non-zero non-graphical v6 native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified non-graphical v6 load image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedNonGraphicalLoadImage {
        &self.image
    }

    /// Returns the complete retained v6 artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns exact cleanup evidence for this mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest::from_mapping(self.mapping)
    }

    /// Returns exact target assumptions retained by this lifecycle state.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

impl SealedRegisterMaskedNonGraphicalNativeExecutable {
    /// Admits synchronization of the complete non-graphical v6 code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or the
    /// synchronized range differs from the exact image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<
        ReadyRegisterMaskedNonGraphicalNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address = register_masked_non_graphical_entry_address(
            &self.image,
            self.mapping,
        )?;
        Ok(ReadyRegisterMaskedNonGraphicalNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified non-graphical v6 image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedNonGraphicalLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl StagedRegisterMaskedNonGraphicalNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when mapping identity or
    /// final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<
        SealedRegisterMaskedNonGraphicalNativeExecutable,
        NativeExecutableLifecycleError,
    > {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedRegisterMaskedNonGraphicalNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified non-graphical v6 image.
    #[must_use]
    pub const fn image(&self) -> &VerifiedRegisterMaskedNonGraphicalLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits copied non-graphical v6 code in one writable mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] for permission, bytes,
    /// alignment, capacity, address, or entry-range drift.
    pub fn stage(
        image: &VerifiedRegisterMaskedNonGraphicalLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_register_masked_non_graphical_writable_mapping(
            image, mapping,
        )?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl StagedNativeExecutable {
    /// Admits the exact RW-to-RX transition for this same mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when the mapping identity,
    /// range, or final permissions differ.
    pub fn admit_read_execute(
        self,
        mapping: NativeExecutableMappingReport,
    ) -> Result<SealedNativeExecutable, NativeExecutableLifecycleError> {
        if !same_mapping(self.mapping, mapping) {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if mapping.permissions() != self.image.policy().final_permissions() {
            return Err(NativeExecutableLifecycleError::Permissions);
        }
        Ok(SealedNativeExecutable {
            image: self.image,
            mapping,
        })
    }

    /// Returns the exact verified load image retained by this state.
    #[must_use]
    pub const fn image(&self) -> &VerifiedDirectLoadImage {
        &self.image
    }

    /// Returns the exact writable mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Admits exact copied code in one writable platform mapping.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when permissions, bytes,
    /// alignment, capacity, address arithmetic, or entry range disagree.
    pub fn stage(
        image: &VerifiedDirectLoadImage,
        mapping: NativeExecutableMappingReport,
        copied_code: &[u8],
    ) -> Result<Self, NativeExecutableLifecycleError> {
        validate_writable_mapping(image, mapping)?;
        if copied_code != image.code() {
            return Err(NativeExecutableLifecycleError::CodeImage);
        }
        Ok(Self {
            image: image.clone(),
            mapping,
        })
    }
}

impl SealedNativeExecutable {
    /// Admits exact instruction synchronization for the complete code range.
    ///
    /// # Errors
    ///
    /// Returns [`NativeExecutableLifecycleError`] when identity or synchronized
    /// range differs from the exact executable image.
    pub fn admit_instruction_sync(
        self,
        report: NativeInstructionSyncReport,
    ) -> Result<ReadyNativeExecutable, NativeExecutableLifecycleError> {
        if report.mapping_id() != self.mapping.mapping_id() {
            return Err(NativeExecutableLifecycleError::MappingIdentity);
        }
        if report.start_address() != self.mapping.base_address()
            || report.byte_len() != self.image.allocation_len()
        {
            return Err(NativeExecutableLifecycleError::SynchronizationRange);
        }
        let entry_address = entry_address(&self.image, self.mapping)?;
        Ok(ReadyNativeExecutable {
            entry_address,
            image: self.image,
            mapping: self.mapping,
        })
    }

    /// Returns the exact verified load image retained by this state.
    #[must_use]
    pub const fn image(&self) -> &VerifiedDirectLoadImage {
        &self.image
    }

    /// Returns the exact read-execute mapping report retained by this state.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }
}

impl ReadyNativeExecutable {
    /// Returns the non-zero native entrypoint address.
    #[must_use]
    pub const fn entry_address(&self) -> NonZeroUsize {
        self.entry_address
    }

    /// Returns the exact verified load image retained by this executable.
    #[must_use]
    pub const fn image(&self) -> &VerifiedDirectLoadImage {
        &self.image
    }

    /// Returns the complete retained artifact identity.
    #[must_use]
    pub const fn key(&self) -> &NativeArtifactKey {
        self.image.key()
    }

    /// Returns the exact synchronized mapping report.
    #[must_use]
    pub const fn mapping(&self) -> NativeExecutableMappingReport {
        self.mapping
    }

    /// Returns an exact future cleanup request for the complete mapping.
    #[must_use]
    pub const fn release_request(&self) -> NativeExecutableReleaseRequest {
        NativeExecutableReleaseRequest {
            base_address: self.mapping.base_address(),
            mapped_len: self.mapping.mapped_len(),
            mapping_id: self.mapping.mapping_id(),
        }
    }

    /// Returns the exact target assumptions retained by this executable.
    #[must_use]
    pub const fn target(&self) -> &NativeTargetIdentity {
        self.image.target()
    }

    /// Returns the exact selected Windows target triple.
    #[must_use]
    pub const fn target_triple(&self) -> &'static str {
        self.image.target_triple()
    }
}

fn register_masked_crazy_entry_address(
    image: &VerifiedRegisterMaskedCrazyLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn register_masked_output_entry_address(
    image: &VerifiedRegisterMaskedOutputLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn register_masked_no_operation_entry_address(
    image: &VerifiedRegisterMaskedNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn register_masked_no_operation_halt_entry_address(
    image: &VerifiedRegisterMaskedNoOperationHaltLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn register_masked_no_operation_pair_entry_address(
    image: &VerifiedRegisterMaskedNoOperationPairLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn register_masked_no_operation_rotate_entry_address(
    image: &VerifiedRegisterMaskedNoOperationRotateLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn register_masked_rotate_no_operation_entry_address(
    image: &VerifiedRegisterMaskedRotateNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn register_masked_rotate_entry_address(
    image: &VerifiedRegisterMaskedRotateLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn register_masked_non_graphical_entry_address(
    image: &VerifiedRegisterMaskedNonGraphicalLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn register_masked_entry_address(
    image: &VerifiedRegisterMaskedLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn direct_fused_entry_address(
    image: &VerifiedDirectFusedLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn execution_geometry_entry_address(
    image: &VerifiedExecutionGeometryLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

fn entry_address(
    image: &VerifiedDirectLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<NonZeroUsize, NativeExecutableLifecycleError> {
    let value = mapping
        .base_address()
        .get()
        .checked_add(image.entry_offset())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if value >= code_end {
        return Err(NativeExecutableLifecycleError::EntryRange);
    }
    NonZeroUsize::new(value).ok_or(NativeExecutableLifecycleError::EntryRange)
}

const fn is_aligned(value: usize, alignment: usize) -> bool {
    match alignment.checked_sub(1) {
        Some(mask) => value & mask == 0,
        None => false,
    }
}

const fn same_mapping(
    left: NativeExecutableMappingReport,
    right: NativeExecutableMappingReport,
) -> bool {
    left.base_address().get() == right.base_address().get()
        && left.mapped_len() == right.mapped_len()
        && left.mapping_id().get() == right.mapping_id().get()
}

pub(super) fn validate_direct_fused_writable_mapping(
    image: &VerifiedDirectFusedLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_direct_fused_mapping_ranges(image, mapping)
}

pub(super) fn validate_execution_geometry_writable_mapping(
    image: &VerifiedExecutionGeometryLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_execution_geometry_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_crazy_writable_mapping(
    image: &VerifiedRegisterMaskedCrazyLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_crazy_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_output_writable_mapping(
    image: &VerifiedRegisterMaskedOutputLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_output_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_no_operation_writable_mapping(
    image: &VerifiedRegisterMaskedNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_no_operation_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_no_operation_halt_writable_mapping(
    image: &VerifiedRegisterMaskedNoOperationHaltLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_no_operation_halt_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_no_operation_pair_writable_mapping(
    image: &VerifiedRegisterMaskedNoOperationPairLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_no_operation_pair_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_no_operation_rotate_writable_mapping(
    image: &VerifiedRegisterMaskedNoOperationRotateLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_no_operation_rotate_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_rotate_no_operation_writable_mapping(
    image: &VerifiedRegisterMaskedRotateNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_rotate_no_operation_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_rotate_writable_mapping(
    image: &VerifiedRegisterMaskedRotateLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_rotate_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_non_graphical_writable_mapping(
    image: &VerifiedRegisterMaskedNonGraphicalLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_non_graphical_mapping_ranges(image, mapping)
}

pub(super) fn validate_register_masked_writable_mapping(
    image: &VerifiedRegisterMaskedLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_register_masked_mapping_ranges(image, mapping)
}

pub(super) fn validate_writable_mapping(
    image: &VerifiedDirectLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    if mapping.permissions() != image.policy().initial_permissions() {
        return Err(NativeExecutableLifecycleError::Permissions);
    }
    if mapping.mapped_len() < image.allocation_len() {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    if !is_aligned(
        mapping.base_address().get(),
        image.minimum_instruction_alignment(),
    ) {
        return Err(NativeExecutableLifecycleError::MappingAlignment);
    }
    validate_mapping_ranges(image, mapping)
}

fn validate_direct_fused_mapping_ranges(
    image: &VerifiedDirectFusedLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry = direct_fused_entry_address(image, mapping)?;
    Ok(())
}

fn validate_execution_geometry_mapping_ranges(
    image: &VerifiedExecutionGeometryLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry = execution_geometry_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_crazy_mapping_ranges(
    image: &VerifiedRegisterMaskedCrazyLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry = register_masked_crazy_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_output_mapping_ranges(
    image: &VerifiedRegisterMaskedOutputLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry = register_masked_output_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_no_operation_mapping_ranges(
    image: &VerifiedRegisterMaskedNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry = register_masked_no_operation_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_no_operation_halt_mapping_ranges(
    image: &VerifiedRegisterMaskedNoOperationHaltLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry =
        register_masked_no_operation_halt_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_no_operation_pair_mapping_ranges(
    image: &VerifiedRegisterMaskedNoOperationPairLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry =
        register_masked_no_operation_pair_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_no_operation_rotate_mapping_ranges(
    image: &VerifiedRegisterMaskedNoOperationRotateLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry =
        register_masked_no_operation_rotate_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_rotate_no_operation_mapping_ranges(
    image: &VerifiedRegisterMaskedRotateNoOperationLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry =
        register_masked_rotate_no_operation_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_rotate_mapping_ranges(
    image: &VerifiedRegisterMaskedRotateLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry = register_masked_rotate_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_non_graphical_mapping_ranges(
    image: &VerifiedRegisterMaskedNonGraphicalLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry = register_masked_non_graphical_entry_address(image, mapping)?;
    Ok(())
}

fn validate_register_masked_mapping_ranges(
    image: &VerifiedRegisterMaskedLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry = register_masked_entry_address(image, mapping)?;
    Ok(())
}

fn validate_mapping_ranges(
    image: &VerifiedDirectLoadImage,
    mapping: NativeExecutableMappingReport,
) -> Result<(), NativeExecutableLifecycleError> {
    let mapping_end = mapping
        .base_address()
        .get()
        .checked_add(mapping.mapped_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    let code_end = mapping
        .base_address()
        .get()
        .checked_add(image.allocation_len())
        .ok_or(NativeExecutableLifecycleError::AddressOverflow)?;
    if code_end > mapping_end {
        return Err(NativeExecutableLifecycleError::MappingCapacity);
    }
    let _entry = entry_address(image, mapping)?;
    Ok(())
}
