...
// Boundary-Contract:
// - Owns:
//   - Indexed runner execution for already-loaded rotate v6 sequences.
// - Must-Not:
//   - Allocate, release, cache, or alter admitted sequence topology.
// - Allows:
//   - Inputs: loaded rotate owner, runner, rebased entry, caller buffers.
...
// - Merge-When:
//   - One reviewed rotate coordinator safely owns load and execution.
// - Summary:
//   - Executes retained rotate mappings without adapter work.
// - Description:
...
//! Indexed execution for loaded rotate register-masked v6 sequences.
...
use super::runner::RegisterMaskedRotateNativeRunner;
use super::{
    register_masked_resident as resident,
    register_masked_rotate_loaded_sequence as loaded_sequence,
};

/// Admitted result of executing one loaded rotate v6 sequence.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedRotateNativeSequenceOutcome {
...
pub struct RegisterMaskedRotateNativeSequenceExecutionFailure<RunnerError> {
    cause: Box<
        resident::RegisterMaskedRotateNativeOwnerExecutionFailure<RunnerError>,
    >,
...
/// Result of executing one already-loaded rotate v6 sequence.
pub type RegisterMaskedRotateNativeSequenceExecutionResult<RunnerError> =
    Result<
        RegisterMaskedRotateNativeSequenceOutcome,
        Box<RegisterMaskedRotateNativeSequenceExecutionFailure<RunnerError>>,
    >;

impl<RunnerError>
    RegisterMaskedRotateNativeSequenceExecutionFailure<RunnerError>
{
...
impl<RunnerError: Display> Display
    for RegisterMaskedRotateNativeSequenceExecutionFailure<RunnerError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "rotate v6 sequence step {} failed after {} steps: {}",
            self.step_index, self.completed_steps, self.cause,
        )
    }
}

impl RegisterMaskedRotateNativeSequenceOutcome {
...
pub fn execute_loaded_register_masked_rotate_native_sequence<Runner>(
    loaded: &loaded_sequence::LoadedRegisterMaskedRotateNativeSequence,
    runner: &mut Runner,
    entry: ProfileMachineObservation,
    buffers: NativeRegionBuffers<'_>,
) -> RegisterMaskedRotateNativeSequenceExecutionResult<Runner::Error>
where
    Runner: RegisterMaskedRotateNativeRunner,
{
    use RegisterMaskedRotateNativeSequenceOutcome as Outcome;
    ...
    for (index, owner) in loaded.owners().iter().enumerate() {
        let outcome = owner
            ...
            .map_err(|cause| {
                Box::new(RegisterMaskedRotateNativeSequenceExecutionFailure {
                    cause,
