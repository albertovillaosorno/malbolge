...
// Boundary-Contract:
// - Owns:
//   - Indexed runner execution for already-loaded no-operation v6 sequences.
// - Must-Not:
//   - Allocate, release, cache, or alter admitted sequence topology.
// - Allows:
//   - Inputs: loaded no-operation owner, runner, rebased entry, caller buffers.
...
// - Merge-When:
//   - One reviewed no-operation coordinator safely owns load and execution.
// - Summary:
//   - Executes retained no-operation mappings without adapter work.
// - Description:
...
//! Indexed execution for loaded no-operation register-masked v6 sequences.
...
use super::runner::RegisterMaskedNoOperationNativeRunner;
use super::{
    register_masked_no_operation_loaded_sequence as loaded_sequence,
    register_masked_resident as resident,
};

/// Admitted result of executing one loaded no-operation v6 sequence.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedNoOperationNativeSequenceOutcome {
...
pub struct RegisterMaskedNoOperationNativeSequenceExecutionFailure<RunnerError>
{
    cause: Box<
        resident::RegisterMaskedNoOperationNativeOwnerExecutionFailure<
            RunnerError,
        >,
    >,
...
/// Result of executing one already-loaded no-operation v6 sequence.
pub type RegisterMaskedNoOperationNativeSequenceExecutionResult<RunnerError> =
    Result<
...
impl<RunnerError>
    RegisterMaskedNoOperationNativeSequenceExecutionFailure<RunnerError>
{
...
impl<RunnerError: Display> Display
    for RegisterMaskedNoOperationNativeSequenceExecutionFailure<RunnerError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "no-operation v6 sequence step {} failed after {} steps: {}",
            self.step_index, self.completed_steps, self.cause,
        )
    }
}

impl RegisterMaskedNoOperationNativeSequenceOutcome {
...
pub fn execute_loaded_register_masked_no_operation_native_sequence<Runner>(
    loaded: &loaded_sequence::LoadedRegisterMaskedNoOperationNativeSequence,
    runner: &mut Runner,
    entry: ProfileMachineObservation,
    buffers: NativeRegionBuffers<'_>,
) -> RegisterMaskedNoOperationNativeSequenceExecutionResult<Runner::Error>
where
    Runner: RegisterMaskedNoOperationNativeRunner,
{
    use RegisterMaskedNoOperationNativeSequenceOutcome as Outcome;
