...
// Boundary-Contract:
// - Owns:
//   - Loaded mapping ownership and reverse cleanup for admitted no-operation
//     register-masked v6 sequence plans.
// - Must-Not:
//   - Execute native code, call runners, own lease caches, or alter plan
//     admission semantics.
// - Allows:
//   - Inputs: admitted no-operation plans and a memory adapter.
...
// - Merge-When:
//   - One reviewed no-operation sequence coordinator owns load and execution.
// - Summary:
//   - Publishes only fully loaded no-operation plans and releases in reverse.
// - Description:
...
//! Loaded ownership for admitted no-operation register-masked v6 plans.
...
use super::register_masked_no_operation_sequence as sequence;
...
/// Indexed failure while loading one admitted no-operation v6 sequence.
...
#[derive(Debug, Eq, PartialEq)]
pub struct RegisterMaskedNoOperationNativeSequenceReleaseFailure<MemoryError> {
...
/// Result of loading every mapping for one admitted no-operation v6 plan.
pub type RegisterMaskedNoOperationNativeSequenceLoadResult<MemoryError> =
    Result<
        LoadedRegisterMaskedNoOperationNativeSequence,
        Box<RegisterMaskedNoOperationNativeSequenceLoadFailure<MemoryError>>,
    >;

/// Result of releasing every mapping retained by a no-operation sequence.
...
#[derive(Debug)]
pub struct LoadedRegisterMaskedNoOperationNativeSequence {
    owners: Vec<RegisterMaskedNoOperationNativeExecutableOwner>,
    plan: sequence::RegisterMaskedNoOperationNativeSequencePlan,
}

impl<MemoryError>
    RegisterMaskedNoOperationNativeSequenceLoadFailure<MemoryError>
{
    ...
    pub const fn owner_failure(
        &self,
    ) -> &RegisterMaskedNoOperationNativeOwnerLoadFailure<MemoryError> {
        &self.cause
    }
}

impl<MemoryError: Display> Display
    for RegisterMaskedNoOperationNativeSequenceLoadFailure<MemoryError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "no-operation v6 sequence load failed at step {}: {}",
    ...
    RegisterMaskedNoOperationNativeSequenceReleaseFailure<MemoryError>
{
    ...
    pub fn retry<Adapter>(
        self,
        adapter: &mut Adapter,
    ) -> RegisterMaskedNoOperationNativeSequenceReleaseResult<MemoryError>
    where
        Adapter: NativeExecutableMemoryAdapter<Error = MemoryError>,
    {
        retry_no_operation_release_failures(adapter, self.failures)
    }
}

impl<MemoryError: Display> Display
    for RegisterMaskedNoOperationNativeSequenceReleaseFailure<MemoryError>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        write!(
            f,
            "no-operation v6 sequence release retained {} of {} mappings",
            self.failed_count(),
            self.attempted_count,
        )
    }
...
impl LoadedRegisterMaskedNoOperationNativeSequence {
    ...
    pub const fn plan(
        &self,
    ) -> &sequence::RegisterMaskedNoOperationNativeSequencePlan {
        &self.plan
    }
    ...
    {
        release_no_operation_owners(adapter, self.owners)
    }
}
...
pub fn load_register_masked_no_operation_native_sequence<Adapter>(
    plan: &sequence::RegisterMaskedNoOperationNativeSequencePlan,
    adapter: &mut Adapter,
) -> RegisterMaskedNoOperationNativeSequenceLoadResult<Adapter::Error>
where
    Adapter: NativeExecutableMemoryAdapter,
{
    ...
    {
        match RegisterMaskedNoOperationNativeExecutableOwner::load(
            ...
            Err(cause) => {
                ...
                let cleanup_failure =
                    release_no_operation_owners(adapter, owners).err();
                return Err(Box::new(
    ...
    Ok(LoadedRegisterMaskedNoOperationNativeSequence {
        owners,
        plan: plan.clone(),
    })
}

fn release_no_operation_owners<Adapter>(
    adapter: &mut Adapter,
    owners: Vec<RegisterMaskedNoOperationNativeExecutableOwner>,
) -> RegisterMaskedNoOperationNativeSequenceReleaseResult<Adapter::Error>
where
