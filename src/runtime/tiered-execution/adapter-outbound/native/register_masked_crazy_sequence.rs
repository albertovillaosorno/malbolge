...
// Boundary-Contract:
// - Owns:
//   - Exact Crazy register-masked v6 sequence plan admission.
// - Must-Not:
//   - Allocate executable mappings, call native runners, own caches, or release
//     executable memory.
// - Allows:
//   - Inputs: ordered v6 one-step programs and verified Crazy artifacts.
...
// - Summary:
//   - Admits exact Crazy v6 sequence topology before any mapping.
// - Description:
...
//! Exact plan admission for Crazy register-masked v6 sequences.
...
/// Failure while admitting one ordered Crazy v6 sequence plan.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedCrazyNativeSequencePlanError {
    ...
    /// A Crazy sequence must contain at least one semantic step.
    Empty,
...
/// Exact ordered identity for one admitted Crazy v6 sequence.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RegisterMaskedCrazyNativeSequenceKey {
    artifact_keys: Vec<NativeArtifactKey>,
}

/// Ordered Crazy v6 programs and exact verified artifacts.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RegisterMaskedCrazyNativeSequencePlan {
    artifacts: Vec<VerifiedRegisterMaskedCrazyNativeObjectArtifact>,
    entry: ProfileMachineObservation,
    exit: ProfileMachineObservation,
    programs: Vec<RegisterMaskedRegionEffectProgram>,
}

impl Display for RegisterMaskedCrazyNativeSequencePlanError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        match self {
            Self::ArtifactCount { artifacts, programs } => write!(
                f,
                "Crazy v6 sequence has {artifacts} artifacts for \
                 {programs} programs",
            ),
            Self::ArtifactIdentity { index } => write!(
                f,
                "Crazy v6 sequence artifact identity differs at step \
                 {index}",
            ),
            Self::Empty => {
                f.write_str("Crazy v6 sequence requires at least one step")
            },
            Self::Identity { index, .. } => {
                write!(f, "Crazy v6 sequence identity failed at step {index}")
            },
            Self::ObservationChain { index } => write!(
                f,
                "Crazy v6 sequence observation chain broke at step \
                 {index}",
            ),
            Self::ProfileMismatch { index } => {
                write!(f, "Crazy v6 sequence profile changed at step {index}")
            },
            Self::ProgramShape { index } => write!(
                f,
                "Crazy v6 sequence step {index} is not one complete \
                 effect",
            ),
            Self::TargetMismatch { index } => {
                write!(f, "Crazy v6 sequence target changed at step {index}")
            },
            Self::TerminationBeforeEnd { index } => write!(
                f,
                "Crazy v6 step {index} terminated before sequence end",
            ),
        }
    }
}

impl RegisterMaskedCrazyNativeSequenceKey {
    ...
    /// Derives exact ordered identity from one admitted Crazy plan.
    #[must_use]
    pub fn from_plan(plan: &RegisterMaskedCrazyNativeSequencePlan) -> Self {
        Self {
            artifact_keys: plan
                .artifacts()
                .iter()
...
impl RegisterMaskedCrazyNativeSequencePlan {
    ...
    /// authority. Reviewed Crazy artifacts each cover one semantic
    /// step. Ordered continuous steps may compose; a later position after
    /// termination fails.
    ///
    /// # Errors
    ///
    /// Returns [`RegisterMaskedCrazyNativeSequencePlanError`] for empty,
    ...
    pub fn new(
        ...
        artifacts: &[VerifiedRegisterMaskedCrazyNativeObjectArtifact],
    ) -> Result<Self, RegisterMaskedCrazyNativeSequencePlanError> {
        use RegisterMaskedCrazyNativeSequencePlanError as Error;
...
fn validate_program_chain(
    programs: &[RegisterMaskedRegionEffectProgram],
) -> Result<(), RegisterMaskedCrazyNativeSequencePlanError> {
    use RegisterMaskedCrazyNativeSequencePlanError as Error;
