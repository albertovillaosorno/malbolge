...
pub(super) fn register_masked_rotate_no_operation_code(
    ...
    push_guard_branch(&mut words, &mut guard_branches, 0x5400_0003);
    for (address, live_in) in [
        (template.entry_code_pointer, template.rotate_code_live_in),
        (template.entry_data_pointer, template.rotate_data_live_in),
        (
            template.no_operation_code_pointer,
            template.no_operation_live_in,
        ),
    ] {
        push_indexed_memory_guard(
            &mut words,
    ...
    words.push(0x3941_3009);
    push_guard_branch(&mut words, &mut guard_branches, 0x3500_0009);
    push_register_masked_rotate_no_operation_commit(&mut words, template);
    let guard_miss = words.len();
    words.extend_from_slice(&[0x5280_0020, 0xd65f_03c0]);
    patch_guard_branches(&mut words, &guard_branches, guard_miss)?;
    Some(encode_words(&words))
...
fn push_register_masked_rotate_no_operation_commit(
    ...
    for (address, value) in [
        ...
        (
            template.no_operation_encrypted_address,
            template.no_operation_encrypted_value,
        ),
    ] {
        words.extend_from_slice(&[
            movz_w10(address),
