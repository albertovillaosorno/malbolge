...
use super::{
    DirectCrazyError, DirectDeoptError, DirectExecutionGeometryCrazyError,
...
/// Promotes only guarded non-aliasing jump-data bound to exact v5 geometry.
///
/// # Errors
///
/// Returns [`DirectExecutionGeometryJumpDataError`] when v5 shape, identity,
/// target, COFF structure, or any canonical object byte differs.
pub fn verify_direct_execution_geometry_jump_data(
    artifact: &UntrustedNativeObjectArtifact,
    program: &ExecutionGeometryRegionEffectProgram,
) -> Result<
    VerifiedExecutionGeometryJumpDataNativeObjectArtifact,
...
> {
    ...
    validate_execution_geometry_jump_data_target(artifact.key().target())?;
    let expected_key = NativeArtifactKey::new_execution_geometry(
        program,
        artifact.key().target().clone(),
    )?;
    ...
    let admitted = structurally_admit_coff(artifact)?;
    let expected = execution_geometry_jump_data_coff(artifact.key(), selected)?;
    if admitted.object() != expected {
        return Err(DirectExecutionGeometryJumpDataError::ObjectBytes);
    }
    Ok(VerifiedExecutionGeometryJumpDataNativeObjectArtifact {
        artifact: admitted,
    })
}
...
/// Promotes only the canonical mask-aware v6 Crazy object.
///
/// This verifier deliberately stops before load-image or invocation authority.
///
/// # Errors
///
/// Returns the register-masked Crazy error for shape, target, identity, COFF,
/// or canonical-byte mismatch.
pub fn verify_direct_register_masked_crazy(
    artifact: &UntrustedNativeObjectArtifact,
    program: &RegisterMaskedRegionEffectProgram,
) -> Result<
    VerifiedRegisterMaskedCrazyNativeObjectArtifact,
    DirectRegisterMaskedCrazyError,
> {
    let selected = validate_register_masked_crazy_program(program)
        .map_err(|_error| DirectRegisterMaskedCrazyError::ProgramShape)?;
    validate_register_masked_crazy_target(artifact.key().target())?;
    let expected_key = NativeArtifactKey::new_register_masked(
        program,
        artifact.key().target().clone(),
    )?;
    if artifact.key() != &expected_key {
        return Err(DirectRegisterMaskedCrazyError::ProgramShape);
    }
    let admitted = structurally_admit_coff(artifact)?;
    let expected = register_masked_crazy_coff(artifact.key(), selected)?;
    if admitted.object() != expected {
...
/// # Errors
///
/// Returns the register-masked output error for shape, target, identity, COFF,
/// or canonical-byte mismatch.
pub fn verify_direct_register_masked_output(
    artifact: &UntrustedNativeObjectArtifact,
    program: &RegisterMaskedRegionEffectProgram,
) -> Result<
    VerifiedRegisterMaskedOutputNativeObjectArtifact,
    DirectRegisterMaskedOutputError,
> {
    let selected = validate_register_masked_output_program(program)
        .map_err(|_error| DirectRegisterMaskedOutputError::ProgramShape)?;
    validate_register_masked_output_target(artifact.key().target())?;
    let expected_key = NativeArtifactKey::new_register_masked(
        program,
        artifact.key().target().clone(),
    )?;
    if artifact.key() != &expected_key {
        return Err(DirectRegisterMaskedOutputError::ProgramShape);
    }
    let admitted = structurally_admit_coff(artifact)?;
    let expected = register_masked_output_coff(artifact.key(), selected)?;
    if admitted.object() != expected {
        return Err(DirectRegisterMaskedOutputError::ObjectBytes);
    }
    Ok(VerifiedRegisterMaskedOutputNativeObjectArtifact { artifact: admitted })
}
...
/// # Errors
///
/// Returns [`DirectRegisterMaskedNonGraphicalError`] for v6 shape, target,
/// identity, COFF, or canonical-byte mismatch.
pub fn verify_direct_register_masked_non_graphical(
    artifact: &UntrustedNativeObjectArtifact,
    program: &RegisterMaskedRegionEffectProgram,
) -> Result<
    VerifiedRegisterMaskedNonGraphicalNativeObjectArtifact,
...
> {
    ...
    validate_register_masked_non_graphical_target(artifact.key().target())?;
    let expected_key = NativeArtifactKey::new_register_masked(
        program,
        artifact.key().target().clone(),
    )?;
    ...
    let admitted = structurally_admit_coff(artifact)?;
    let expected =
        register_masked_non_graphical_coff(artifact.key(), selected)?;
    if admitted.object() != expected {
        return Err(DirectRegisterMaskedNonGraphicalError::ObjectBytes);
    }
    Ok(VerifiedRegisterMaskedNonGraphicalNativeObjectArtifact {
        artifact: admitted,
    })
}
