...
// Boundary-Contract:
// - Owns:
//   - Exact rotate register-masked v6 sequence plan admission.
// - Must-Not:
//   - Allocate executable mappings, call native runners, own caches, or release
//     executable memory.
// - Allows:
//   - Inputs: ordered v6 one-step programs and verified rotate artifacts.
...
// - Summary:
//   - Admits exact rotate v6 sequence topology before any mapping.
// - Description:
...
//! Exact plan admission for rotate register-masked v6 sequences.
...
use super::direct::VerifiedRegisterMaskedRotateNativeObjectArtifact;
use crate::execution_cache::{NativeArtifactKey, NativeIdentityError};

/// Failure while admitting one ordered rotate v6 sequence plan.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegisterMaskedRotateNativeSequencePlanError {
    ...
    /// A rotate sequence must contain at least one semantic step.
    Empty,
...
/// Exact ordered identity for one admitted rotate v6 sequence.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RegisterMaskedRotateNativeSequenceKey {
    artifact_keys: Vec<NativeArtifactKey>,
}

/// Ordered rotate v6 programs and exact verified artifacts.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RegisterMaskedRotateNativeSequencePlan {
    artifacts: Vec<VerifiedRegisterMaskedRotateNativeObjectArtifact>,
    entry: ProfileMachineObservation,
    exit: ProfileMachineObservation,
    programs: Vec<RegisterMaskedRegionEffectProgram>,
}

impl Display for RegisterMaskedRotateNativeSequencePlanError {
    fn fmt(&self, f: &mut Formatter<'_>) -> FormatResult {
        match self {
            Self::ArtifactCount { artifacts, programs } => write!(
                f,
                "rotate v6 sequence has {artifacts} artifacts for \
                 {programs} programs",
            ),
            Self::ArtifactIdentity { index } => write!(
                f,
                "rotate v6 sequence artifact identity differs at step \
                 {index}",
            ),
            Self::Empty => {
                f.write_str("rotate v6 sequence requires at least one step")
            },
            Self::Identity { index, .. } => {
                write!(f, "rotate v6 sequence identity failed at step {index}")
            },
            Self::ObservationChain { index } => write!(
                f,
                "rotate v6 sequence observation chain broke at step \
                 {index}",
            ),
            Self::ProfileMismatch { index } => {
                write!(f, "rotate v6 sequence profile changed at step {index}")
            },
            Self::ProgramShape { index } => write!(
                f,
                "rotate v6 sequence step {index} is not one complete \
                 effect",
            ),
...
impl RegisterMaskedRotateNativeSequenceKey {
    ...
    /// Derives exact ordered identity from one admitted rotate plan.
    #[must_use]
    pub fn from_plan(plan: &RegisterMaskedRotateNativeSequencePlan) -> Self {
        Self {
            artifact_keys: plan
                .artifacts()
                .iter()
...
impl RegisterMaskedRotateNativeSequencePlan {
    ...
    /// authority. Reviewed rotate artifacts each cover one semantic
    /// step. Ordered continuous steps may compose; a later position after
    /// termination fails.
    ///
    /// # Errors
    ///
    /// Returns [`RegisterMaskedRotateNativeSequencePlanError`] for empty,
    ...
    pub fn new(
        ...
        artifacts: &[VerifiedRegisterMaskedRotateNativeObjectArtifact],
    ) -> Result<Self, RegisterMaskedRotateNativeSequencePlanError> {
        use RegisterMaskedRotateNativeSequencePlanError as Error;
...
fn validate_program_chain(
    programs: &[RegisterMaskedRegionEffectProgram],
) -> Result<(), RegisterMaskedRotateNativeSequencePlanError> {
    use RegisterMaskedRotateNativeSequencePlanError as Error;
