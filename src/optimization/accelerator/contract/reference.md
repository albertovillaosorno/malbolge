...
state through bounded resident execution.

`ProfileRunGeometry` plus
...
15-trit/14,348,907-word profile executes through the same geometry-bound CUDA
kernel model. `accelerator/cpu/` remains the mandatory scalar primitive reference;
CUDA results are checked against normative Rust execution.
...
safe-Rust fallback. Retained RTX 4060 N14 current-at-capture evidence includes
device-side shared-state replication and persistent scalable sessions. Complete-snapshot
