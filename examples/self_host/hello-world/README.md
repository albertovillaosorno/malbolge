...
The CLI recognizes the exact function-like token `__malbolge_output_byte` outside
comments and literals, then links `cli/adapters/guest/output.c` only into the
...
This directory does not contain a generated `main.malbolge` yet. Native execution
is scaffolding, not C-to-Malbolge compilation or guest-runtime evidence. A future
