...
The classic loader (`vm/src/loader.rs`) and profile-driven loader
(`vm/src/profile_machine.rs`) use the same position-dependent decode model.
...
The grammar currently follows the canonical Rust loader in `vm/src/loader.rs`.
That loader uses `u8::is_ascii_whitespace()`, which excludes vertical tab
`0x0B`.

The independent C VM in `vm/c/malbolge_vm.c` currently accepts `0x0B` as
