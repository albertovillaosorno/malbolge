...
# Boundary-Contract:
# - Owns:
#   - Dense rank/unrank evidence for four-component K5 edge values under the
#     residual V4=S2xS2 stabilizer of two disjoint equal vertex-pair blocks.
# - Must-Not:
...
# - Summary:
#   - Densely rank residual widened V4 K5 edge orbits.
# - Description:
...
# - Defaults:
#   - Direct V4 orbit exhaustion stops at mass two; arithmetic reaches 14.
#

"""Dense residual V4 ranking for widened K5 edge assignments."""
...
_COMPONENTS_PER_EDGE = 4
_EDGE_COUNT = 10
_EXHAUSTIVE_MASS = 2
_MAXIMUM_MASS = 14
_WIDTH_FOURTEEN_COUNT = 601_406_812_712
...
# X is core[8] plus two column-margin edge values[4+4].
# Y is an unordered pair of row bundles[12+12] after the first S2 quotient.
    ...
    if len(left) == _SIDE_COMPONENTS and len(right) == _SIDE_COMPONENTS:
        ...
        if left != right:
            ...
            if left_mass < right_mass:
                ...
                population = _composition_count(left_mass, _SIDE_COMPONENTS)
                result = (
                    prefix
...
def _x_nonfixed_rank(
    ...
    rank = prefix + core_rank * _side_nonfixed_count(side_mass) + side_rank
    return rank, flipped
...
def _z_fixed_count(total: int) -> int:
    return sum(
        _composition_count(margin_mass, _SIDE_COMPONENTS)
        * _composition_count((total - margin_mass) // 2, _SIDE_COMPONENTS)
...
def _z_fixed_rank(vector: _Vector) -> int | None:
    if len(vector) != _ROW_COMPONENTS or _t(vector) != vector:
        return None
    margin = vector[0:_SIDE_COMPONENTS]
    cell = vector[_SIDE_COMPONENTS : 2 * _SIDE_COMPONENTS]
    ...
    prefix = sum(
        _composition_count(mass, _SIDE_COMPONENTS)
        * _composition_count((total - mass) // 2, _SIDE_COMPONENTS)
        for mass in range(margin_mass)
        if (total - mass) % 2 == 0
    )
    return (
        prefix
        + margin_rank * _composition_count(cell_mass, _SIDE_COMPONENTS)
        + cell_rank
    )
...
def _z_fixed_unrank(total: int, rank: int) -> _Vector | None:
    ...
    for margin_mass in range(total + 1):
        ...
        margin_count = _composition_count(margin_mass, _SIDE_COMPONENTS)
        cell_count = _composition_count(cell_mass, _SIDE_COMPONENTS)
        block = margin_count * cell_count
        if remaining >= block:
            remaining -= block
            continue
        margin_rank, cell_rank = divmod(remaining, cell_count)
        margin = _composition_unrank(margin_mass, _SIDE_COMPONENTS, margin_rank)
        cell = _composition_unrank(cell_mass, _SIDE_COMPONENTS, cell_rank)
...
def _z_nonfixed_rank(vector: _Vector) -> tuple[int, bool] | None:
    if len(vector) != _ROW_COMPONENTS or any(value < 0 for value in vector):
        return None
    margin = vector[0:_SIDE_COMPONENTS]
    sides = _canonical_nonfixed_sides(
        vector[_SIDE_COMPONENTS : 2 * _SIDE_COMPONENTS],
        vector[2 * _SIDE_COMPONENTS : 3 * _SIDE_COMPONENTS],
    )
    if sides is None:
        return None
    ...
    rank = prefix + margin_key[1] * _side_nonfixed_count(side_mass) + sides[2]
    return rank, sides[3]
...
def _extract(edge_values: _EdgeValues) -> _StateParts:
    return (
        _flatten(edge_values, _CORE_EDGES),
        _flatten(edge_values, (_COLUMN_EDGES[0],)),
        _flatten(edge_values, (_COLUMN_EDGES[1],)),
        _flatten(edge_values, _ROW_ZERO_EDGES),
        _flatten(edge_values, _ROW_ONE_EDGES),
    )
...
def _rank(edge_values: _EdgeValues) -> int | None:
    ...
    return _rank_moving_x_state(parts, x_mass, y_mass, prefix=prefix)
...
def _assign_values(
    ...
    values = tuple(
        tuple(vector[index : index + _COMPONENTS_PER_EDGE])
        for index in range(0, len(vector), _COMPONENTS_PER_EDGE)
    )
    for edge, value in zip(edges, values, strict=True):
        assert len(value) == _COMPONENTS_PER_EDGE
        result[_EDGE_INDEX[edge]] = (value[0], value[1], value[2], value[3])


def _build(parts: _StateParts) -> _EdgeValues:
    result: list[_EdgeValue | None] = [None] * _EDGE_COUNT
    _assign_values(result, _CORE_EDGES, parts[0])
    _assign_values(result, (_COLUMN_EDGES[0],), parts[1])
    _assign_values(result, (_COLUMN_EDGES[1],), parts[2])
    _assign_values(result, _ROW_ZERO_EDGES, parts[3])
    _assign_values(result, _ROW_ONE_EDGES, parts[4])
...
def _edge_values_from_vector(vector: _Vector) -> _EdgeValues:
    values = tuple(
        tuple(vector[index : index + _COMPONENTS_PER_EDGE])
        for index in range(0, 40, _COMPONENTS_PER_EDGE)
    )
    assert all(len(value) == _COMPONENTS_PER_EDGE for value in values)
...
def _burnside_count(total: int) -> int:
    ...
    single = _fixed_scalar_count((1,) * 16 + (2,) * 12, total)
    double = _fixed_scalar_count((1,) * 8 + (2,) * 16, total)
    return (identity + 2 * single + double) // 4


def test_s6_s5_v4_dense_rank_matches_direct_small_orbits() -> None:
...
def test_s6_s5_v4_dense_rank_roundtrips_through_mass_fourteen() -> None:
...
def test_s6_s5_v4_block_factorization_matches_nested_fixed_count() -> None:
    """Diagonal X/Y blocks reproduce the independently proved V4 total."""
    observed = tuple(_v4_count(total) for total in range(_MAXIMUM_MASS + 1))
