...
#     stabilizer is three-pair V4 on three paired endpoint orbits.
# - Must-Not:
...
#   - Outputs: dense exact-three-pair-V4 pair-graph ranks.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Canonical blocks target the three-pair V4 conjugacy class.
# - Description:
#   - Interval Mobius coefficients isolate exact three-pair V4 stabilizers.
# - Usage:
...
"""Dense exact-three-pair-V4 rank for pair-valued K6 edge graphs."""
...
_FIXED_COMPONENTS = 2
_EXPECTED_TRIPLE_BLOCK_SIZES = (2, 2, 2, 2, 2, 2, 4, 4)
...
def _contains_transposition(group: Subgroup) -> bool:
    return any(
        sum(PERMUTATIONS[element][vertex] == vertex for vertex in range(_ARITY))
        == _ARITY - 2
...
_TARGET_SUBGROUPS = tuple(
    ...
    and _contains_transposition(group)
)
...
def test_pair_three_pair_v4_counts_match_independent_sequence() -> None:
    """Target-Mobius blocks reproduce exact three-pair V4 pair counts."""
...
def test_pair_three_pair_v4_rank_exhausts_masses_three_through_five() -> None:
    """Every exact three-pair V4 pair rank through mass five roundtrips."""
...
def test_pair_three_pair_v4_rank_roundtrips_through_fourteen() -> None:
    """Exact three-pair V4 pair ranks invert through mass fourteen."""
...
def test_pair_three_pair_v4_triple_geometry_and_counts() -> None:
...
def test_pair_three_pair_v4_branch_rank_roundtrips_through_fourteen() -> None:
