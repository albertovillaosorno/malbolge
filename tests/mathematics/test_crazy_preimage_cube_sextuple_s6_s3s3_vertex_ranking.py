...
# Boundary-Contract:
# - Owns:
#   - Dense rank/unrank for the S6 top-level vertex partition (3,3).
# - Must-Not:
#   - Claim ranking for another S6 Young stabilizer.
# - Allows:
#   - Inputs: sorted (3,3) vertex pairs and 52 residual scalar counts.
#   - Outputs: dense full-stratum rank under the residual S3-times-S3 action.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Rank nine invariant residual blocks through a thirty-six-element chain.
# - Description:
#   - Canonical block orbits update the remaining subgroup and suffix counts.
# - Usage:
#   - Completes the 28,825,612,500-class mass-fourteen (3,3) stratum.
# - Defaults:
#   - Direct residual orbit exhaustion stops at mass two; ranks reach fourteen.
#

"""Dense stabilizer-chain rank for the S6 (3,3) Young stratum."""
...
_VERTEX_PARTITION = (3, 3)
_RESIDUAL_COMPONENTS = 52
_WIDTH_FOURTEEN_RESIDUAL = 1_699_875_471_048
_WIDTH_FOURTEEN_COUNT = 28_825_612_500
...
_GROUP_BLOCKS = ((0, 1, 2), (3, 4, 5))


def _as_permutation(block_orders: tuple[tuple[int, ...], ...]) -> _Permutation:
    order = list(range(_ARITY))
    for block, block_order in zip(_GROUP_BLOCKS, block_orders, strict=True):
...
_S3_S3 = tuple(
    _as_permutation(block_orders)
    for block_orders in product(
        tuple(permutations(_GROUP_BLOCKS[0])),
        tuple(permutations(_GROUP_BLOCKS[1])),
    )
)
_IDENTITY_GROUP = 1
_ALL_GROUP = (1 << len(_S3_S3)) - 1
...
def _label_orbits() -> tuple[tuple[int, ...], ...]:
    ...
    while unseen:
        seed = min(unseen)
        orbit = tuple(
            sorted({_permuted_symbol(seed, order) for order in _S3_S3})
        )
        unseen -= set(orbit)
        result.append(orbit)
...
def _group_elements(group: int) -> tuple[int, ...]:
    return tuple(index for index in range(len(_S3_S3)) if group & (1 << index))
...
def _block_mapping(block: int, element: int) -> tuple[int, ...]:
    ...
    return tuple(
        index[_permuted_symbol(label, _S3_S3[element])] for label in labels
    )
...
def _fixed_count_from_cycles(total: int, cycles: tuple[int, ...]) -> int:
...
def _suffix_count(block: int, total: int, group: int) -> int:
    ...
    elements = _group_elements(group)
    fixed = sum(
        _fixed_count_from_cycles(total, _remaining_cycles(block, element))
...
def test_s6_s3s3_label_blocks_have_exact_orbit_geometry() -> None:
    """Residual labels have the reviewed fixed/pair/quad/octet geometry."""
    assert (4, *(3,) * 4, *(9,) * 4) == _BLOCK_SIZES
    assert sum(_BLOCK_SIZES) == _RESIDUAL_COMPONENTS


def test_s6_s3s3_residual_rank_matches_direct_small_orbits() -> None:
    """Dense residual ranks agree with direct S3xS3 orbits through mass two."""
...
def test_s6_s3s3_residual_counts_match_burnside_sequence() -> None:
    """Stabilizer-chain suffix counts reproduce the exact S3xS3 sequence."""
    observed = tuple(
...
def test_s6_s3s3_rank_exhausts_small_full_strata() -> None:
    """Complete (3,3) ranks form one interval through total mass six."""
...
def test_s6_s3s3_rank_roundtrips_through_fourteen() -> None:
    """Boundary and interior (3,3) ranks roundtrip through mass fourteen."""
    observed = {
