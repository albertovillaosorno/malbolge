...
# Boundary-Contract:
# - Owns:
#   - Dense normalizer-free and exact rank/unrank for the transitive order-60
#     stabilizer in the top-level all-equal S6 residual stratum.
# - Must-Not:
#   - Claim dense rank for another exact full-S6 stabilizer type.
# - Allows:
#   - Inputs: H-fixed residual assignments for the transitive order-60 H.
...
# - Usage:
#   - Constructive rank for the transitive order-60 endpoint type.
# - Defaults:
#   - Exact residual and complete rank/unrank reach total mass fourteen.
#

"""Dense exact rank for the full-S6 transitive order-60 stabilizer."""
...
_EXHAUSTIVE_MASS = 14
_H_ORDER = 60
_NORMALIZER_ORDER = 120
_QUOTIENT_ORDER = 2
_EXPECTED_QUOTIENT_SUBGROUPS = 2
_EXPECTED_H_ORBIT_SIZES = {1: 2, 10: 2, 15: 2}
_EXPECTED_BLOCK_SIZES = (2, 2, 2)
_EXPECTED_BLOCK_WEIGHTS = (1, 15, 10)
...
_EXPECTED_COMPLETE_COUNTS = _EXPECTED_EXACT_COUNTS
...
_SECOND_GENERATOR: _Permutation = (0, 2, 1, 4, 3, 5)
...
def test_transitive_order60_normalizer_geometry_is_exact() -> None:
    """H has an order-two quotient and three weighted rank blocks."""
    orbit_sizes = {
...
def test_transitive_order60_free_rank_matches_reviewed_counts() -> None:
...
def test_transitive_order60_minimal_extensions_match_exact_exclusions() -> None:
...
def test_transitive_order60_exact_rank_roundtrips_through_fourteen() -> None:
...
def test_transitive_order60_exact_samples_have_stabilizer_sixty() -> None:
    """Sampled retained representatives have exact S6 stabilizer H."""
    for total in (10, 11, 12, _MAXIMUM_MASS):
        count = _exact_count(total)
...
def test_transitive_order60_complete_rank_roundtrips_through_fourteen() -> None:
