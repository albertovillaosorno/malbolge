...
# Boundary-Contract:
# - Owns:
#   - Dense normalizer-free and exact rank/unrank for the V4-(4,2)
...
# - Summary:
#   - Rank the order-four normalizer quotient and skip 217 exact extensions.
# - Description:
#   - Exact extensions are transitive S3 stabilizers.
# - Usage:
#   - Constructive rank for the V4 endpoint-orbit-(4,2) type.
# - Defaults:
#   - Exact residual and complete rank/unrank reach total mass fourteen.
#

"""Dense exact rank for the full-S6 V4-(4,2) stabilizer."""
...
_EXTENSION_ORDER = 12
_EXPECTED_H_ORBIT_SIZES = {1: 4, 2: 8, 4: 8}
_EXPECTED_BLOCK_SIZES = (4, 2, 2, 2, 2, 2, 2, 2, 2)
_EXPECTED_BLOCK_WEIGHTS = (1, 2, 4, 2, 2, 2, 4, 4, 4)
...
def _exception_groups() -> tuple[_Group, ...]:
    ...
    for element in _S6:
        ...
        if group & normalizer != _H:
            continue
        if len(group) == _EXTENSION_ORDER and _vertex_orbit_sizes(group) == (
            _ARITY,
        ):
            result.add(group)
...
def test_v4_42_normalizer_geometry_is_exact() -> None:
...
def test_v4_42_free_rank_matches_reviewed_counts() -> None:
...
def test_v4_42_exceptions_are_one_order12_orbit() -> None:
    """All exceptions are one normalizer orbit of transitive order-12 groups."""
...
def test_v4_42_exact_rank_roundtrips_through_fourteen() -> None:
    """Sparse order-12 deletion gives one dense exact-H residual interval."""
    derived = tuple(
        free - excluded
...
def test_v4_42_exact_samples_have_stabilizer_four() -> None:
...
def test_v4_42_complete_rank_roundtrips_through_fourteen() -> None:
