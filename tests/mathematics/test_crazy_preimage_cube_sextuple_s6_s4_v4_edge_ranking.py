...
# Boundary-Contract:
# - Owns:
#   - Dense rank/unrank for four-component K4 edges under the nested S6 V4.
# - Must-Not:
#   - Claim complete (4,1,1) S6 ranking or rank other second-layer strata.
# - Allows:
#   - Inputs: four-component K4 edge assignments of mass zero through fourteen.
#   - Outputs: exact dense V4 edge-orbit rank/unrank.
#   - Side effects: none.
# - Split-When:
#   - The spoke stabilizer is not conjugate to the 2+2 Young subgroup.
# - Merge-When:
#   - Complete nested (4,1,1) S6 ranking owns this V4 edge quotient.
# - Summary:
...
# - Usage:
#   - Constructive edge core for the nested (2,2) second-layer S6 stratum.
# - Defaults:
#   - Direct orbit exhaustion stops at mass three; arithmetic reaches 14.
#

"""Dense V4 ranking for four-component K4 edges in the nested S6 stratum."""
...
_MAXIMUM_MASS = 14
_PAIR_COMPONENTS = 4
_WIDTH_FOURTEEN_COUNT = 1_528_935_120
...
_B = (0, 1, 3, 2)
_AB = (1, 0, 3, 2)
_t = cast("Callable[[_Row], _Row]", itemgetter(4, 5, 6, 7, 0, 1, 2, 3))
...
type _Row = tuple[int, int, int, int, int, int, int, int]
...
def _rows(total: int) -> tuple[_Row, ...]:
    return tuple(
        ...
        for v in _weak_compositions(total, _COMPONENTS)
    )
...
def _flatten(
    ...
    values = tuple(
        value
        for edge in edges
        for value in edge_pairs[_EDGE_INDEX[edge]]
    )
    assert len(values) == _COMPONENTS
...
def _as_edge(value: tuple[int, ...]) -> _EdgeValue:
...
def _assign(
    ...
    values = (
        _as_edge(row[:_PAIR_COMPONENTS]),
        _as_edge(row[_PAIR_COMPONENTS:]),
    )
    for edge, value in zip(edges, values, strict=True):
        result[_EDGE_INDEX[edge]] = value
...
def _edge_pairs(vector: _Vector) -> _EdgePairs:
    values = tuple(
        _as_edge(vector[index : index + _PAIR_COMPONENTS])
        for index in range(0, _PAIR_COMPONENTS * _EDGE_COUNT, _PAIR_COMPONENTS)
    )
