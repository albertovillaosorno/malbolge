...
# Boundary-Contract:
# - Owns:
#   - Dense rank/unrank for the seven non-transposition symmetric S5 strata.
# - Must-Not:
#   - Re-rank the single-transposition or trivial-stabilizer strata.
# - Allows:
#   - Inputs: pair-valued K5 edges with seven exact stabilizers through mass 14.
...
# - Usage:
#   - Completes dense ranking for all nontrivial S5 stabilizer exceptions.
# - Defaults:
#   - Direct S5 orbit exhaustion stops at mass three; arithmetic reaches 14.
#

"""Dense ranking for the seven smaller exact-S5 stabilizer strata."""
...
_EXPECTED_MASS_FOURTEEN = {
    (2, (2, 2, 1), (1, 1, 2, 2, 2, 2)): (33_360, 21_920),
    (4, (4, 1), (2, 2, 2, 4)): (306, 194),
    (4, (2, 2, 1), (1, 1, 2, 2, 4)): (10_752, 10_466),
    (6, (3, 1, 1), (1, 3, 3, 3)): (582, 402),
    (8, (4, 1), (2, 4, 4)): (112, 106),
    (12, (3, 2), (1, 3, 6)): (174, 174),
    (24, (4, 1), (4, 6)): (6, 6),
}
...
_GROUPS = (
    _generated(_DOUBLE_01_23),
    _generated(_DOUBLE_01_23, _DOUBLE_02_13),
    _generated(_SWAP_01, _SWAP_23),
...
def _states_from(
    ...
    for first in range(remaining // weight + 1):
        for second in range((remaining - weight * first) // weight + 1):
            remainder = remaining - weight * (first + second)
            result.extend(
                ((first, second), *suffix)
                for suffix in _states_from(weights, index + 1, remainder)
            )
    return tuple(result)
...
def _edge_pairs(key: _Key, state: _OrbitState) -> _EdgePairs:
    group = dict(_representatives())[key]
...
def _permute(edge_pairs: _EdgePairs, order: _Permutation) -> _EdgePairs:
    ...
    return tuple(edge_pairs[permutation[edge]] for edge in range(_EDGE_COUNT))


def _stabilizer(edge_pairs: _EdgePairs) -> _Group:
    return frozenset(
        order for order in _S5 if _permute(edge_pairs, order) == edge_pairs
    )
...
def _accepted_states(key: _Key, total: int) -> tuple[_OrbitState, ...]:
    ...
    return tuple(
        ...
        if _stabilizer(_edge_pairs(key, state)) == group
    )
...
def test_remaining_exception_keys_are_the_seven_smaller_types() -> None:
    """The generic rank owns the seven populated non-transposition types."""
    observed = {key for key, _ in _representatives()}
    assert observed == set(_EXPECTED_MASS_FOURTEEN)


def test_remaining_exception_mass_fourteen_counts() -> None:
...
def test_remaining_exception_rank_matches_direct_small_s5_orbits() -> None:
    """Each accepted state represents one direct S5 orbit through mass three."""
    for key, group in _representatives():
        for total in range(_EXHAUSTIVE_MASS + 1):
            ...
            for rank, state in enumerate(_accepted_states(key, total)):
                edge_pairs = _edge_pairs(key, state)
                assert _stabilizer(edge_pairs) == group
                representative = min(
                    _permute(edge_pairs, order) for order in _S5
                )
...
def test_remaining_exception_rank_exhausts_small_domains() -> None:
...
def test_remaining_exception_rank_roundtrips_through_mass_fourteen() -> None:
    """Boundary and interior ranks roundtrip for all seven types through 14."""
    for key, _ in _representatives():
