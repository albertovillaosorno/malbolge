...
#     stabilizer is one transposition.
# - Must-Not:
#   - Claim that the remaining weight-three labels have already broken H.
# - Allows:
#   - Inputs: fifteen two-component K6 edge values through mass fourteen.
#   - Outputs: dense exact-transposition pair-graph ranks.
#   - Side effects: none.
# - Split-When:
#   - Triple-breaking under the retained transposition becomes constructive.
# - Merge-When:
#   - A generic exact-stabilizer pair-graph rank owns this specialization.
# - Summary:
#   - Canonical blocks target the transposition conjugacy class.
# - Description:
#   - Interval Mobius coefficients isolate exact transposition stabilizers.
# - Usage:
#   - Supplies the dominant pair-symmetric branch for later triple breaking.
# - Defaults:
#   - Exhaustive ranks reach pair mass six; sampled ranks reach fourteen.
#

"""Dense exact-transposition rank for pair-valued K6 edge graphs."""
...
_TRANSPOSED_TRIPLE_PAIRS = 6
_FIXED_TRIPLES = 8
_PAIRED_TRIPLE_COORDINATES = 6
_FIXED_COMPONENTS = 2
...
type _TripleLayout = tuple[tuple[int, ...], tuple[tuple[int, int], ...]]
type _TripleParts = tuple[_Vector, _Vector, _Vector]
...
_TRANSPOSTION_VERTEX_ORBITS = (2, 1, 1, 1, 1)
_TARGET_ORDER = 2
_TARGET_SUBGROUPS = tuple(
    group
    for group in _SUBGROUPS
    if len(group) == _TARGET_ORDER
    and _vertex_orbit_sizes(group) == _TRANSPOSTION_VERTEX_ORBITS
)
...
def _triple_fixed_count(total: int) -> int:
    return sum(
        comb(total - 2 * paired + _FIXED_TRIPLES - 1, _FIXED_TRIPLES - 1)
        * comb(
            paired + _TRANSPOSED_TRIPLE_PAIRS - 1, _TRANSPOSED_TRIPLE_PAIRS - 1
        )
        for paired in range(total // 2 + 1)
    )


def _triple_free_count(total: int) -> int:
    all_assignments = comb(
        total + _TRIPLE_COORDINATES - 1, _TRIPLE_COORDINATES - 1
    )
    fixed = _triple_fixed_count(total)
    assert (all_assignments - fixed) % _TARGET_ORDER == 0
    return (all_assignments - fixed) // _TARGET_ORDER
...
@cache
def _bounded_vectors(total: int, parts: int) -> tuple[_Vector, ...]:
    if parts == 1:
        return tuple((value,) for value in range(total + 1))
    return tuple(
        (first, *suffix)
        for first in range(total + 1)
        for suffix in _bounded_vectors(total - first, parts - 1)
    )


def _paired_triple_count(total: int) -> int:
    all_assignments = _weak_count(total, 2 * _PAIRED_TRIPLE_COORDINATES)
    fixed = (
        _weak_count(total // 2, _PAIRED_TRIPLE_COORDINATES)
        if total % 2 == 0
        else 0
    )
    assert (all_assignments - fixed) % _TARGET_ORDER == 0
...
@cache
def _pair_vector_entries(total: int) -> tuple[tuple[_Vector, int, int], ...]:
    prefix = 0
    result: list[tuple[_Vector, int, int]] = []
    for left in _bounded_vectors(total, _PAIRED_TRIPLE_COORDINATES):
        right_mass = total - sum(left)
        rights = _weak_vectors(right_mass, _PAIRED_TRIPLE_COORDINATES)
        cut = bisect_right(rights, left)
        count = len(rights) - cut
        if count != 0:
            result.append((left, prefix, cut))
            prefix += count
    assert prefix == _paired_triple_count(total)
    return tuple(result)


@cache
def _pair_vector_entry_map(total: int) -> dict[_Vector, tuple[int, int]]:
    return {
        left: (prefix, cut) for left, prefix, cut in _pair_vector_entries(total)
    }


def _canonical_pair_vectors(
    left: _Vector, right: _Vector
) -> tuple[_Vector, _Vector] | None:
    result: tuple[_Vector, _Vector] | None = None
    valid_lengths = len(left) == _PAIRED_TRIPLE_COORDINATES and len(
        right
    ) == len(left)
    if (
        valid_lengths
        ...
        result = (right, left) if right < left else (left, right)
    return result


def _pair_vector_local_rank(left: _Vector, right: _Vector) -> int | None:
    entry = _pair_vector_entry_map(sum(left) + sum(right)).get(left)
    rights = _weak_vectors(sum(right), _PAIRED_TRIPLE_COORDINATES)
    result: int | None = None
    if entry is not None:
        prefix, cut = entry
        position = bisect_right(rights, right) - 1
        if position >= cut and rights[position] == right:
            result = prefix + position - cut
    return result


def _paired_triple_rank(left: _Vector, right: _Vector) -> int | None:
    canonical = _canonical_pair_vectors(left, right)
    return None if canonical is None else _pair_vector_local_rank(*canonical)


def _paired_triple_unrank(
    total: int, rank: int
) -> tuple[_Vector, _Vector] | None:
    if rank < 0 or rank >= _paired_triple_count(total):
        return None
    for left, prefix, cut in _pair_vector_entries(total):
        rights = _weak_vectors(total - sum(left), _PAIRED_TRIPLE_COORDINATES)
        count = len(rights) - cut
        if rank >= prefix + count:
            continue
        return left, rights[cut + rank - prefix]
    raise AssertionError


@cache
def _triple_layout(group: Subgroup) -> _TripleLayout:
    assert len(group) == _TARGET_ORDER
    generator = next(element for element in group if element != 0)
    mapping = _TRIPLE_PERMUTATIONS[generator]
    fixed = tuple(
        index for index, image in enumerate(mapping) if image == index
    )
    pairs = tuple(
        (index, image) for index, image in enumerate(mapping) if index < image
    )
    assert len(fixed) == _FIXED_TRIPLES
    assert len(pairs) == _PAIRED_TRIPLE_COORDINATES
    return fixed, pairs


def _split_triples(
    vector: _Vector, group: Subgroup
) -> tuple[_Vector, _Vector, _Vector] | None:
    if len(vector) != _TRIPLE_COORDINATES or any(value < 0 for value in vector):
        return None
    fixed_indices, pairs = _triple_layout(group)
    fixed = tuple(vector[index] for index in fixed_indices)
    left = tuple(vector[first] for first, _ in pairs)
    right = tuple(vector[second] for _, second in pairs)
    return fixed, left, right


def _build_triples(parts: _TripleParts, group: Subgroup) -> _Vector:
    fixed, left, right = parts
    fixed_indices, pairs = _triple_layout(group)
    result = [0] * _TRIPLE_COORDINATES
    for index, value in zip(fixed_indices, fixed, strict=True):
        result[index] = value
    for (first, second), left_value, right_value in zip(
        pairs, left, right, strict=True
    ):
        result[first] = left_value
        result[second] = right_value
    return tuple(result)


def _triple_rank(total: int, vector: _Vector, group: Subgroup) -> int | None:
    split = _split_triples(vector, group)
    if split is None or sum(vector) != total:
        return None
    fixed, left, right = split
    paired_rank = _paired_triple_rank(left, right)
    if paired_rank is None:
        return None
    fixed_mass = sum(fixed)
    paired_mass = total - fixed_mass
    prefix = sum(
        _weak_count(earlier, _FIXED_TRIPLES)
        * _paired_triple_count(total - earlier)
        for earlier in range(fixed_mass)
    )
    return (
        prefix
        + _weak_rank(fixed) * _paired_triple_count(paired_mass)
        + paired_rank
    )


def _triple_unrank(total: int, rank: int, group: Subgroup) -> _Vector | None:
    if rank < 0 or rank >= _triple_free_count(total):
        return None
    remaining = rank
    for fixed_mass in range(total + 1):
        paired_mass = total - fixed_mass
        paired_count = _paired_triple_count(paired_mass)
        block = _weak_count(fixed_mass, _FIXED_TRIPLES) * paired_count
        if remaining >= block:
            remaining -= block
            continue
        fixed_rank, paired_rank = divmod(remaining, paired_count)
        fixed = _weak_unrank(fixed_mass, _FIXED_TRIPLES, fixed_rank)
        pair = _paired_triple_unrank(paired_mass, paired_rank)
        assert fixed is not None
        assert pair is not None
        return _build_triples((fixed, pair[0], pair[1]), group)
    raise AssertionError
...
def test_pair_transposition_counts_match_independent_lattice_sequence() -> None:
    """Target-Mobius blocks reproduce exact transposition pair counts."""
...
def test_pair_transposition_rank_exhausts_masses_three_through_five() -> None:
    """Every exact-transposition pair rank through mass five roundtrips."""
...
def test_pair_transposition_rank_roundtrips_through_fourteen() -> None:
    """Exact-transposition pair ranks invert through mass fourteen."""
...
def test_pair_transposition_triple_breaking_count_matches_branch() -> None:
    """Free triples over transposition pair graphs give the branch count."""
    observed = tuple(_residual_branch_count(total) for total in range(15))
    assert observed == _EXPECTED_RESIDUAL_BRANCH_COUNTS


def test_transposition_triple_rank_matches_free_counts() -> None:
    """Eight fixed triples plus six swapped pairs rank every free H orbit."""
    group = _TARGET_SUBGROUPS[0]
    for total in range(_MAXIMUM_MASS + 1):
        observed = sum(
            _weak_count(fixed_mass, _FIXED_TRIPLES)
            * _paired_triple_count(total - fixed_mass)
            for fixed_mass in range(total + 1)
        )
        assert observed == _triple_free_count(total)
        for rank in _sample_ranks(observed):
            vector = _triple_unrank(total, rank, group)
            assert vector is not None
            assert _triple_rank(total, vector, group) == rank


def test_transposition_branch_rank_roundtrips_through_fourteen() -> None:
    """Pair, triple-breaking, and fixed-scalar ranks compose densely."""
    observed = tuple(_residual_branch_count(total) for total in range(15))
    ...
    for total in range(15):
        for rank in _sample_ranks(observed[total]):
