...
# Boundary-Contract:
# - Owns:
#   - Dense normalizer-free and exact rank/unrank for the triple-transposition
...
# - Usage:
#   - Constructive rank for the fixed-point-free involution type.
# - Defaults:
#   - Exact residual and complete rank/unrank reach total mass fourteen.
#

"""Dense exact rank for the full-S6 triple-transposition stabilizer."""
...
_QUOTIENT_ORDER = 24
_EXPECTED_QUOTIENT_SUBGROUPS = 30
_EXPECTED_EXCEPTION_GROUPS = 4
_S3_ORDER = 6
_EXPECTED_H_ORBIT_SIZES = {1: 8, 2: 22}
_EXPECTED_BLOCK_SIZES = (2, 6, 6, 6, 4, 3, 3)
_EXPECTED_BLOCK_WEIGHTS = (1, 2, 2, 2, 2, 1, 1)
...
_TRIPLE_SWAP: _Permutation = (1, 0, 3, 2, 5, 4)
_H_GENERATORS = (_TRIPLE_SWAP,)
...
def _exception_groups() -> tuple[_Group, ...]:
    result: set[_Group] = set()
    normalizer = frozenset(_NORMALIZER)
    for element in _S6:
        group = _generated((*_H_GENERATORS, element))
        if group & normalizer != _H:
            continue
        if len(group) == _S3_ORDER and _vertex_orbit_sizes(group) == (_ARITY,):
            result.add(group)
    return tuple(sorted(result, key=tuple))


_EXCEPTION_GROUPS = _exception_groups()
...
def _exception_ranks(total: int) -> tuple[int, ...]:
    ranks: set[int] = set()
    for vector in _exact_group_vectors(_EXCEPTION_GROUPS[0], total):
...
def test_triple_transposition_normalizer_geometry_is_exact() -> None:
    """H has order-24 quotient, thirty subgroups, and seven rank blocks."""
    orbit_sizes = {
...
def test_triple_transposition_free_rank_matches_reviewed_counts() -> None:
...
def test_triple_transposition_exceptions_are_one_s3_orbit() -> None:
    """All nonnormalizing exceptions are one normalizer orbit of S3 groups."""
    assert len(_EXCEPTION_GROUPS) == _EXPECTED_EXCEPTION_GROUPS
    conjugates = {
        _conjugate_group(_EXCEPTION_GROUPS[0], normalizer)
        for normalizer in _NORMALIZER
    }
    assert conjugates == set(_EXCEPTION_GROUPS)
    assert (
        tuple(
            free - exact
    ...
    exclusions = _exception_ranks(_MAXIMUM_MASS)
    assert len(exclusions) == _EXPECTED_EXCEPTIONS[-1]


def test_triple_transposition_exact_rank_roundtrips_through_fourteen() -> None:
    """Sparse S3 deletion gives one dense exact-H residual interval."""
    derived = tuple(
        free - excluded
...
def test_triple_transposition_exact_samples_have_stabilizer_two() -> None:
...
def test_triple_transposition_complete_rank_roundtrips_through_fourteen() -> (
    None
):
