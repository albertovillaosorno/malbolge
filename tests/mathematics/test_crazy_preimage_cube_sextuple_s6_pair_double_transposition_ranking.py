...
#     stabilizer is one double transposition.
# - Must-Not:
...
#   - Outputs: dense exact-double-transposition pair-graph ranks.
#   - Side effects: none.
# - Split-When:
#   - Triple-breaking under this retained involution becomes constructive.
# - Merge-When:
#   - A generic exact-stabilizer pair-graph rank owns this specialization.
# - Summary:
#   - Canonical blocks target the double-transposition conjugacy class.
# - Description:
#   - Interval Mobius coefficients isolate this exact involution type.
# - Usage:
#   - Supplies the double-transposition pair branch and triple breaking.
# - Defaults:
#   - Exhaustive ranks reach pair mass six; sampled ranks reach fourteen.
#

"""Dense exact-double-transposition rank for pair-valued K6 edge graphs."""
...
_TRANSPOSED_TRIPLE_PAIRS = 8
_FIXED_TRIPLES = 4
_PAIRED_TRIPLE_COORDINATES = 8
_FIXED_COMPONENTS = 2
...
def test_pair_double_transposition_counts_match_lattice() -> None:
...
def test_pair_double_transposition_rank_exhausts_small_masses() -> None:
    """Exact double-transposition pair ranks roundtrip through mass five."""
...
def test_pair_double_transposition_rank_roundtrips_through_fourteen() -> None:
    """Exact double-transposition pair ranks invert through mass fourteen."""
...
def test_pair_double_transposition_triple_count_matches_branch() -> None:
    """Free triples over double-transposition pairs give the branch count."""
    observed = tuple(_residual_branch_count(total) for total in range(15))
    assert observed == _EXPECTED_RESIDUAL_BRANCH_COUNTS


def test_double_transposition_triple_rank_matches_free_counts() -> None:
...
def test_double_transposition_branch_rank_roundtrips_through_fourteen() -> None:
