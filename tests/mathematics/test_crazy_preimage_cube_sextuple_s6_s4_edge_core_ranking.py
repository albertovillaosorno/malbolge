...
# Boundary-Contract:
# - Owns:
#   - Dense rank/unrank evidence for four-component K4 edges under full S4.
# - Must-Not:
#   - Claim the complete nested (4,1,1) S6 rank or any other S6 stratum.
# - Allows:
...
# - Merge-When:
#   - Complete nested (4,1,1) S6 ranking owns this widened full-S4 edge core.
# - Summary:
#   - Rank full-S4 four-component K4 edge orbits by opposite-edge blocks.
# - Description:
...
# - Usage:
#   - Final edge-core prerequisite for dense nested (4,1,1) S6 ranking.
# - Defaults:
#   - Direct full-S4 orbit exhaustion stops at mass three.
#

"""Dense full-S4 ranking for four-component K4 edge assignments."""
...
_EDGE_COMPONENTS = 4
_PAIR_COMPONENTS = 2
_WIDTH_FOURTEEN_COUNT = 255_543_816
_EDGES = tuple(
    (left, right)
    for left in _ACTIVE
...
_t = cast("Callable[[_Block], _Block]", itemgetter(4, 5, 6, 7, 0, 1, 2, 3))
...
def _fixed_representatives(total: int) -> tuple[_Block, ...]:
    ...
    return tuple(
        ...
        for value in _weak_compositions(half, _EDGE_COMPONENTS)
    )
...
def _as_edge(values: tuple[int, ...]) -> _EdgeValue:
    assert len(values) == _EDGE_COMPONENTS
...
def _edges_from_blocks(blocks: _Blocks) -> _EdgePairs:
    ...
    for block, (left_edge, right_edge) in zip(blocks, _OPPOSITE, strict=True):
        result[_EDGE_INDEX[left_edge]] = _as_edge(block[:_EDGE_COMPONENTS])
        result[_EDGE_INDEX[right_edge]] = _as_edge(block[_EDGE_COMPONENTS:])
...
def _edge_pairs_from_vector(vector: _Vector) -> _EdgePairs:
    values = tuple(
        _as_edge(vector[index : index + _EDGE_COMPONENTS])
        for index in range(
            0,
            _EDGE_COMPONENTS * _EDGE_COUNT,
            _EDGE_COMPONENTS,
        )
    )
...
def test_s4_edge_core_rank_matches_direct_small_orbits() -> None:
    """Small widened K4 assignments collapse to one contiguous S4 rank."""
    for total in range(_EXHAUSTIVE_MASS + 1):
        observed: dict[int, set[_EdgePairs]] = {}
        for vector in _weak_compositions(total, _EDGE_COMPONENTS * _EDGE_COUNT):
