...
# Boundary-Contract:
# - Owns:
#   - Dense rank/unrank for the all-distinct second-layer (4,1,1) S6 slice.
# - Must-Not:
#   - Claim ranking for nontrivial second-layer S4 stabilizers.
# - Allows:
#   - Inputs: sextuple mass zero through fourteen in this nested trivial
#     stratum.
#   - Outputs: dense ranks after unique ordering of four six-component bundles.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Distinct vertex bundles orient S4, leaving 24 labeled edge scalars.
# - Description:
#   - Prefix bundle masses and strict combinadics, then rank edge compositions.
# - Usage:
#   - Largest constructive second-layer slice of the (4,1,1) S6 stratum.
# - Defaults:
#   - Full rank exhaustion stops at mass seven; boundary checks reach mass 14.
#

"""Dense all-distinct second-layer ranking for the S6 (4,1,1) stratum."""
...
_WIDTH_FOURTEEN_COUNT = 167_523_430_983
_EXPECTED_COUNTS = {
    5: 20,
    6: 1_010,
    7: 25_440,
...
type _Bundles = tuple[_Vector, _Vector, _Vector, _Vector]
...
def _bundle_block_count(masses: tuple[int, int, int, int]) -> int:
    result = 1
...
def _bundle_count(total: int) -> int:
    return sum(
        _bundle_block_count(masses) for masses in _mass_quadruples(total)
    )
...
def _ordered_bundle_data(
    ...
    if len(set(ordered)) != _ACTIVE_COUNT:
        return None
    masses = ordered[0][0], ordered[1][0], ordered[2][0], ordered[3][0]
    ranks = ordered[0][1], ordered[1][1], ordered[2][1], ordered[3][1]
    return masses, ranks
...
def _vector_to_residual(vector: _Vector) -> _Residual:
    ...
    bundles = tuple(
        vector[
            cursor + index * _VERTEX_COMPONENTS :
            cursor + (index + 1) * _VERTEX_COMPONENTS
        ]
    ...
    b0, b1, b2, b3 = bundles
    return fixed, (b0, b1, b2, b3), edges
...
def _permute_residual(state: _Residual, order: tuple[int, ...]) -> _Residual:
    ...
    permuted_edges = _as_edges(
        tuple(
            edges[
                _K4_EDGE_INDEX[
                    _ordered_edge(inverse[left], inverse[right])
                ]
            ]
            for left, right in _K4_EDGES
        )
...
def test_s6_s4_distinct_bundle_rank_matches_small_s4_orbits() -> None:
    """The unique bundle order makes the abstract S4 residual rank invariant."""
...
def test_s6_s4_distinct_bundle_rank_exhausts_small_full_domains() -> None:
...
def test_s6_s4_distinct_bundle_rank_roundtrips_through_fourteen() -> None:
...
def test_s6_s4_distinct_bundle_counts_match_reviewed_sequence() -> None:
    """Mass-five-through-fourteen counts match the nested factorization."""
    observed = {
