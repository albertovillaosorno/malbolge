...
#     stabilizer is transitive-order12 on three paired endpoint orbits.
# - Must-Not:
...
#   - Outputs: dense exact-transitive-order12 pair-graph ranks.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Canonical blocks target the transitive-order12 conjugacy class.
# - Description:
#   - Interval Mobius coefficients isolate exact transitive-order12 stabilizers.
# - Usage:
...
"""Dense exact-transitive-order12 rank for pair-valued K6 edge graphs."""
...
if TYPE_CHECKING:
    from tests.mathematics.s6_subgroup_lattice import Permutation
...
_FIXED_COMPONENTS = 2
_EXPECTED_TRIPLE_BLOCK_SIZES = (12, 6, 2)
_EXPECTED_TARGET_CONJUGATES = 60
_EXPECTED_TARGET_SIX_CYCLES = 2
_HARD_BLOCK_SIZE = 12
...
def _compose_elements(left: int, right: int) -> int:
    permutation = cast(
        "Permutation",
        tuple(
            PERMUTATIONS[left][PERMUTATIONS[right][vertex]]
            for vertex in range(_ARITY)
        ),
    )
    return _PERMUTATION_INDEX[permutation]


@cache
def _inverse_element(element: int) -> int:
    order = PERMUTATIONS[element]
    inverse = [0] * _ARITY
    for source, destination in enumerate(order):
        inverse[destination] = source
    return _PERMUTATION_INDEX[cast("Permutation", tuple(inverse))]


def _conjugate_group(group: Subgroup, element: int) -> Subgroup:
    inverse = _inverse_element(element)
    return frozenset(
        _compose_elements(_compose_elements(element, member), inverse)
        for member in group
    )
...
_TARGET_VERTEX_ORBITS = (6,)
_TARGET_ORDER = 12


def _is_six_cycle(element: int) -> bool:
    order = PERMUTATIONS[element]
    seen: set[int] = set()
    lengths: list[int] = []
    for start in range(_ARITY):
        if start in seen:
            continue
        vertex = start
        length = 0
    ...
    return tuple(sorted(lengths, reverse=True)) == (6,)


def _six_cycle_count(group: Subgroup) -> int:
    return sum(_is_six_cycle(element) for element in group)
...
_TARGET_SUBGROUPS = tuple(
    ...
    and _six_cycle_count(group) == _EXPECTED_TARGET_SIX_CYCLES
)
...
@cache
def _reference_transporter(group: Subgroup) -> tuple[int, int]:
    if group == _REFERENCE_TARGET:
        return 0, 0
    for element in range(_GROUP_ORDER):
        if _conjugate_group(_REFERENCE_TARGET, element) == group:
            return element, _inverse_element(element)
    raise AssertionError


def _transport_triples(vector: _Vector, element: int) -> _Vector:
    mapping = _TRIPLE_PERMUTATIONS[element]
    return tuple(vector[mapping[index]] for index in range(_TRIPLE_COORDINATES))
...
@cache
def _ordered_compositions(total: int) -> tuple[tuple[int, ...], ...]:
    if total == 0:
        return ((),)
    return tuple(
        (first, *tail)
        for first in range(1, total + 1)
        for tail in _ordered_compositions(total - first)
    )


@cache
def _hard_mask_mapping(block: _TripleBlock, element: int) -> tuple[int, ...]:
    positions = {coordinate: index for index, coordinate in enumerate(block)}
    return tuple(
        positions[_TRIPLE_PERMUTATIONS[element][coordinate]]
        for coordinate in block
    )


def _hard_map_mask(block: _TripleBlock, element: int, mask: int) -> int:
    result = 0
    for source, destination in enumerate(_hard_mask_mapping(block, element)):
        if mask & (1 << source):
...
@cache
def _hard_subset_masks(mask: int, size: int) -> tuple[int, ...]:
    bits = tuple(
        index for index in range(_HARD_BLOCK_SIZE) if mask & (1 << index)
    )
    return tuple(
        sum(1 << index for index in choice)
        for choice in combinations(bits, size)
    )


@cache
def _hard_canonical_blocks(
    block: _TripleBlock,
    remaining: int,
    *,
    size: int,
    ...
    unseen = set(_hard_subset_masks(remaining, size))
    result: list[int] = []
    while unseen:
        seed = min(unseen)
        orbit = {_hard_map_mask(block, element, seed) for element in group}
        result.append(min(orbit))
        unseen -= orbit
    return tuple(sorted(result))


def _hard_block_stabilizer(
    block: _TripleBlock, mask: int, group: Subgroup
) -> Subgroup:
    return frozenset(
        element
        for element in group
        if _hard_map_mask(block, element, mask) == mask
    )


@cache
def _hard_patterns_from(
    block: _TripleBlock,
    weights: tuple[int, ...],
    remaining: int,
    *,
    group: Subgroup,
) -> tuple[tuple[tuple[int, ...], Subgroup], ...]:
    if not weights:
        return (((), group),) if remaining == 0 else ()
    result: list[tuple[tuple[int, ...], Subgroup]] = []
    for mask in _hard_canonical_blocks(
        block, remaining, size=weights[0], group=group
    ):
        stabilizer = _hard_block_stabilizer(block, mask, group)
        for suffix, final_group in _hard_patterns_from(
            block,
            weights[1:],
            remaining ^ mask,
            group=stabilizer,
        ):
            result.append(((mask, *suffix), final_group))
    return tuple(result)


@cache
def _hard_scalar_sequences(
    weights: tuple[int, ...], total: int, minimum: int = 0
) -> tuple[tuple[int, ...], ...]:
    if not weights:
        return ((),) if total == 0 else ()
    weight = weights[0]
    return tuple(
        (value, *suffix)
        for value in range(minimum, total // weight + 1)
        for suffix in _hard_scalar_sequences(
            weights[1:], total - weight * value, value + 1
        )
    )


def _hard_vector_from_pattern(
    pattern: tuple[int, ...], values: tuple[int, ...]
) -> _Vector:
    result = [0] * _HARD_BLOCK_SIZE
    for mask, value in zip(pattern, values, strict=True):
        for index in range(_HARD_BLOCK_SIZE):
            if mask & (1 << index):
                result[index] = value
    return tuple(result)
...
@cache
def _triple_hard_classes(mass: int) -> tuple[_TripleLocalClass, ...]:
    block = _triple_blocks(_REFERENCE_TARGET)[0]
    assert len(block) == _HARD_BLOCK_SIZE
    full_mask = (1 << _HARD_BLOCK_SIZE) - 1
    rows: dict[_Vector, Subgroup] = {}
    for weights in _ordered_compositions(_HARD_BLOCK_SIZE):
        values = _hard_scalar_sequences(weights, mass)
        if not values:
            continue
        patterns = _hard_patterns_from(
            block, weights, full_mask, group=_REFERENCE_TARGET
        )
        for pattern, _ in patterns:
            for colors in values:
                raw = _hard_vector_from_pattern(pattern, colors)
                representative = min(
                    _triple_transform(raw, block, element)
                    for element in _REFERENCE_TARGET
                )
                rows[representative] = _triple_stabilizer(
                    representative, block, _REFERENCE_TARGET
                )
...
def _triple_local_classes(
    ...
    if target == _REFERENCE_TARGET and step == 0 and group == target:
        return _triple_hard_classes(mass)
...
def _triple_rank_for_pair_group(
    total: int, vector: _Vector, group: Subgroup
) -> int | None:
    transporter, _ = _reference_transporter(group)
    reference = _transport_triples(vector, transporter)
    return _triple_rank(total, reference, _REFERENCE_TARGET)


def _triple_unrank_for_pair_group(
    total: int, rank: int, group: Subgroup
) -> _Vector | None:
    _, inverse = _reference_transporter(group)
    reference = _triple_unrank(total, rank, _REFERENCE_TARGET)
    return None if reference is None else _transport_triples(reference, inverse)
...
def _residual_branch_rank_data(
    ...
    if valid and pair_rank is not None:
        triple_rank = _triple_rank_for_pair_group(masses[2], triples, group)
...
def _residual_branch_unrank_block(
    ...
    triples = _triple_unrank_for_pair_group(
        triple_mass, triple_rank, _stabilizer_group(pairs)
    )
...
def test_pair_transitive_order12_counts_match_independent_sequence() -> None:
    """Target-Mobius blocks reproduce exact transitive-order12 pair counts."""
    ...
    assert all(
        _six_cycle_count(group) == _EXPECTED_TARGET_SIX_CYCLES
        for group in _TARGET_SUBGROUPS
    )


def test_pair_transitive_order12_rank_exhausts_masses_three_through_six() -> (
    None
):
    """Every exact transitive-order12 pair rank through mass six roundtrips."""
...
def test_pair_transitive_order12_rank_roundtrips_through_fourteen() -> None:
    """Exact transitive-order12 pair ranks invert through mass fourteen."""
...
def test_pair_transitive_order12_triple_geometry_and_counts() -> None:
    """Three H-invariant triple blocks give the reviewed free quotient."""
...
def test_pair_transitive_order12_branch_rank_roundtrips_through_fourteen() -> (
    None
):
    """Pair, free-H triple, fixed, and vertex ranks compose densely."""
