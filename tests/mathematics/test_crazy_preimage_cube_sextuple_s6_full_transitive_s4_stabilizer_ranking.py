...
# Boundary-Contract:
# - Owns:
#   - Dense normalizer-free and exact rank/unrank for the transitive S4
#     stabilizer in the top-level all-equal S6 residual stratum.
# - Must-Not:
#   - Claim dense rank for another exact full-S6 stabilizer type.
# - Allows:
#   - Inputs: H-fixed residual assignments for the transitive S4 H.
...
# - Usage:
#   - Constructive rank for the transitive S4 endpoint type.
# - Defaults:
#   - Exact residual and complete rank/unrank reach total mass fourteen.
#

"""Dense exact rank for the full-S6 transitive S4 stabilizer."""
...
_EXPECTED_ELEMENT_ORDER_COUNTS = {1: 1, 2: 9, 3: 8, 4: 6}
_EXPECTED_CENTER_ORDER = 1
_EXPECTED_H_ORBIT_SIZES = {1: 2, 3: 2, 4: 2, 12: 3}
_EXPECTED_BLOCK_SIZES = (2, 2, 3, 2)
_EXPECTED_BLOCK_WEIGHTS = (1, 3, 12, 4)
_EXPECTED_FREE_COUNTS = (
    0, 0, 0, 0, 1, 2, 3, 6, 10, 14, 21, 30, 41, 56, 74,
)
_EXPECTED_EXACT_COUNTS = _EXPECTED_FREE_COUNTS
_EXPECTED_EXCEPTIONS = (0,) * (_MAXIMUM_MASS + 1)
_EXPECTED_COMPLETE_COUNTS = (
    0, 0, 0, 0, 1, 2, 3, 6, 10, 14, 23, 34, 47, 68, 94,
)
...
_THIRD_GENERATOR: _Permutation = (1, 0, 2, 3, 5, 4)
_FOURTH_GENERATOR: _Permutation = (2, 3, 0, 1, 4, 5)
_H_GENERATORS = (
    _FIRST_GENERATOR,
    _SECOND_GENERATOR,
    _THIRD_GENERATOR,
...
def test_transitive_s4_normalizer_geometry_is_exact() -> None:
    """H has the S4 order profile, quotient two, and four rank blocks."""
    orbit_sizes = {
        size: sum(len(orbit) == size for orbit in _H_ORBITS)
        for size in (1, 3, 4, 12)
    }
    element_order_counts = {
        order: sum(len(_generated((element,))) == order for element in _H)
        for order in _EXPECTED_ELEMENT_ORDER_COUNTS
    }
    center_order = sum(
        all(
            _compose(element, other) == _compose(other, element)
            for other in _H
        )
        for element in _H
    )
    assert element_order_counts == _EXPECTED_ELEMENT_ORDER_COUNTS
    assert center_order == _EXPECTED_CENTER_ORDER
...
def test_transitive_s4_free_rank_matches_reviewed_counts() -> None:
...
def test_transitive_s4_minimal_extensions_match_exact_exclusions() -> None:
...
def test_transitive_s4_exact_rank_roundtrips_through_fourteen() -> None:
...
def test_transitive_s4_exact_samples_have_stabilizer_twenty_four() -> None:
    """Sampled retained representatives have exact S6 stabilizer H."""
    for total in (4, 8, 12, _MAXIMUM_MASS):
        count = _exact_count(total)
...
def test_transitive_s4_complete_rank_roundtrips_through_fourteen() -> None:
