...
# Boundary-Contract:
# - Owns:
#   - Dense H-fixed and exact rank/unrank for the transitive order-12
#     stabilizer in the top-level all-equal S6 residual stratum.
# - Must-Not:
#   - Claim dense rank for another exact full-S6 stabilizer type.
# - Allows:
#   - Inputs: H-fixed residual assignments for the populated transitive
#     order-12 H.
#   - Outputs: free N(H)/H ranks, exact-H ranks, and complete outer ranks.
#   - Side effects: none.
# - Split-When:
#   - A strict extension family needs its own reusable rank.
# - Merge-When:
#   - Complete dense full-S6 ranking owns every exact stabilizer stratum.
# - Summary:
#   - Rank a self-normalizing transitive order-12 residual stratum.
# - Description:
#   - Minimal strict extensions are deleted from the H-fixed interval.
# - Usage:
#   - Constructive rank for the populated transitive order-12 endpoint type.
# - Defaults:
#   - Exact residual and complete rank/unrank reach total mass fourteen.
#

"""Dense exact rank for the full-S6 transitive order-12 stabilizer."""
...
_EXPECTED_H_ORBIT_SIZES = {1: 2, 2: 1, 3: 2, 6: 5, 12: 1}
_EXPECTED_BLOCK_SIZES = (2, 1, 2, 5, 1)
_EXPECTED_BLOCK_WEIGHTS = (1, 2, 3, 6, 12)
_EXPECTED_FREE_COUNTS = (
    1, 2, 4, 8, 13, 20, 36, 54, 81, 124, 176, 244, 357, 486, 660,
)
_EXPECTED_EXACT_COUNTS = (
    0, 0, 0, 0, 0, 2, 7, 14, 26, 48, 76, 116, 187, 272, 393,
)
_EXPECTED_EXCEPTIONS = (
    1, 2, 4, 8, 13, 18, 29, 40, 55, 76, 100, 128, 170, 214, 267,
)
_EXPECTED_COMPLETE_COUNTS = (
    0, 0, 0, 0, 0, 2, 7, 14, 26, 48, 76, 120, 201, 300, 445,
)
...
_SECOND_GENERATOR: _Permutation = (1, 0, 4, 5, 2, 3)
_THIRD_GENERATOR: _Permutation = (2, 5, 0, 3, 4, 1)
_H_GENERATORS = (_FIRST_GENERATOR, _SECOND_GENERATOR, _THIRD_GENERATOR)
...
def test_transitive_order12_normalizer_geometry_is_exact() -> None:
    """H is self-normalizing with five weighted rank blocks."""
    orbit_sizes = {
...
def test_transitive_order12_free_rank_matches_reviewed_counts() -> None:
...
def test_transitive_order12_minimal_extensions_match_exact_exclusions() -> None:
...
def test_transitive_order12_exact_rank_roundtrips_through_fourteen() -> None:
...
def test_transitive_order12_exact_samples_have_stabilizer_twelve() -> None:
    """Sampled retained representatives have exact S6 stabilizer H."""
    for total in (5, 6, 7, _MAXIMUM_MASS):
        count = _exact_count(total)
...
def test_transitive_order12_complete_rank_roundtrips_through_fourteen() -> None:
