...
#     stabilizer is transitive-order72 on three paired endpoint orbits.
# - Must-Not:
...
#   - Outputs: dense exact-transitive-order72 pair-graph ranks.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Canonical blocks target the transitive-order72 conjugacy class.
# - Description:
#   - Interval Mobius coefficients isolate exact transitive-order72 stabilizers.
# - Usage:
...
"""Dense exact-transitive-order72 rank for pair-valued K6 edge graphs."""
...
_FIXED_COMPONENTS = 2
_EXPECTED_TRIPLE_BLOCK_SIZES = (18, 2)
_EXPECTED_TARGET_CONJUGATES = 10
_HARD_BLOCK_SIZE = 18
_MAXIMUM_HARD_COLORS = 5
...
def _ordered_compositions(
    total: int, maximum_parts: int = _MAXIMUM_HARD_COLORS
) -> tuple[tuple[int, ...], ...]:
    if total == 0:
        return ((),)
    if maximum_parts == 0:
    ...
    return tuple(
        (first, *tail)
        for first in range(1, total + 1)
        for tail in _ordered_compositions(total - first, maximum_parts - 1)
    )
...
def test_pair_transitive_order72_counts_match_independent_sequence() -> None:
    """Target-Mobius blocks reproduce exact transitive-order72 pair counts."""
...
def test_pair_transitive_order72_rank_exhausts_masses_three_through_six() -> (
    None
):
    """Every exact transitive-order72 pair rank through mass six roundtrips."""
...
def test_pair_transitive_order72_rank_roundtrips_through_fourteen() -> None:
    """Exact transitive-order72 pair ranks invert through mass fourteen."""
...
def test_pair_transitive_order72_triple_geometry_and_counts() -> None:
    """Two H-invariant triple blocks give the reviewed free quotient."""
...
def test_pair_transitive_order72_branch_rank_roundtrips_through_fourteen() -> (
    None
):
