...
# Boundary-Contract:
# - Owns:
#   - Dense normalizer-free and exact rank/unrank for the S2-times-S2
#     stabilizer in the top-level all-equal S6 residual stratum.
# - Must-Not:
#   - Claim dense rank for another exact full-S6 stabilizer type.
# - Allows:
#   - Inputs: H-fixed residual assignments for H generated by independent
#     endpoint swaps (23) and (45).
...
# - Summary:
#   - Rank the order-four normalizer quotient and skip 211,177 exact extensions.
# - Description:
#   - Exact extensions are S3-times-S2 or S3-times-S3.
# - Usage:
#   - Constructive rank for the largest remaining proper symmetric S6 type.
# - Defaults:
#   - Exact residual and complete rank/unrank reach total mass fourteen.
#

"""Dense exact rank for the full-S6 S2-times-S2 stabilizer."""
...
_EXPECTED_S3S2_GROUPS = 4
_EXPECTED_S3S3_GROUPS = 2
_EXPECTED_H_ORBIT_SIZES = {1: 12, 2: 12, 4: 4}
_EXPECTED_BLOCK_SIZES = (4, 2, 4, 4, 4, 2, 2, 2, 2, 2)
_EXPECTED_BLOCK_WEIGHTS = (1, 4, 1, 2, 2, 1, 1, 2, 2, 4)
...
_FIRST_SWAP: _Permutation = (0, 1, 3, 2, 4, 5)
_SECOND_SWAP: _Permutation = (0, 1, 2, 3, 5, 4)
_H_GENERATORS = (_FIRST_SWAP, _SECOND_SWAP)
...
def _exception_rank_sets(total: int) -> tuple[tuple[int, ...], tuple[int, ...]]:
    ...
    for group in (_S3S2_GROUPS[0], _S3S3_GROUPS[0]):
        ranks: set[int] = set()
...
def _exact_exclusions(total: int) -> tuple[int, ...]:
    s3s2, s3s3 = _exception_rank_sets(total)
    assert not set(s3s2) & set(s3s3)
    return tuple(sorted((*s3s2, *s3s3)))
...
def test_s2s2_normalizer_geometry_is_exact() -> None:
    """H has quotient order four, five subgroups, and ten rank blocks."""
    orbit_sizes = {
...
def test_s2s2_free_rank_matches_reviewed_counts() -> None:
...
def test_s2s2_exclusions_are_exact_s3s2_and_s3s3() -> None:
    """Nonnormalizing exceptions are exact S3-times-S2 or S3-times-S3."""
    assert len(_S3S2_GROUPS) == _EXPECTED_S3S2_GROUPS
    assert len(_S3S3_GROUPS) == _EXPECTED_S3S3_GROUPS
    for groups in (_S3S2_GROUPS, _S3S3_GROUPS):
        conjugates = {
            _conjugate_group(groups[0], normalizer)
            for normalizer in _NORMALIZER
        }
        assert conjugates == set(groups)
    assert tuple(
        free - exact
        ...
        s3s2 + s3s3
        for s3s2, s3s3 in zip(
            _EXPECTED_S3S2_EXCEPTIONS, _EXPECTED_S3S3_EXCEPTIONS, strict=True
        )
    )
    s3s2, s3s3 = _exception_rank_sets(_MAXIMUM_MASS)
    assert len(s3s2) == _EXPECTED_S3S2_EXCEPTIONS[-1]
    assert len(s3s3) == _EXPECTED_S3S3_EXCEPTIONS[-1]
    assert not set(s3s2) & set(s3s3)


def test_s2s2_exact_rank_roundtrips_through_fourteen() -> None:
    """Sparse exclusion gives one dense exact-H residual interval."""
    derived = tuple(
        free - s3s2 - s3s3
        for free, s3s2, s3s3 in zip(
            _EXPECTED_FREE_COUNTS,
            _EXPECTED_S3S2_EXCEPTIONS,
            _EXPECTED_S3S3_EXCEPTIONS,
            strict=True,
...
def test_s2s2_exact_samples_have_stabilizer_four() -> None:
...
def test_s2s2_complete_rank_roundtrips_through_fourteen() -> None:
