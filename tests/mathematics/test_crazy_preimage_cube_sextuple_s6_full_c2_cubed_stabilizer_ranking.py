...
# Boundary-Contract:
# - Owns:
#   - Dense normalizer-free and exact rank/unrank for the C2-cubed
#     stabilizer in the top-level all-equal S6 residual stratum.
# - Must-Not:
#   - Claim dense rank for another exact full-S6 stabilizer type.
# - Allows:
#   - Inputs: H-fixed residual assignments for the C2-cubed subgroup.
...
# - Summary:
#   - Rank the order-four normalizer S3 quotient with no external exceptions.
# - Description:
#   - Normalizer-free already means exact-H for this subgroup.
# - Usage:
#   - Constructive rank for the independent three-swap C2-cubed type.
# - Defaults:
#   - Exact residual and complete rank/unrank reach total mass fourteen.
#

"""Dense exact rank for the full-S6 C2-cubed stabilizer."""
...
_QUOTIENT_ORDER = 6
_EXPECTED_QUOTIENT_SUBGROUPS = 6
_EXPECTED_H_GENERATORS = 3
_EXPECTED_VERTEX_ORBITS = (2, 2, 2)
_EXPECTED_H_ORBIT_SIZES = {1: 8, 2: 6, 4: 6}
_EXPECTED_BLOCK_SIZES = (2, 1, 6, 3, 3, 3, 3)
_EXPECTED_BLOCK_WEIGHTS = (1, 8, 2, 1, 1, 4, 4)
...
_FIRST_GENERATOR: _Permutation = (1, 0, 2, 3, 4, 5)
_SECOND_GENERATOR: _Permutation = (0, 1, 3, 2, 4, 5)
_THIRD_GENERATOR: _Permutation = (0, 1, 2, 3, 5, 4)
...
def test_c2_cubed_normalizer_geometry_is_exact() -> None:
    """H has order-four quotient, five subgroups, and eleven rank blocks."""
    orbit_sizes = {
...
def test_c2_cubed_free_rank_matches_exact_counts() -> None:
...
def test_c2_cubed_exact_rank_roundtrips_through_fourteen() -> None:
...
def test_c2_cubed_exact_samples_have_stabilizer_eight() -> None:
    """Sampled retained representatives have exact S6 stabilizer H."""
    for total in (3, 4, 5, _MAXIMUM_MASS):
        count = _exact_count(total)
...
def test_c2_cubed_complete_rank_roundtrips_through_fourteen() -> None:
