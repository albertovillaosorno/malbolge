...
# Boundary-Contract:
# - Owns:
#   - Independent evidence for aggregate ordered-triple preimage-cube quotients.
# - Must-Not:
...
#   - Outputs: exact raw and canonical ordered-triple case counts.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Sum ordered cube-triple quotient classes across reachable fixed pairs.
# - Description:
#   - Brute-forces small widths and checks a weighted-binomial closed identity.
# - Usage:
#   - Referenced by the mathematical correspondence manifest.
# - Defaults:
#   - Brute force stops at width four; arithmetic reaches width fourteen.
#

"""Evidence for global ordered cube-triple quotient counts."""
...
def _ordered_triple_classes(dimension: int) -> int:
    return comb(
        dimension + _TRIPLE_PATTERN_COUNT - 1,
        _TRIPLE_PATTERN_COUNT - 1,
    )
...
def _independent_width_counts(trit_count: int) -> tuple[int, int]:
    ...
    for accumulator in range(domain):
        for target in range(domain):
            dimension = _ambiguity_dimension(
                accumulator,
                target,
                trit_count,
            )
...
def test_global_ordered_triple_quotient_has_exact_closed_count() -> None:
    """Ambiguity-class summation equals the closed triple-quotient count."""
    for trit_count in range(1, _MAXIMUM_TRITS + 1):
        assert _direct_canonical_count(
            trit_count
