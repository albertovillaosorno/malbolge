...
#     stabilizer is S4-times-S2 on three paired endpoint orbits.
# - Must-Not:
...
#   - Outputs: dense exact-S4-times-S2 pair-graph ranks.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Canonical blocks target the S4-times-S2 conjugacy class.
# - Description:
#   - Interval Mobius coefficients isolate exact S4-times-S2 stabilizers.
# - Usage:
...
"""Dense exact-S4-times-S2 rank for pair-valued K6 edge graphs."""
...
from tests.mathematics import (
    test_crazy_preimage_cube_sextuple_s6_s4s2_vertex_ranking as s4s2_rank,
)
...
_FIXED_COMPONENTS = 2
_ACTIVE_ORBIT_SIZE = 4
_PAIRED_ORBIT_SIZE = 2
_HARD_TRIPLE_BLOCK_SIZE = 12
_EXPECTED_TRIPLE_BLOCK_SIZES = (12, 4, 4)
...
def _reference_vertex_orbits() -> tuple[tuple[int, ...], tuple[int, ...]]:
    unseen = set(range(_ARITY))
    ...
    while unseen:
        ...
        orbit = tuple(
            sorted({
                PERMUTATIONS[element][seed] for element in _REFERENCE_TARGET
            })
        )
        unseen -= set(orbit)
        orbits.append(orbit)
    active = next(orbit for orbit in orbits if len(orbit) == _ACTIVE_ORBIT_SIZE)
    paired = next(orbit for orbit in orbits if len(orbit) == _PAIRED_ORBIT_SIZE)
...
@cache
def _hard_coordinate_layout(coordinate: int) -> tuple[int, int]:
    active, paired = _reference_vertex_orbits()
    triple = _TRIPLES[coordinate]
    active_vertices = tuple(vertex for vertex in triple if vertex in active)
    paired_vertex = next(vertex for vertex in triple if vertex in paired)
    first = active.index(active_vertices[0])
    second = active.index(active_vertices[1])
    edge = (first, second) if first < second else (second, first)
    return _K4_EDGES.index(edge), paired.index(paired_vertex)


@cache
def _hard_layout(block: _TripleBlock) -> tuple[tuple[int, int], ...]:
    return tuple(_hard_coordinate_layout(coordinate) for coordinate in block)


def _hard_vector(edge_pairs: _K4EdgePairs, block: _TripleBlock) -> _Vector:
    return tuple(
        edge_pairs[edge][component] for edge, component in _hard_layout(block)
    )


def _swap_edge_components(edge_pairs: _K4EdgePairs) -> _K4EdgePairs:
    return cast(
        "_K4EdgePairs",
        tuple((right, left) for left, right in edge_pairs),
    )
...
def _triple_hard_classes(mass: int) -> tuple[_TripleLocalClass, ...]:
    ...
    assert len(block) == _HARD_TRIPLE_BLOCK_SIZE
    seen: set[int] = set()
    rows: list[tuple[int, _Vector, Subgroup]] = []
    for rank in range(s4s2_rank.s4_pair_edge_class_count(mass)):
        edge_pairs = s4s2_rank.s4_pair_edge_unrank(mass, rank)
        assert edge_pairs is not None
        swapped = s4s2_rank.s4_pair_edge_rank(_swap_edge_components(edge_pairs))
        assert swapped is not None
        orbit_rank = min(rank, swapped)
        if orbit_rank in seen:
            continue
        seen.add(orbit_rank)
        representative_pairs = s4s2_rank.s4_pair_edge_unrank(mass, orbit_rank)
        assert representative_pairs is not None
        raw = _hard_vector(representative_pairs, block)
        representative = min(
        ...
        rows.append((
            orbit_rank,
            representative,
            _triple_stabilizer(representative, block, _REFERENCE_TARGET),
        ))
...
def test_pair_s4_times_s2_counts_match_independent_sequence() -> None:
    """Target-Mobius blocks reproduce exact S4-times-S2 pair counts."""
...
def test_pair_s4_times_s2_rank_exhausts_masses_three_through_five() -> None:
    """Every exact S4-times-S2 pair rank through mass five roundtrips."""
...
def test_pair_s4_times_s2_rank_roundtrips_through_fourteen() -> None:
    """Exact S4-times-S2 pair ranks invert through mass fourteen."""
...
def test_pair_s4_times_s2_triple_geometry_and_counts() -> None:
    """Three H-invariant triple blocks give the reviewed free-H quotient."""
...
def test_pair_s4_times_s2_branch_rank_roundtrips_through_fourteen() -> None:
