...
#     stabilizer is D8-411 on three paired endpoint orbits.
# - Must-Not:
...
#   - Outputs: dense exact-D8-411 pair-graph ranks.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Canonical blocks target the D8-411 conjugacy class.
# - Description:
#   - Interval Mobius coefficients isolate exact D8-411 stabilizers.
# - Usage:
...
"""Dense exact-D8-411 rank for pair-valued K6 edge graphs."""
...
_FIXED_COMPONENTS = 2
_EXPECTED_TRIPLE_BLOCK_SIZES = (2, 2, 4, 4, 4, 4)
_EXPECTED_TARGET_CONJUGATES = 45
_EXPECTED_TARGET_TRANSPOSITIONS = 2
_TRANSPOSITION_MOVED_ENDPOINTS = 2
...
_TARGET_VERTEX_ORBITS = (4, 1, 1)
_TARGET_ORDER = 8


def _is_transposition(element: int) -> bool:
    order = PERMUTATIONS[element]
    moved = tuple(index for index in range(_ARITY) if order[index] != index)
    return (
        len(moved) == _TRANSPOSITION_MOVED_ENDPOINTS
        and order[moved[0]] == moved[1]
        and order[moved[1]] == moved[0]
    )


def _transposition_count(group: Subgroup) -> int:
    return sum(_is_transposition(element) for element in group)
...
_TARGET_SUBGROUPS = tuple(
    ...
    and _transposition_count(group) == _EXPECTED_TARGET_TRANSPOSITIONS
)
...
def test_pair_d8_411_counts_match_independent_sequence() -> None:
    """Target-Mobius blocks reproduce exact D8-411 pair counts."""
    ...
    assert all(
        _transposition_count(group) == _EXPECTED_TARGET_TRANSPOSITIONS
        for group in _TARGET_SUBGROUPS
    )


def test_pair_d8_411_rank_exhausts_masses_three_through_five() -> None:
    """Every exact D8-411 pair rank through mass five roundtrips."""
...
def test_pair_d8_411_rank_roundtrips_through_fourteen() -> None:
    """Exact D8-411 pair ranks invert through mass fourteen."""
...
def test_pair_d8_411_triple_geometry_and_counts() -> None:
...
def test_pair_d8_411_branch_rank_roundtrips_through_fourteen() -> None:
