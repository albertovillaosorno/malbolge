...
# Boundary-Contract:
# - Owns:
#   - Independent composition evidence for dense global S5 quotient ranking.
# - Must-Not:
#   - Re-prove local S5 rank semantics or apply endpoint symmetry to
#     direction-sensitive analyses.
# - Allows:
#   - Inputs: reachable-pair base-seven digits and proved local S5 ranks.
#   - Outputs: exact dense global rank/unrank through width fourteen.
#   - Side effects: none.
# - Split-When:
#   - A larger endpoint group needs a distinct global composition.
# - Merge-When:
#   - Global S5 accounting owns the same ragged-domain rank composition.
# - Summary:
#   - Compose reachable-pair order with complete dense unordered-quintuple rank.
# - Description:
#   - Uses ambiguity-weighted suffix sums of exact local S5 class counts.
# - Usage:
...
"""Dense global ranking for endpoint-unordered S5 preimage quintuple classes."""
...
def _choose_digit(position: int, ambiguity: int, rank: int) -> tuple[int, int]:
    remaining = rank
...
def test_global_s5_rank_exhausts_small_ragged_domains() -> None:
    """Widths one through four receive every global S5 rank exactly once."""
...
def test_global_s5_rank_roundtrips_checked_boundaries() -> None:
    """Boundary and interior S5 ranks roundtrip through every checked width."""
...
def test_global_s5_prefix_matches_direct_small_pair_sums() -> None:
    """Digit-DP prefixes equal direct earlier fixed-pair S5 class sums."""
    for trit_count in range(1, _EXHAUSTIVE_TRITS + 1):
        running = 0
        for pair_rank in range(_integer_power(_PAIR_RADIX, trit_count)):
            digits = _pair_digits(pair_rank, trit_count)
