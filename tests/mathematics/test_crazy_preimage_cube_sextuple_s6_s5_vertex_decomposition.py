...
# Boundary-Contract:
# - Owns:
#   - Nested S5 factorization of the S6 vertex partition (5,1).
# - Must-Not:
#   - Claim dense ranking for any second-layer S5 stabilizer.
# - Allows:
#   - Inputs: residual and complete sextuple mass zero through fourteen.
#   - Outputs: exact fixed/vertex-bundle/K5-edge residual quotient counts.
#   - Side effects: none.
# - Split-When:
#   - A second-layer S5 stabilizer receives constructive rank/unrank.
# - Merge-When:
#   - Complete dense S6 ranking owns the same nested S5 factorization.
# - Summary:
#   - Sort five two-component vertex bundles, then quotient K5 edge bundles.
# - Description:
#   - Leaves the seven Young stabilizers indexed by integer partitions of five.
# - Usage:
#   - Canonical prerequisite for dense ranking of the (5,1) S6 stratum.
# - Defaults:
#   - Direct residual orbits stop at mass two; arithmetic reaches mass 14.
#

"""Nested S5 factorization of the S6 (5,1) Young stratum."""
...
_EDGE_COMPONENTS = 4
_EDGE_WEIGHT = 2
_VERTEX_BUNDLE_COUNT = 5
_EDGE_BUNDLE_COUNT = 10
_WIDTH_FOURTEEN_RESIDUAL_COUNT = 511_090_971_734
_WIDTH_FOURTEEN_STRATUM_COUNT = 310_719_486_939
_TOP_PARTITION = (5, 1)
_SECOND_PARTITIONS = {
    (1, 1, 1, 1, 1),
    (2, 1, 1, 1),
...
_S5: tuple[_Permutation, ...] = tuple(
    _as_permutation((*order, 5)) for order in permutations(_ACTIVE)
)
...
def _active_bits(label: int) -> tuple[int, int, int, int, int]:
    values = tuple(
...
def _active_edge(label: int) -> tuple[int, int]:
    bits = _active_bits(label)
    weight = sum(bits)
    assert weight in {_EDGE_WEIGHT, _ACTIVE_COUNT - _EDGE_WEIGHT}
    target = 1 if weight == _EDGE_WEIGHT else 0
    values = tuple(index for index, value in enumerate(bits) if value == target)
    left, right = values
    return left, right


def _vertex_orbit_item(
    orbit: tuple[int, ...],
) -> tuple[int, int, int, int, int]:
    by_vertex = {_active_vertex(label): label for label in orbit}
    values = tuple(by_vertex[index] for index in _ACTIVE)
    first, second, third, fourth, fifth = values
    return first, second, third, fourth, fifth
...
def _label_geometry(
    ...
    fixed: list[int] = []
    vertices: list[tuple[int, int, int, int, int]] = []
    edges: list[tuple[int, ...]] = []
    for orbit in _s5_orbits():
        if len(orbit) == 1:
            fixed.append(orbit[0])
        elif len(orbit) == _ACTIVE_COUNT:
            vertices.append(_vertex_orbit_item(orbit))
        else:
            assert len(orbit) == len(_K5_EDGES)
            by_edge = {_active_edge(label): label for label in orbit}
            edges.append(tuple(by_edge[edge] for edge in _K5_EDGES))
...
def _full_residual_count(mass: int) -> int:
    fixed = sum(
        _fixed_count_from_cycles(_label_cycles(order), mass) for order in _S5
    )
    assert fixed % len(_S5) == 0
...
def _edge_cycles(order: tuple[int, ...]) -> tuple[int, ...]:
    permutation = tuple(
        _K5_EDGE_INDEX[_ordered_edge(order[left], order[right])]
        for left, right in _K5_EDGES
    )
    unseen = set(range(len(_K5_EDGES)))
    result: list[int] = []
    while unseen:
        current = min(unseen)
        length = 0
...
def _choice_partition(counts: tuple[int, int, int, int, int]) -> _Partition:
    return tuple(
        sorted(
            (
                index + 1
...
def _mass_choices(mass: int) -> tuple[tuple[int, int, _Partition, int], ...]:
    ...
    for values in product(*ranges):
        singles, doubles, triples, quads, quints = values
        item = _choice_data(
            mass,
            population,
            (singles, doubles, triples, quads, quints),
        )
        if item is not None:
            result.append(item)
    return tuple(result)
...
def test_s6_s5_residual_labels_have_exact_nested_geometry() -> None:
    """The 52 labels are two fixed scalars plus vertex and K5-edge bundles."""
...
def test_s6_s5_vertex_bundles_leave_exact_five_stabilizers() -> None:
    """Five sorted two-component bundles induce seven Young groups."""
    histogram = _vertex_bundle_histogram()
...
def test_s6_s5_nested_factorization_matches_direct_small_orbits() -> None:
    """The nested factorization agrees with raw S5 residual orbits."""
...
def test_s6_s5_nested_factorization_matches_burnside_through_fourteen() -> None:
    """Five second-layer stabilizers reconstruct the residual S5 quotient."""
    for mass in range(_MAXIMUM_MASS + 1):
