...
# Boundary-Contract:
# - Owns:
#   - Dense normalizer-free and exact rank/unrank for the transitive A4-times-S2
#     stabilizer in the top-level all-equal S6 residual stratum.
# - Must-Not:
#   - Claim dense rank for another exact full-S6 stabilizer type.
# - Allows:
#   - Inputs: H-fixed residual assignments for the transitive A4-times-S2 H.
#   - Outputs: free N(H)/H ranks, exact-H ranks, and complete outer ranks.
#   - Side effects: none.
# - Split-When:
#   - A nonnormalizing strict extension reaches the free quotient.
# - Merge-When:
#   - Complete dense full-S6 ranking owns every exact stabilizer stratum.
# - Summary:
#   - Rank the order-two normalizer quotient with no residual exceptions.
# - Description:
#   - Normalizer-free already equals exact H through mass fourteen.
# - Usage:
#   - Constructive rank for the transitive A4-times-S2 endpoint type.
# - Defaults:
#   - Exact residual and complete rank/unrank reach total mass fourteen.
#

"""Dense exact rank for the full-S6 transitive A4-times-S2 stabilizer."""
...
_QUOTIENT_ORDER = 2
_EXPECTED_QUOTIENT_SUBGROUPS = 2
_A4_ORDER = 12
_A4_INVOLUTIONS = 3
_C2_ORDER = 2
_EXPECTED_H_ORBIT_SIZES = {1: 2, 3: 2, 6: 2, 8: 1, 12: 2}
_EXPECTED_BLOCK_SIZES = (2, 2, 1, 2, 2)
_EXPECTED_BLOCK_WEIGHTS = (1, 3, 8, 12, 6)
_EXPECTED_FREE_COUNTS = (
    0, 0, 0, 0, 0, 0, 1, 2, 3, 6, 9, 12, 19, 26, 34,
)
_EXPECTED_EXACT_COUNTS = _EXPECTED_FREE_COUNTS
_EXPECTED_EXCEPTIONS = (0,) * (_MAXIMUM_MASS + 1)
_EXPECTED_COMPLETE_COUNTS = (
    0, 0, 0, 0, 0, 0, 1, 2, 3, 6, 9, 12, 21, 30, 40,
)
...
_FIRST_GENERATOR: _Permutation = (0, 1, 2, 3, 5, 4)
_SECOND_GENERATOR: _Permutation = (0, 1, 3, 2, 4, 5)
_THIRD_GENERATOR: _Permutation = (1, 0, 2, 3, 4, 5)
_FOURTH_GENERATOR: _Permutation = (2, 3, 4, 5, 0, 1)
_H_GENERATORS = (
    _FIRST_GENERATOR,
    _SECOND_GENERATOR,
    _THIRD_GENERATOR,
...
def test_a4_times_s2_normalizer_geometry_is_exact() -> None:
    """H is A4 times S2 with an order-two quotient and five rank blocks."""
    orbit_sizes = {
        size: sum(len(orbit) == size for orbit in _H_ORBITS)
        for size in (1, 3, 6, 8, 12)
    }
    central = _compose(
        _compose(_FIRST_GENERATOR, _SECOND_GENERATOR), _THIRD_GENERATOR
    )
    a4 = _generated((
        _compose(_FIRST_GENERATOR, _SECOND_GENERATOR),
        _FOURTH_GENERATOR,
    ))
    c2 = _generated((central,))
    product = {_compose(left, right) for left in a4 for right in c2}
    assert len(a4) == _A4_ORDER
    assert sum(
        element != _IDENTITY and _compose(element, element) == _IDENTITY
        for element in a4
    ) == _A4_INVOLUTIONS
    assert len(c2) == _C2_ORDER
    assert len(a4 & c2) == 1
    assert all(
        _compose(central, element) == _compose(element, central)
        for element in _H
    )
    assert product == set(_H)
    assert _vertex_orbit_sizes(_H) == (6,)
...
def test_a4_times_s2_free_rank_matches_reviewed_counts() -> None:
    """The order-two normalizer quotient gives the reviewed free sequence."""
...
def test_a4_times_s2_minimal_extensions_match_exact_exclusions() -> None:
...
def test_a4_times_s2_exact_rank_roundtrips_through_fourteen() -> None:
...
def test_a4_times_s2_exact_samples_have_stabilizer_twenty_four() -> None:
    """Sampled retained representatives have exact S6 stabilizer H."""
    for total in (6, 9, 12, _MAXIMUM_MASS):
        count = _exact_count(total)
...
def test_a4_times_s2_complete_rank_roundtrips_through_fourteen() -> None:
