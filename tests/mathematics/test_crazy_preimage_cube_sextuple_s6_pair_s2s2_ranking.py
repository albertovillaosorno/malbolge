...
#     stabilizer is S2-times-S2 on two disjoint pairs.
# - Must-Not:
...
#   - Outputs: dense exact-S2-times-S2 pair-graph ranks.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Canonical blocks target the two-pair V4 conjugacy class.
# - Description:
#   - Interval Mobius coefficients isolate exact two-pair V4 stabilizers.
# - Usage:
...
"""Dense exact-S2-times-S2 rank for pair-valued K6 edge graphs."""
...
_FIXED_COMPONENTS = 2
_EXPECTED_TRIPLE_BLOCK_SIZES = (1, 1, 1, 1, 2, 2, 2, 2, 4, 4)
...
def test_pair_s2s2_counts_match_independent_sequence() -> None:
    """Target-Mobius blocks reproduce exact two-pair V4 pair counts."""
...
def test_pair_s2s2_rank_exhausts_masses_three_through_five() -> None:
    """Every exact two-pair V4 pair rank through mass five roundtrips."""
...
def test_pair_s2s2_rank_roundtrips_through_fourteen() -> None:
    """Exact two-pair V4 pair ranks invert through mass fourteen."""
...
def test_pair_s2s2_triple_geometry_and_counts() -> None:
...
def test_pair_s2s2_branch_rank_roundtrips_through_fourteen() -> None:
