...
#     stabilizer is one triple transposition.
# - Must-Not:
#   - Claim coverage for pair-graph stabilizers outside this involution.
# - Allows:
#   - Inputs: pair-valued K6 edges and twenty triples through mass fourteen.
#   - Outputs: dense pair, free-triple, residual, and complete branch ranks.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Canonical blocks target the triple-transposition conjugacy class.
# - Description:
#   - Interval Mobius coefficients isolate this exact involution type.
# - Usage:
#   - Supplies the triple-transposition pair branch and triple breaking.
# - Defaults:
#   - Exhaustive ranks reach pair mass five; sampled ranks reach fourteen.
#

"""Dense exact-triple-transposition rank for pair-valued K6 edge graphs."""
...
_TRANSPOSED_TRIPLE_PAIRS = 10
_FIXED_TRIPLES = 0
_PAIRED_TRIPLE_COORDINATES = 10
_FIXED_COMPONENTS = 2
...
def _triple_fixed_count(total: int) -> int:
    if total % 2 != 0:
        return 0
    return _weak_count(total // 2, _TRANSPOSED_TRIPLE_PAIRS)
...
def _triple_rank(total: int, vector: _Vector, group: Subgroup) -> int | None:
    ...
    if fixed:
        return None
    return _paired_triple_rank(left, right)


def _triple_unrank(total: int, rank: int, group: Subgroup) -> _Vector | None:
    if rank < 0 or rank >= _triple_free_count(total):
        return None
    pair = _paired_triple_unrank(total, rank)
    if pair is None:
        return None
    return _build_triples(((), pair[0], pair[1]), group)
...
def test_pair_triple_transposition_counts_match_lattice() -> None:
...
def test_pair_triple_transposition_rank_exhausts_small_masses() -> None:
    """Exact triple-transposition pair ranks roundtrip through mass five."""
...
def test_pair_triple_transposition_rank_roundtrips_through_fourteen() -> None:
    """Exact triple-transposition pair ranks invert through mass fourteen."""
...
def test_pair_triple_transposition_triple_count_matches_branch() -> None:
    """Free triples over triple-transposition pairs give the branch count."""
    observed = tuple(_residual_branch_count(total) for total in range(15))
    assert observed == _EXPECTED_RESIDUAL_BRANCH_COUNTS


def test_triple_transposition_triple_rank_matches_free_counts() -> None:
    """Ten swapped triple pairs rank every free involution orbit."""
    group = _TARGET_SUBGROUPS[0]
    fixed, pairs = _triple_layout(group)
    assert len(fixed) == 0
    assert len(pairs) == _PAIRED_TRIPLE_COORDINATES
    for total in range(_MAXIMUM_MASS + 1):
        observed = _paired_triple_count(total)
...
def test_triple_transposition_branch_rank_roundtrips_through_fourteen() -> None:
