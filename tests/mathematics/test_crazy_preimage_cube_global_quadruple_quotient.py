...
# Boundary-Contract:
# - Owns:
#   - Independent aggregate accounting for ordered quadruple cube quotients.
# - Must-Not:
...
#   - Outputs: exact raw and canonical ordered-quadruple case counts.
#   - Side effects: none.
# - Split-When:
...
# - Summary:
#   - Sum ordered cube-quadruple quotient classes over reachable fixed pairs.
# - Description:
...
"""Prospective evidence for global ordered cube-quadruple quotient counts."""
...
def _ordered_quadruple_classes(dimension: int) -> int:
    return comb(dimension + _QUADRUPLE_PATTERN_COUNT - 1, 15)


def _direct_canonical_count(trit_count: int) -> int:
    return sum(
        _fixed_pair_class_count(trit_count, dimension)
        * _ordered_quadruple_classes(dimension)
        for dimension in range(trit_count + 1)
    )


def _closed_canonical_count(trit_count: int) -> int:
    separators = _QUADRUPLE_PATTERN_COUNT - 1
    return sum(
    ...
    for accumulator in range(domain):
        for target in range(domain):
            ...
            if dimension is None:
                continue
            raw += _integer_power(_QUADRUPLE_PATTERN_COUNT, dimension)
            canonical += _ordered_quadruple_classes(dimension)
    return raw, canonical


def test_small_widths_match_independent_fixed_pair_enumeration() -> None:
    """Small fixed pairs sum to exact raw and quotient quadruple counts."""
    for trit_count in range(1, _EXHAUSTIVE_TRITS + 1):
        raw, canonical = _independent_width_counts(trit_count)
        assert raw == _integer_power(37, trit_count)
        assert canonical == _closed_canonical_count(trit_count)


def test_global_ordered_quadruple_quotient_has_exact_closed_count() -> None:
    """Ambiguity-class summation equals the closed quadruple quotient count."""
    for trit_count in range(1, _MAXIMUM_TRITS + 1):
        ...
        raw = sum(
            _fixed_pair_class_count(trit_count, dimension)
            * _integer_power(_QUADRUPLE_PATTERN_COUNT, dimension)
            for dimension in range(trit_count + 1)
        )
        assert raw == _integer_power(37, trit_count)
