...
# Boundary-Contract:
# - Owns:
#   - Dense rank/unrank evidence for four-component K5 edge values under a
#     residual S3xS2 stabilizer on disjoint equal vertex blocks with sizes
#     three and two.
# - Must-Not:
#   - Claim ranking for S4/S5 residual stabilizers or complete dense S5 ranking.
# - Allows:
#   - Inputs: residual edge-value mass zero through fourteen.
...
# - Summary:
#   - Densely rank residual S3xS2 widened K5 edge orbits.
# - Description:
#   - Quotients three twelve-scalar bundles by S3, then by their common internal
#     involution using fixed/nonfixed multiset blocks.
# - Usage:
#   - Constructive coverage for the order-twelve S5 residual strata.
# - Defaults:
#   - Direct product-group orbit exhaustion stops at mass two.
#

"""Dense residual S3xS2 ranking for widened K5 edge assignments."""
...
_MOVING_TAG = "m"
_MAXIMUM_MASS = 14
_WIDTH_FOURTEEN_COUNT = 200_608_118_832
_EXPECTED_COUNTS = (
    1,
    12,
    134,
...
def _t(vector: _Vector) -> _Vector:
    ...
    width = _PAIR_COMPONENTS
    return (
        vector[0:width]
        + vector[2 * width : 3 * width]
        + vector[width : 2 * width]
    )
...
def _fixed_count(total: int) -> int:
    return sum(
        _composition_count(margin_mass, _PAIR_COMPONENTS)
        * _composition_count((total - margin_mass) // 2, _PAIR_COMPONENTS)
...
def _fixed_rank(vector: _Vector) -> int | None:
    if len(vector) != _BUNDLE_COMPONENTS or _t(vector) != vector:
        return None
    margin = vector[0:_PAIR_COMPONENTS]
    side = vector[_PAIR_COMPONENTS : 2 * _PAIR_COMPONENTS]
    ...
    prefix = sum(
        ...
        * _composition_count((total - mass) // 2, _PAIR_COMPONENTS)
        for mass in range(margin_mass)
        if (total - mass) % 2 == 0
    )
    return (
        prefix
        + margin_rank * _composition_count(side_mass, _PAIR_COMPONENTS)
        + side_rank
    )
...
def _fixed_unrank(total: int, rank: int) -> _Vector:
    ...
    for margin_mass in range(total + 1):
        ...
        side_count = _composition_count(side_mass, _PAIR_COMPONENTS)
        block = _composition_count(margin_mass, _PAIR_COMPONENTS) * side_count
        if remaining >= block:
            remaining -= block
            continue
        margin_rank, side_rank = divmod(remaining, side_count)
        margin = _composition_unrank(margin_mass, _PAIR_COMPONENTS, margin_rank)
        side = _composition_unrank(side_mass, _PAIR_COMPONENTS, side_rank)
...
def _side_moving_count(total: int) -> int:
    ...
    for left_mass in range((total + 1) // 2):
        ...
        count += _composition_count(
            left_mass, _PAIR_COMPONENTS
        ) * _composition_count(right_mass, _PAIR_COMPONENTS)
    if total % 2 == 0:
        half = total // 2
    ...
    if len(left) == _PAIR_COMPONENTS and len(right) == _PAIR_COMPONENTS:
        ...
        if left != right:
            ...
            if left_key[0] < right_key[0]:
                ...
                population = _composition_count(left_key[0], _PAIR_COMPONENTS)
...
def _moving_rank(vector: _Vector) -> tuple[int, bool] | None:
    ...
    sides = _side_moving_rank(
        vector[_PAIR_COMPONENTS : 2 * _PAIR_COMPONENTS],
        vector[2 * _PAIR_COMPONENTS : 3 * _PAIR_COMPONENTS],
    )
    if sides is None:
        return None
    margin_mass = sum(margin)
    margin_rank = _composition_rank(margin, margin_mass)
    assert margin_rank is not None
    side_mass = sum(vector[_PAIR_COMPONENTS:])
...
def _sym2_moving_rank(state: _Sym2, total: int) -> tuple[int, bool] | None:
    ...
    if moving[0] is None or moving[1] is None:
        return None
    return _sym2_all_moving_rank((moving[0], moving[1]), fixed, moving_count)
...
def _distinct_block_count(masses: tuple[int, int, int]) -> int:
    ...
    return _fixed_count(first) * quotient_tail + _moving_count(first) * raw_tail
...
def _distinct_block_unrank(
    ...
    if rank < fixed_block:
        ...
        second, third = _pair_quotient_unrank(masses[1], masses[2], tail_rank)
...
def _sym2_left_block_count(left_mass: int, right_mass: int) -> int:
    return _sym2_fixed_count(left_mass) * _orbit_count(
        right_mass
...
def _sym2_right_block_count(left_mass: int, right_mass: int) -> int:
    return _fixed_count(left_mass) * _sym2_orbit_count(
        right_mass
...
def _sym2_right_block_rank(left: _Vector, state: _Sym2, total: int) -> int:
    ...
    return (
        prefix
        + moving[0] * _sym2_raw_count(total)
        + _sym2_raw_rank(state, total)
    )
...
def _sym3_rank_doubled_moving(
    ...
    *,
    ...
    doubled_bits = [
        bits[index] for index, orbit in enumerate(orbits) if orbit == doubled
    ]
    single_index = next(
...
def _sym3_rank_all_moving(
    ...
    return _sym3_rank_distinct_moving(orbits, bits, moving_count, prefix=prefix)
...
def _sym3_unrank_all_moving(total: int, rank: int) -> _Bundles:
    ...
    if rank < same_block:
        ...
        result = _sym3_unrank_doubled_moving(total, rank - same_block, moving)
    else:
...
def _sym3_quotient_unrank(total: int, rank: int) -> _Bundles:
    ...
    if rank < fixed3:
        ...
        result = _sym3_unrank_two_fixed(total, rank - fixed3 - fixed_pair)
    elif rank < fixed3 + fixed_pair + ffm + fmm:
        result = _sym3_unrank_one_fixed(total, rank - fixed3 - fixed_pair - ffm)
    else:
...
def _flatten(
    ...
    values: list[int] = []
    for edge in edges:
        values.extend(edge_values[_EDGE_INDEX[edge]])
    return tuple(values)


def _rank(edge_values: _EdgeValues) -> int | None:
    if len(edge_values) != _EDGE_COUNT or any(
        value < 0 for pair in edge_values for value in pair
    ):
        return None
    fixed = _flatten(edge_values, (_FIXED_EDGE,))
    bundles = tuple(
        _flatten(edge_values, _bundle_edges(vertex))
...
def _assign_bundle(
    result: list[_EdgeValue | None],
    vertex: int,
    bundle: _Vector,
) -> None:
    values = tuple(
        tuple(bundle[index : index + _PAIR_COMPONENTS])
        for index in range(0, _BUNDLE_COMPONENTS, _PAIR_COMPONENTS)
    )
    for edge, value in zip(_bundle_edges(vertex), values, strict=True):
        assert len(value) == _PAIR_COMPONENTS
        result[_EDGE_INDEX[edge]] = (value[0], value[1], value[2], value[3])
...
def _unrank(total: int, rank: int) -> _EdgeValues | None:
    ...
    for fixed_mass in range(total + 1):
        ...
        result[_EDGE_INDEX[_FIXED_EDGE]] = (
            fixed[0],
            fixed[1],
            fixed[2],
            fixed[3],
...
def _permute_edges(
    ...
    for source, destination in enumerate(permutation):
        result[destination] = edge_values[source]
    assert all(pair is not None for pair in result)
    return tuple(pair for pair in result if pair is not None)
...
def _edge_values_from_vector(vector: _Vector) -> _EdgeValues:
    values = tuple(
        tuple(vector[index : index + _PAIR_COMPONENTS])
        for index in range(0, 40, _PAIR_COMPONENTS)
    )
    assert all(len(value) == _PAIR_COMPONENTS for value in values)
...
def _burnside_count(total: int) -> int:
    ...
    transposition = _fixed_scalar_count((1,) * 16 + (2,) * 12, total)
    three_cycle = _fixed_scalar_count((1,) * 4 + (3,) * 12, total)
    double_swap = _fixed_scalar_count((1,) * 8 + (2,) * 16, total)
    mixed = _fixed_scalar_count((1,) * 4 + (3,) * 4 + (6,) * 4, total)
    return (
        identity
        + 4 * transposition
        + 2 * three_cycle
...
def test_s6_s5_s3s2_dense_rank_matches_direct_small_orbits() -> None:
    ...
    for total in range(_EXHAUSTIVE_MASS + 1):
        representatives: dict[int, set[_EdgeValues]] = {}
        for vector in _weak_compositions(total, 40):
            edge_values = _edge_values_from_vector(vector)
            rank = _rank(edge_values)
            assert rank is not None
            orbit = {_permute_edges(edge_values, order) for order in orders}
...
def test_s6_s5_s3s2_dense_rank_roundtrips_through_mass_fourteen() -> None:
    ...
    for total in range(_MAXIMUM_MASS + 1):
        ...
        for rank in {0, count // 4, count // 2, (3 * count) // 4, count - 1}:
            ...
            for order in orders:
                assert _rank(_permute_edges(edge_values, order)) == rank
    assert _edge_orbit_count(_MAXIMUM_MASS) == _WIDTH_FOURTEEN_COUNT


def test_s6_s5_s3s2_bundle_blocks_match_descended_fixed_count() -> None:
    """Constructive blocks reproduce the reviewed product-group sequence."""
    observed = tuple(
        _edge_orbit_count(total) for total in range(_MAXIMUM_MASS + 1)
    )
