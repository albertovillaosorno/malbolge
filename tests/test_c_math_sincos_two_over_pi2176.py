...
# Boundary-Contract:
# - Owns:
#   - Q2176 two-over-pi table provenance and full-binary64 quotient certificate.
# - Must-Not:
#   - Use host pi/libm as numerical authority or claim final sin/cos rounding.
# - Allows:
#   - Inputs: the runtime's directed Q2176 table.
...
# - Summary:
#   - Proves the Q2176 reciprocal table is wide enough for every binary64 tie.
# - Description:
...
# - Defaults:
#   - Full-domain periodic reduction begins at magnitude four.
#

"""Q2176 two-over-pi table and full-domain quotient separation evidence."""
...
LIMBS = 68
FRACTION_BITS = 2176
DENOMINATOR_LIMIT_BITS = 1073
HALF_BOUNDARY_FACTOR = 30
ATAN5_TERMS = 1450
ATAN239_TERMS = 420
EXPECTED_CONVERGENTS = 641
RECIPROCAL_ULPS = 1
...
#include <stdio.h>
int main(void) {
  MalbolgeGuestMathFixed2176Interval value;
  uint32_t index = UINT32_C(0);
  if (!malbolge_guest_math_two_over_pi_interval2176(&value)) return 80;
  while (index < MALBOLGE_GUEST_MATH_FIXED_2176_LIMBS) {
    (void)printf("%08" PRIx32 " %08" PRIx32 "\n",
                 value.lower.limbs[index], value.upper.limbs[index]);
    ++index;
  }
  if (malbolge_guest_math_two_over_pi_interval2176(0)) return 81;
  return 0;
}
"""
...
def _actual_table(tmp_path: Path) -> tuple[int, int]:
    source = tmp_path / "two-over-pi2176.c"
    binary = tmp_path / "two-over-pi2176"
...
def _assert_full_domain_margin() -> None:
    ...
    # For |x| >= 4, p >= -50. The reduced denominator of 4*x/(2*q+1)
    # is therefore below 2^1073. Legendre says any closer rational must be one
    # of the certified convergents above. A relevant half-boundary has
    # 4*x/(2*q+1) < 5 and pi < 4, so the one-ulp reciprocal cell cannot straddle
    # it when the conservative 30*2^-2176 margin is below the separation.
    table_uncertainty = Fraction(HALF_BOUNDARY_FACTOR, 1 << FRACTION_BITS)
    assert table_uncertainty < separation


def test_q2176_table_matches_machin_and_proves_full_domain_margin(
    tmp_path: Path,
) -> None:
