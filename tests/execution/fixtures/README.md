...
The sixteen `native-*-coff.hex` fixtures freeze the complete direct Windows COFF
...
halt-fetch/non-graphical/no-operation revision 2, and jump-code/jump-data revision
1 use metadata v3. Exact x86-64/AArch64 object sizes are 413/415 bytes for deopt,
...
622/731 for jump-code, and 564/699 for jump-data
respectively.
...
`memory[5]=93`, `memory[7]=11`, and `memory[11]=68`; it encrypts the loaded target
to 33 and advances `C/D` to 12/8. Every memory-backed fixture guards its exact
