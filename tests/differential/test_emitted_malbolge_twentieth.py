...
# Boundary-Contract:
# - Owns:
#   - Independent twentieth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twenty-cell sources and analyzer CLI JSON
#     reports.
...
# - Split-When:
#   - Twenty-first-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact twentieth-step
#     public surface.
# - Summary:
#   - Compares the twentieth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twentieth opcode after `/oo<jjjjjjj*jjjjjj`,
#     `j*pj*ppppj/*jjjjjj`, and `o<o<<<<<</o<*ooooo` histories, including a
...
"""Twentieth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 20
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*ooooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjjj"),
    tuple(b"j*pj*ppppj/*jjjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, twentieth) for history in _HISTORIES for twentieth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected twentieth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twentieth.malbolge"
...
@pytest.mark.parametrize(("history", "twentieth"), _CASES)
def test_twentieth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every twentieth opcode across three carried histories."""
    opcodes = (ord("<"), *history, twentieth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            twentieth in _ALIAS_WRITING_OPCODES
        )
