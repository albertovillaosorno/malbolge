...
# Boundary-Contract:
# - Owns:
#   - Independent eighth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted eight-cell sources and analyzer CLI JSON reports.
...
# - Split-When:
#   - Ninth-step differential reachability needs a separately bounded model.
# - Merge-When:
#   - Another independent verifier differential owns this exact eighth-step
#     public surface.
# - Summary:
#   - Compares the eighth bounded CLI transition to independent 1998 semantics.
# - Description:
#   - Covers every eighth opcode after `/oo<jj`, `j*pj*p`, and `o<o<<<`
#     histories.
# - Usage:
...
"""Independent eighth-transition differential tests for emitted Malbolge."""
...
_CASES: Final = tuple(
    (history, eighth) for history in _HISTORIES for eighth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = "selected eighth-step fixture reached a non-graphical fetch"
...
@pytest.mark.parametrize(("history", "eighth"), _CASES)
def test_eighth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int, int, int],
    eighth: int,
) -> None:
    """Compare every eighth opcode across three distinct carried histories."""
    opcodes = (ord("<"), *history, eighth)
    source_tuple = tuple(
    ...
    for key in ("sixth_transition", "seventh_transition", "eighth_transition"):
