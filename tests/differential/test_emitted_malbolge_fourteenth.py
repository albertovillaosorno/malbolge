...
# Boundary-Contract:
# - Owns:
#   - Independent fourteenth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted fourteen-cell sources and analyzer CLI JSON
#     reports.
...
# - Split-When:
#   - Fifteenth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact fourteenth-step
#     public surface.
# - Summary:
#   - Compares the fourteenth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every fourteenth opcode after `/oo<jjjjjjj*`, `j*pj*ppppj/*`, and
...
"""Independent fourteenth-transition differential tests for emitted Malbolge."""
...
_CASES: Final = tuple(
    (history, fourteenth) for history in _HISTORIES for fourteenth in _OPCODES
)
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "fourteenth.malbolge"
...
@pytest.mark.parametrize(("history", "fourteenth"), _CASES)
def test_fourteenth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int, int, int, int, int, int, int, int, int],
    fourteenth: int,
) -> None:
