...
# Boundary-Contract:
# - Owns:
#   - Independent nineteenth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted nineteen-cell sources and analyzer CLI JSON
#     reports.
...
# - Split-When:
#   - Twentieth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact nineteenth-step
#     public surface.
# - Summary:
#   - Compares the nineteenth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every nineteenth opcode after `/oo<jjjjjjj*jjjjj`,
#     `j*pj*ppppj/*jjjjj`, and `o<o<<<<<</o<*oooo` histories, including a final
...
"""Nineteenth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 19
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*oooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjj"),
    tuple(b"j*pj*ppppj/*jjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, nineteenth) for history in _HISTORIES for nineteenth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected nineteenth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "nineteenth.malbolge"
...
@pytest.mark.parametrize(("history", "nineteenth"), _CASES)
def test_nineteenth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every nineteenth opcode across three carried histories."""
    opcodes = (ord("<"), *history, nineteenth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            nineteenth in _ALIAS_WRITING_OPCODES
        )
