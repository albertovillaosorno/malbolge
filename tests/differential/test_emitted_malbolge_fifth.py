...
# Boundary-Contract:
# - Owns:
#   - Independent fifth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted five-cell sources and analyzer CLI JSON reports.
#   - Outputs: exact agreement with a self-contained historical five-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Sixth-step differential reachability needs a separately bounded model.
# - Merge-When:
#   - Another independent verifier differential owns this exact fifth-step
#     public surface.
# - Summary:
#   - Compares the fifth CLI transition to independent 1998 semantics.
# - Description:
#   - Covers every fifth opcode after `/oo`, `j*p`, and `o<o` carried histories.
# - Usage:
...
"""Independent fifth-transition differential tests for emitted Malbolge."""
...
_CASES: Final = tuple(
    (history, fifth) for history in _HISTORIES for fifth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = "selected fifth-step fixture reached a non-graphical fetch"
...
@pytest.mark.parametrize(("history", "fifth"), _CASES)
def test_fifth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int],
    fifth: int,
) -> None:
    """Compare every fifth opcode across three distinct carried histories."""
    opcodes = (ord("<"), *history, fifth)
    source_tuple = tuple(
    ...
    for key, transition in zip(
        ...
        _assert_followup(cast("dict[str, object]", document[key]), transition)
    ...
    assert continuations[-1] == document["fifth_transition"]
