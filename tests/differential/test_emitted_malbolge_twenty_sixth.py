...
# Boundary-Contract:
# - Owns:
#   - Independent twenty-sixth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twenty-six-cell sources and analyzer CLI JSON
#     reports.
#   - Outputs: exact agreement with a self-contained historical
#     twenty-six-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Twenty-seventh-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact
#     twenty-sixth-step public surface.
# - Summary:
#   - Compares the twenty-sixth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twenty-sixth opcode after `/oo<jjjjjjj*jjjjjjjjjjjj`,
#     `j*pj*ppppj/*jjjjjjjjjjjj`, and `o<o<<<<<</o<*ooooooooooo` histories,
...
"""Twenty-sixth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 26
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*ooooooooooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjjjjjjjjj"),
    tuple(b"j*pj*ppppj/*jjjjjjjjjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, twenty_sixth)
    for history in _HISTORIES
    for twenty_sixth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected twenty-sixth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twenty-sixth.malbolge"
...
@pytest.mark.parametrize(("history", "twenty_sixth"), _CASES)
def test_twenty_sixth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every twenty-sixth opcode across three carried histories."""
    opcodes = (ord("<"), *history, twenty_sixth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            twenty_sixth in _ALIAS_WRITING_OPCODES
        )
