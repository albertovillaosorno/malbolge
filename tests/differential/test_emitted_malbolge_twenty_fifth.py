...
# Boundary-Contract:
# - Owns:
#   - Independent twenty-fifth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twenty-five-cell sources and analyzer CLI JSON
#     reports.
#   - Outputs: exact agreement with a self-contained historical
#     twenty-five-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Twenty-sixth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact
#     twenty-fifth-step public surface.
# - Summary:
#   - Compares the twenty-fifth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twenty-fifth opcode after `/oo<jjjjjjj*jjjjjjjjjjj`,
#     `j*pj*ppppj/*jjjjjjjjjjj`, and `o<o<<<<<</o<*oooooooooo` histories,
...
"""Twenty-fifth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 25
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*oooooooooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjjjjjjjj"),
    tuple(b"j*pj*ppppj/*jjjjjjjjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, twenty_fifth)
    for history in _HISTORIES
    for twenty_fifth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected twenty-fifth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twenty-fifth.malbolge"
...
@pytest.mark.parametrize(("history", "twenty_fifth"), _CASES)
def test_twenty_fifth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every twenty-fifth opcode across three carried histories."""
    opcodes = (ord("<"), *history, twenty_fifth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            twenty_fifth in _ALIAS_WRITING_OPCODES
        )
