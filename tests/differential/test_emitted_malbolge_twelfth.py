...
# Boundary-Contract:
# - Owns:
#   - Independent twelfth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twelve-cell sources and analyzer CLI JSON reports.
...
# - Split-When:
#   - Thirteenth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact twelfth-step
#     public surface.
# - Summary:
#   - Compares the twelfth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twelfth opcode after `/oo<jjjjjj`, `j*pj*ppppj`, and
#     `o<o<<<<<</` histories, including a final C=D alias row and one aliased
#     data-write/self-encryption case.
# - Usage:
...
"""Independent twelfth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 12
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</")
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjj"),
    tuple(b"j*pj*ppppj"),
    _ALIAS_HISTORY,
...
_CASES: Final = tuple(
    (history, twelfth) for history in _HISTORIES for twelfth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = "selected twelfth-step fixture reached a non-graphical fetch"
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twelfth.malbolge"
...
@pytest.mark.parametrize(("history", "twelfth"), _CASES)
def test_twelfth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int, int, int, int, int, int, int],
    twelfth: int,
) -> None:
    """Compare every twelfth opcode across three distinct carried histories."""
    opcodes = (ord("<"), *history, twelfth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (twelfth == ord("*"))
