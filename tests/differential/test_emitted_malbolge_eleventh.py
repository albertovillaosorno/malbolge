...
# Boundary-Contract:
# - Owns:
#   - Independent eleventh-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted eleven-cell sources and analyzer CLI JSON reports.
...
# - Split-When:
#   - Twelfth-step differential reachability needs a separately bounded model.
# - Merge-When:
#   - Another independent verifier differential owns this exact eleventh-step
#     public surface.
# - Summary:
#   - Compares the eleventh bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every eleventh opcode after `/oo<jjjjj`, `j*pj*pppp`, and
#     `o<o<<<<<<` histories.
# - Usage:
...
"""Independent eleventh-transition differential tests for emitted Malbolge."""
...
_CASES: Final = tuple(
    (history, eleventh) for history in _HISTORIES for eleventh in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = "selected eleventh-step fixture reached a non-graphical fetch"
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "eleventh.malbolge"
...
@pytest.mark.parametrize(("history", "eleventh"), _CASES)
def test_eleventh_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int, int, int, int, int, int],
    eleventh: int,
) -> None:
    """Compare every eleventh opcode across three distinct carried histories."""
    opcodes = (ord("<"), *history, eleventh)
    source_tuple = tuple(
