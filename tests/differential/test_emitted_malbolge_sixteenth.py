...
# Boundary-Contract:
# - Owns:
#   - Independent sixteenth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted sixteen-cell sources and analyzer CLI JSON
#     reports.
...
# - Split-When:
#   - Seventeenth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact sixteenth-step
#     public surface.
# - Summary:
#   - Compares the sixteenth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every sixteenth opcode after `/oo<jjjjjjj*jj`,
#     `j*pj*ppppj/*jj`, and `o<o<<<<<</o<*o` histories, including a final C=D
#     alias row and two aliased data-write/self-encryption cases.
# - Usage:
...
"""Independent sixteenth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 16
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*o")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jj"),
    tuple(b"j*pj*ppppj/*jj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, sixteenth) for history in _HISTORIES for sixteenth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected sixteenth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "sixteenth.malbolge"
...
@pytest.mark.parametrize(("history", "sixteenth"), _CASES)
def test_sixteenth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every sixteenth opcode across three carried histories."""
    opcodes = (ord("<"), *history, sixteenth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            sixteenth in _ALIAS_WRITING_OPCODES
        )
