...
# Boundary-Contract:
# - Owns:
#   - Independent tenth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted ten-cell sources and analyzer CLI JSON reports.
...
# - Split-When:
#   - Eleventh-step differential reachability needs a separately bounded model.
# - Merge-When:
#   - Another independent verifier differential owns this exact tenth-step
#     public surface.
# - Summary:
#   - Compares the tenth bounded CLI transition to independent 1998 semantics.
# - Description:
#   - Covers every tenth opcode after `/oo<jjjj`, `j*pj*ppp`, and `o<o<<<<<`
#     histories.
# - Usage:
...
"""Independent tenth-transition differential tests for emitted Malbolge."""
...
_CASES: Final = tuple(
    (history, tenth) for history in _HISTORIES for tenth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = "selected tenth-step fixture reached a non-graphical fetch"
...
@pytest.mark.parametrize(("history", "tenth"), _CASES)
def test_tenth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int, int, int, int, int],
    tenth: int,
) -> None:
    """Compare every tenth opcode across three distinct carried histories."""
    opcodes = (ord("<"), *history, tenth)
    source_tuple = tuple(
