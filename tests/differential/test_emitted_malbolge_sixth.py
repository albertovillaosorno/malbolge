...
# Boundary-Contract:
# - Owns:
#   - Independent sixth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted six-cell sources and analyzer CLI JSON reports.
#   - Outputs: exact agreement with a self-contained historical six-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Seventh-step differential reachability needs a separately bounded model.
# - Merge-When:
#   - Another independent verifier differential owns this exact sixth-step
#     public surface.
# - Summary:
#   - Compares the sixth bounded CLI transition to independent 1998 semantics.
# - Description:
#   - Covers every sixth opcode after `/oo<`, `j*pj`, and `o<o<` histories.
# - Usage:
...
"""Independent sixth-transition differential tests for emitted Malbolge."""
...
_CASES: Final = tuple(
    (history, sixth) for history in _HISTORIES for sixth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = "selected sixth-step fixture reached a non-graphical fetch"
...
@pytest.mark.parametrize(("history", "sixth"), _CASES)
def test_sixth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int],
    sixth: int,
) -> None:
    """Compare every sixth opcode across three distinct carried histories."""
    opcodes = (ord("<"), *history, sixth)
    source_tuple = tuple(
