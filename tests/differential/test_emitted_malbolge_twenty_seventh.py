...
# Boundary-Contract:
# - Owns:
#   - Independent twenty-seventh-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twenty-seven-cell sources and analyzer CLI JSON
#     reports.
#   - Outputs: exact agreement with a self-contained historical
#     twenty-seven-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Twenty-eighth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact
#     twenty-seventh-step public surface.
# - Summary:
#   - Compares the twenty-seventh bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twenty-seventh opcode after `/oo<jjjjjjj*jjjjjjjjjjjjj`,
#     `j*pj*ppppj/*jjjjjjjjjjjjj`, and `o<o<<<<<</o<*oooooooooooo` histories,
#     including a final C=D alias row and two aliased data-write/self-encryption
#     cases.
# - Usage:
...
"""Twenty-seventh-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 27
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*oooooooooooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjjjjjjjjjj"),
    tuple(b"j*pj*ppppj/*jjjjjjjjjjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, twenty_seventh)
    for history in _HISTORIES
    for twenty_seventh in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected twenty-seventh-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twenty-seventh.malbolge"
...
@pytest.mark.parametrize(("history", "twenty_seventh"), _CASES)
def test_twenty_seventh_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every twenty-seventh opcode across three carried histories."""
    opcodes = (ord("<"), *history, twenty_seventh)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            twenty_seventh in _ALIAS_WRITING_OPCODES
        )
