...
# Boundary-Contract:
# - Owns:
#   - Independent twenty-first-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twenty-one-cell sources and analyzer CLI JSON
#     reports.
#   - Outputs: exact agreement with a self-contained historical
#     twenty-one-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Twenty-second-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact
#     twenty-first-step public surface.
# - Summary:
#   - Compares the twenty-first bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twenty-first opcode after `/oo<jjjjjjj*jjjjjjj`,
#     `j*pj*ppppj/*jjjjjjj`, and `o<o<<<<<</o<*oooooo` histories, including a
#     final C=D alias row and two aliased data-write/self-encryption cases.
# - Usage:
...
"""Twenty-first-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 21
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*oooooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjjjj"),
    tuple(b"j*pj*ppppj/*jjjjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, twenty_first)
    for history in _HISTORIES
    for twenty_first in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected twenty-first-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twenty-first.malbolge"
...
@pytest.mark.parametrize(("history", "twenty_first"), _CASES)
def test_twenty_first_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every twenty-first opcode across three carried histories."""
    opcodes = (ord("<"), *history, twenty_first)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            twenty_first in _ALIAS_WRITING_OPCODES
        )
