...
# Boundary-Contract:
# - Owns:
#   - Independent ninth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted nine-cell sources and analyzer CLI JSON reports.
...
# - Split-When:
#   - Tenth-step differential reachability needs a separately bounded model.
# - Merge-When:
#   - Another independent verifier differential owns this exact ninth-step
#     public surface.
# - Summary:
#   - Compares the ninth bounded CLI transition to independent 1998 semantics.
# - Description:
#   - Covers every ninth opcode after `/oo<jjj`, `j*pj*pp`, and `o<o<<<<`
#     histories.
# - Usage:
...
"""Independent ninth-transition differential tests for emitted Malbolge."""
...
_CASES: Final = tuple(
    (history, ninth) for history in _HISTORIES for ninth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = "selected ninth-step fixture reached a non-graphical fetch"
...
@pytest.mark.parametrize(("history", "ninth"), _CASES)
def test_ninth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int, int, int, int],
    ninth: int,
) -> None:
    """Compare every ninth opcode across three distinct carried histories."""
    opcodes = (ord("<"), *history, ninth)
    source_tuple = tuple(
