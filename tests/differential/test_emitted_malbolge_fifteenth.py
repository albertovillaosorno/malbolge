...
# Boundary-Contract:
# - Owns:
#   - Independent fifteenth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted fifteen-cell sources and analyzer CLI JSON
#     reports.
...
# - Split-When:
#   - Sixteenth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact fifteenth-step
#     public surface.
# - Summary:
#   - Compares the fifteenth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every fifteenth opcode after `/oo<jjjjjjj*j`, `j*pj*ppppj/*j`, and
#     `o<o<<<<<</o<*` histories, including a final C=D alias row and two aliased
...
"""Independent fifteenth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 15
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*")
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*j"),
    tuple(b"j*pj*ppppj/*j"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, fifteenth) for history in _HISTORIES for fifteenth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = "selected fifteenth-step fixture reached a non-graphical fetch"
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "fifteenth.malbolge"
...
@pytest.mark.parametrize(("history", "fifteenth"), _CASES)
def test_fifteenth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int, int, int, int, int, int, int, int, int, int],
    fifteenth: int,
) -> None:
    """Compare every fifteenth opcode across three carried histories."""
    opcodes = (ord("<"), *history, fifteenth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (fifteenth in b"*p")
        if fifteenth == ord("*"):
            assert final_expected.write_value == 39_405
            assert final_expected.encryption_input == 39_405
        if fifteenth == ord("p"):
            assert final_expected.write_value == 29_502
            assert final_expected.encryption_input == 29_502
