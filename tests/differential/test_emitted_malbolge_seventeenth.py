...
# Boundary-Contract:
# - Owns:
#   - Independent seventeenth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted seventeen-cell sources and analyzer CLI JSON
#     reports.
#   - Outputs: exact agreement with a self-contained historical
#     seventeen-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Eighteenth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact seventeenth-step
#     public surface.
# - Summary:
#   - Compares the seventeenth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every seventeenth opcode after `/oo<jjjjjjj*jjj`,
#     `j*pj*ppppj/*jjj`, and `o<o<<<<<</o<*oo` histories, including a final
#     C=D alias row and two aliased data-write/self-encryption cases.
# - Usage:
...
"""Seventeenth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 17
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*oo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjj"),
    tuple(b"j*pj*ppppj/*jjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, seventeenth) for history in _HISTORIES for seventeenth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected seventeenth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "seventeenth.malbolge"
...
@pytest.mark.parametrize(("history", "seventeenth"), _CASES)
def test_seventeenth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every seventeenth opcode across three carried histories."""
    opcodes = (ord("<"), *history, seventeenth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            seventeenth in _ALIAS_WRITING_OPCODES
        )
