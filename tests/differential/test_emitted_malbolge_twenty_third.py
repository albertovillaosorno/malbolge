...
# Boundary-Contract:
# - Owns:
#   - Independent twenty-third-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twenty-three-cell sources and analyzer CLI JSON
#     reports.
#   - Outputs: exact agreement with a self-contained historical
#     twenty-three-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Twenty-fourth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact
#     twenty-third-step public surface.
# - Summary:
#   - Compares the twenty-third bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twenty-third opcode after `/oo<jjjjjjj*jjjjjjjjj`,
#     `j*pj*ppppj/*jjjjjjjjj`, and `o<o<<<<<</o<*oooooooo` histories, including
...
"""Twenty-third-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 23
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*oooooooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjjjjjj"),
    tuple(b"j*pj*ppppj/*jjjjjjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, twenty_third)
    for history in _HISTORIES
    for twenty_third in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected twenty-third-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twenty-third.malbolge"
...
@pytest.mark.parametrize(("history", "twenty_third"), _CASES)
def test_twenty_third_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every twenty-third opcode across three carried histories."""
    opcodes = (ord("<"), *history, twenty_third)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            twenty_third in _ALIAS_WRITING_OPCODES
        )
