...
# Boundary-Contract:
# - Owns:
#   - Independent eighteenth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted eighteen-cell sources and analyzer CLI JSON
#     reports.
...
# - Split-When:
#   - Nineteenth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact eighteenth-step
#     public surface.
# - Summary:
#   - Compares the eighteenth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every eighteenth opcode after `/oo<jjjjjjj*jjjj`,
#     `j*pj*ppppj/*jjjj`, and `o<o<<<<<</o<*ooo` histories, including a final
...
"""Eighteenth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 18
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*ooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjj"),
    tuple(b"j*pj*ppppj/*jjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, eighteenth) for history in _HISTORIES for eighteenth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected eighteenth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "eighteenth.malbolge"
...
@pytest.mark.parametrize(("history", "eighteenth"), _CASES)
def test_eighteenth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every eighteenth opcode across three carried histories."""
    opcodes = (ord("<"), *history, eighteenth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            eighteenth in _ALIAS_WRITING_OPCODES
        )
