...
# Boundary-Contract:
# - Owns:
#   - Independent seventh-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted seven-cell sources and analyzer CLI JSON reports.
...
# - Split-When:
#   - Eighth-step differential reachability needs a separately bounded model.
# - Merge-When:
#   - Another independent verifier differential owns this exact seventh-step
#     public surface.
# - Summary:
#   - Compares the seventh bounded CLI transition to independent 1998 semantics.
# - Description:
#   - Covers every seventh opcode after `/oo<j`, `j*pj*`, and `o<o<<` histories.
# - Usage:
...
"""Independent seventh-transition differential tests for emitted Malbolge."""
...
_CASES: Final = tuple(
    (history, seventh) for history in _HISTORIES for seventh in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = "selected seventh-step fixture reached a non-graphical fetch"
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "seventh.malbolge"
...
@pytest.mark.parametrize(("history", "seventh"), _CASES)
def test_seventh_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int, int],
    seventh: int,
) -> None:
    """Compare every seventh opcode across three distinct carried histories."""
    opcodes = (ord("<"), *history, seventh)
    source_tuple = tuple(
