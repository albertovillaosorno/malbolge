...
# Boundary-Contract:
# - Owns:
#   - Independent twenty-eighth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twenty-eight-cell sources and analyzer CLI JSON
#     reports.
#   - Outputs: exact agreement with a self-contained historical
#     twenty-eight-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Twenty-ninth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact
#     twenty-eighth-step public surface.
# - Summary:
#   - Compares the twenty-eighth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twenty-eighth opcode after `/oo<jjjjjjj*jjjjjjjjjjjjjj`,
#     `j*pj*ppppj/*jjjjjjjjjjjjjj`, and `o<o<<<<<</o<*ooooooooooooo` histories,
...
"""Twenty-eighth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 28
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*ooooooooooooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjjjjjjjjjjj"),
    tuple(b"j*pj*ppppj/*jjjjjjjjjjjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, twenty_eighth)
    for history in _HISTORIES
    for twenty_eighth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected twenty-eighth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twenty-eighth.malbolge"
...
@pytest.mark.parametrize(("history", "twenty_eighth"), _CASES)
def test_twenty_eighth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every twenty-eighth opcode across three carried histories."""
    opcodes = (ord("<"), *history, twenty_eighth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            twenty_eighth in _ALIAS_WRITING_OPCODES
        )
