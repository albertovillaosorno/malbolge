...
# Boundary-Contract:
# - Owns:
#   - Independent thirteenth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted thirteen-cell sources and analyzer CLI JSON
#     reports.
...
# - Split-When:
#   - Fourteenth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact thirteenth-step
#     public surface.
# - Summary:
#   - Compares the thirteenth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every thirteenth opcode after `/oo<jjjjjjj`, `j*pj*ppppj/`, and
#     `o<o<<<<<</o` histories, including a final C=D alias row and one aliased
...
"""Independent thirteenth-transition differential tests for emitted Malbolge."""
...
_CASES: Final = tuple(
    (history, thirteenth) for history in _HISTORIES for thirteenth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected thirteenth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "thirteenth.malbolge"
...
@pytest.mark.parametrize(("history", "thirteenth"), _CASES)
def test_thirteenth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[int, int, int, int, int, int, int, int, int, int, int],
    thirteenth: int,
) -> None:
    """Compare every thirteenth opcode across three carried histories."""
    opcodes = (ord("<"), *history, thirteenth)
    source_tuple = tuple(
