...
# Boundary-Contract:
# - Owns:
#   - Independent twenty-fourth-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twenty-four-cell sources and analyzer CLI JSON
#     reports.
#   - Outputs: exact agreement with a self-contained historical
#     twenty-four-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Twenty-fifth-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact
#     twenty-fourth-step public surface.
# - Summary:
#   - Compares the twenty-fourth bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twenty-fourth opcode after `/oo<jjjjjjj*jjjjjjjjjj`,
#     `j*pj*ppppj/*jjjjjjjjjj`, and `o<o<<<<<</o<*ooooooooo` histories,
...
"""Twenty-fourth-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 24
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*ooooooooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjjjjjjj"),
    tuple(b"j*pj*ppppj/*jjjjjjjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, twenty_fourth)
    for history in _HISTORIES
    for twenty_fourth in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected twenty-fourth-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twenty-fourth.malbolge"
...
@pytest.mark.parametrize(("history", "twenty_fourth"), _CASES)
def test_twenty_fourth_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every twenty-fourth opcode across three carried histories."""
    opcodes = (ord("<"), *history, twenty_fourth)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            twenty_fourth in _ALIAS_WRITING_OPCODES
        )
