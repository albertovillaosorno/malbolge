...
# Boundary-Contract:
# - Owns:
#   - Independent twenty-second-transition differential evidence for the static
#     analyzer.
# - Must-Not:
#   - Import verifier helpers, import another differential semantic model, or
#     execute an unbounded guest loop.
# - Allows:
#   - Inputs: fixed admitted twenty-two-cell sources and analyzer CLI JSON
#     reports.
#   - Outputs: exact agreement with a self-contained historical
#     twenty-two-step model.
#   - Side effects: test-local source files and bounded subprocess execution.
# - Split-When:
#   - Twenty-third-step differential reachability needs a separately bounded
#     model.
# - Merge-When:
#   - Another independent verifier differential owns this exact
#     twenty-second-step public surface.
# - Summary:
#   - Compares the twenty-second bounded CLI transition to independent 1998
#     semantics.
# - Description:
#   - Covers every twenty-second opcode after `/oo<jjjjjjj*jjjjjjjj`,
#     `j*pj*ppppj/*jjjjjjjj`, and `o<o<<<<<</o<*ooooooo` histories, including
#     a final C=D alias row and two aliased data-write/self-encryption cases.
# - Usage:
...
"""Twenty-second-transition differential tests for emitted Malbolge."""
...
_TRANSITION_LIMIT: Final = 22
_ALIAS_HISTORY: Final = tuple(b"o<o<<<<<</o<*ooooooo")
_ALIAS_WRITING_OPCODES: Final = frozenset((ord("*"), ord("p")))
_HISTORIES: Final = (
    tuple(b"/oo<jjjjjjj*jjjjjjjj"),
    tuple(b"j*pj*ppppj/*jjjjjjjj"),
    _ALIAS_HISTORY,
)
_CASES: Final = tuple(
    (history, twenty_second)
    for history in _HISTORIES
    for twenty_second in _OPCODES
)
...
def _decode(value: int, code_pointer: int) -> int:
    if not _GRAPHICAL_START <= value <= _GRAPHICAL_END:
        message = (
            "selected twenty-second-step fixture reached a non-graphical fetch"
        )
...
def _report(tmp_path: Path, source: bytes) -> tuple[int, dict[str, object]]:
    path = tmp_path / "twenty-second.malbolge"
...
@pytest.mark.parametrize(("history", "twenty_second"), _CASES)
def test_twenty_second_transition_cli_matches_independent_historical_model(
    tmp_path: Path,
    history: tuple[
    ...
    """Compare every twenty-second opcode across three carried histories."""
    opcodes = (ord("<"), *history, twenty_second)
    source_tuple = tuple(
    ...
    if history == _ALIAS_HISTORY:
        ...
        assert final_expected.write_aliases_encryption == (
            twenty_second in _ALIAS_WRITING_OPCODES
        )
